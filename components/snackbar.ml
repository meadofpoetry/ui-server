...
class t ?start_aligned ?action ~message () =
  ...
  let elt = Markup.create ?start_aligned () |> To_dom.of_div in
  ...
  object(self)
    ...
    method set_message x =
      let new_data = { data with message = x } in
      data <- new_data; data_obj <- data_to_js_obj new_data

    method timeout = data.timeout
    method set_timeout x =
      let new_data = { data with timeout = Some x } in
      data <- new_data; data_obj <- data_to_js_obj new_data
    ...
    method set_multiline x =
      let new_data = { data with multiline = x } in
      data <- new_data; data_obj <- data_to_js_obj new_data

    method action_on_bottom = data.action_on_bottom
    method set_action_on_bottom x =
      let new_data = { data with action_on_bottom = x } in
      data <- new_data; data_obj <- data_to_js_obj new_data
