...
module Label = struct
  ...
  class t ~s_selected ~label () =
    let elt = Markup.Select.Label.create ~label () |> Tyxml_js.To_dom.of_element in
    object(self)
      inherit Widget.widget elt ()
      initializer
        React.S.map (fun v -> self#add_or_remove_class (Option.is_some v) Markup.Select.Label.float_above_class)
                    s_selected
        |> ignore
    end
...
module Bottom_line = struct
  ...
  class t () =
    let elt = Markup.Select.Bottom_line.create () |> Tyxml_js.To_dom.of_element in
    object(self)
      inherit Widget.widget elt ()

      method activate (x:bool) = self#add_or_remove_class x Markup.Select.Bottom_line.active_class
    end
...
class ['a] t ?(disabled=false)
  ...
  object(self)
    ...
    method set_selected_index i =
      ...
      self#add_class Markup.Select.is_changing_class;
      Dom_html.setTimeout (fun () -> self#remove_class Markup.Select.is_changing_class) 125.0 |> ignore;
    ...
    method set_disabled (x:bool) : unit =
      self#add_or_remove_class x Markup.Select.disabled_class;
