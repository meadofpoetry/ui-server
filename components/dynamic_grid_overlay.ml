  ...
  object(self)
    ...
    method layout ~cw ~cn ~rh ~rn ~(im: int * int) =
      ...
      let cols_rows =
        if cw > 2
        then (
          let cols_count    = CCList.range 0 cn in
          let cols : image =
            CCList.fold_left (fun acc num ->
                let img = self#draw_line ~pos:one_col_pos ~num ~typ:Row ~w:col_border
                in
                I.blend img acc) empty cols_count
          in
          let rows_count    = CCList.range 0 rn in
          let rows : image  =
            CCList.fold_left (fun acc num ->
                let img = self#draw_line ~pos:one_row_pos ~num ~typ:Column ~w:row_border
                in
                I.blend img acc) empty rows_count
          in
          I.blend cols rows)
        else filler
      in
    ...
    method private size x y = Size2.v (float_of_int x *. 0.26458333) (float_of_int y *. 0.26458333)
    ...
    method private draw_line ~(pos : float) ~(num : int) ~(typ : line) ~(w : float) =
