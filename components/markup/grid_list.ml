  ...
  open Html
    ...
    module Tile = struct
      ...
      module Primary = struct
        ...
        let create_content ?(classes=[]) ?attrs ?src ?alt ?(is_div=false) () =
          if not is_div
          then img ~src:(Html.uri_of_string (Option.get_or ~default:"" src))
                 ...
                 ~a:([ a_class (content_class :: classes) ] <@> attrs) ()
                            ...
                            | Some x -> Some (Printf.sprintf "background-image: url(%s);" x)
                            | None   -> None) in
      ...
      module Caption = struct
        ...
        let create_title ?(classes=[]) ?attrs ~text () =
          span ~a:([ a_class (title_class :: classes) ] <@> attrs) [pcdata text]

        let create_support_text ?(classes=[]) ?attrs ~text () =
          span ~a:([ a_class (support_text_class :: classes) ] <@> attrs) [pcdata text]

        let create ?(classes=[]) ?attrs ?title ?support_text ?icon () =
      ...
      let create ?(classes=[]) ?attrs ?caption ~primary () =
