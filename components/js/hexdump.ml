...
module Markup = Components_tyxml.Hexdump.Make(Xml)(Svg)(Html)
...
let make_style ?(line_number = { prefix = ""; suffix = "" })
      ?(hex = {prefix = ""; suffix = "" })
      ?(char = {prefix = ""; suffix = "" })
      ?(empty_hex = '.')
      ?(empty_char = ' ')
...
let to_line_number i (config:config) =
  ...
  Tyxml.Html.(
    span ~a:[a_class [Markup.Line_number._class]] [txt s]
...
let to_hex_span ?value (config:config) =
  ...
  let udata_id, udata_val, s, _class = match value with
    ...
    | Some (id, v) ->
       ...
       "hex-id", string_of_int id, s, Markup.Hex._class in
  Tyxml.Html.(
    span ~a:[a_class [_class]; a_user_data udata_id udata_val] [txt s]
    |> Format.asprintf "%a" (pp_elt ()))
