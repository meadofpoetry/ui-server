...
let on_load (t : 'a t) (response : 'a) : unit =
  ...
  if t.options.is_last_page response
  then t.can_load <- false;
...
let load_next_page (t : 'a t) : unit =
  ...
  then (
    ...
    let thread =
      Lwt.try_bind (fun () -> t.options.get args)
        (function
         | Ok x -> Lwt.return @@ on_load t x
         | Error e -> Lwt.return @@ on_error t e)
        (fun exn -> Lwt.return @@ on_error t @@ Printexc.to_string exn) in
    t.response <- thread)
...
let get_prefill_distance (t : 'a t) : int =
  match t.options.scroll_target with
  | None ->
     t.window_height - t.options.element##.clientHeight
  | Some target ->
