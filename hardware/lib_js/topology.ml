...
module Port = struct
  ...
  let draw_input t (acc, acc_top) x y cols rows size =                      (*drawing an input element*)
    let font    =
      Font.{ name = ""; slant = `Normal; weight = `W100; size = 18. /. size } in
    let acc_top = setting (Input t) x y y 1. acc_top in                (*setting a name of input to the list*)
    let area = `O { P.o with P.width = 1. /. size } in       (*a void area with given width of border*)
  ...
  let draw_line acc colour x1 y_1 x2 y2 cols rows sz =                    (*draws a connecting line between elements*)
    ...
    let area    = `O { P.o with P.width = 3. /. sz } in
...
module Board = struct
  ...
  let rec draw t (acc, acc_top) x y cols rows sz =
    let font       = Font.{ name = ""; slant = `Normal; weight = `W400; size = 10. /. sz} in
    ...
    let area       = `O {P.o with P.width = 2. /. sz} in
    ...
    | l  ->
       CCList.foldi (fun (acc,acc_top) i z ->
           ...
           | TS2IP ->
              ...
              let draw_line x1 = (Port.draw_line acc colour1 x1 y_child x (y_child +. 0.5) cols rows sz),
...
let render ?on_click ~topology ~(width : int) ~canvas () =
  ...
  let sz     = sqrt (float_of_int @@ rows * rh * cols * cw) in
