...
module Query = struct
  ...
  module Time = struct
    ...
    type t  = [ `Abs of Time.t
              | `Rel of Time.Relative.t
              ] [@@deriving eq]

    (** [to_abs t] converts [t] to absolute timestamp of type [Ptime.t]
     **)
    let to_abs : t -> Time.t = function
      | `Rel x -> Time.Relative.to_absolute x
      | `Abs x -> x

    (** [to_string t] converts [t] to string which can be inserted into URI query
     **)
    let to_string : t -> string = function
      | `Rel x -> Time.Relative.to_string x
      | `Abs x -> Time.to_rfc3339 x
    ...
    module Range = struct
      ...
      let to_time = function
        | `Now                 -> None, None
        | `Past (`From x)      -> Some x, None
        | `Past (`Till x)      -> None, Some x
        | `Past (`Range (f,t)) -> Some f, Some t
