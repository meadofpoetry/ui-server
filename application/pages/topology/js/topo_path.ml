...
class switch (port : Topology.topo_port) setter () =
  ...
  let elt = Tyxml_js.To_dom.of_element @@ Components.Switch.D.switch () in
  object (self)
    ...
    method set_state (x : connection_state) : unit =
      ...
      then
        ...
        | `Muted ->
            ...
            super#set_disabled false
        | `Unavailable -> super#set_disabled true
...
let active_class = Components.BEM.add_modifier _class "active"

let muted_class = Components.BEM.add_modifier _class "muted"

let sync_class = Components.BEM.add_modifier _class "sync"
    ...
    method! layout () : unit =
      ...
      Option.iter
        (fun sw ->
          ...
          sw#root##.style##.left := Js.string @@ Printf.sprintf "%dpx" x)
        switch;
