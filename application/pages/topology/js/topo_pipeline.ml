...
let make_streams (cpu : Topology.topo_cpu) socket =
  ...
  get_streams () >>=? fun init ->
  Event.get_streams socket >>=? fun (id, event) ->
...
let make_structure socket =
  ...
  let notif =
    React.E.merge
      (fun _ -> w#notify)
      ()
      [ React.E.map (fun x -> `Structure x) e ]
  in
...
let make (cpu : Topology.topo_cpu) (socket : Api_js.Websocket.JSON.t) =
  let wrap f () =
    ...
    @@ Components_lab.Loader.make_widget_loader (f socket)
  in
  let tabs =
    [
      ...
      ( `Fun (wrap make_structure),
        Components.Tab.D.tab ~text_label:"Выбор PID" () );
    ]
  in
