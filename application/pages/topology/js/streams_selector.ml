...
module Board = struct
  ...
  let make (board : Topology.topo_board option)
    ...
    let left =
      ...
      | `Limited lim -> Some (React.S.map ~eq:( = ) (fun x -> lim - x) counter)
    in
    let w, settings =
      List.map
        (fun (i : topo_input) ->
          let filtered_streams =
            List.filter
              ...
              stream_list
          in
          make_entry state counter counter_push left filtered_streams i)
        inputs
      |> List.split
    in
...
module Input = struct
  ...
  let make_stream_list stream_list =
    ...
    let add stream =
      let slst = React.S.value signal in
      if List.exists (Stream.equal stream) slst then failwith "stream exists";
      (* TODO fix *)
...
let make_entry :
 ...
 fun cpu st ->
  ...
  | `Board id ->
      let board =
        List.find_opt (fun (b : Topology.topo_board) -> b.control = id)
        @@ Topology.get_boards (`CPU cpu)
      in
      Board.make board st
...
class t ~(init : Stream.stream_table) ~(event : Stream.stream_table React.event)
  ...
  in
  let e_div = React.S.diff (fun _ (o, _) -> o#destroy ()) s_div in
...
let make ~init ~event cpu () = new t ~init ~event cpu
