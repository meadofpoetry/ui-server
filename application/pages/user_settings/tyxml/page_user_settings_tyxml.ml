...
struct
  ...
  module Account = struct
      ...
      | `Guest ->
          ...
          Printf.sprintf
            "%s предоставлена возможность просмотра \
             ...
             запись не позволяет управлять \
             параметрами мониторинга и настройками \
             прибора."
            title
      | `Operator ->
    ...
    let make_greetings ?(classes = []) ?(attrs = []) user =
      ...
      let icon = Icon.SVG.(create_of_d (user_icon_path user)) in
    ...
    let make ?(classes = []) ?(attrs = []) user =
      ...
      make_section
        ...
        [ Card.create_media
            [make_greetings user; make_permissions user; make_accounts_info_link ()]
            ()
  ...
  module Password = struct
    ...
    let make_textfield ?(autocomplete = "off") ?value ~name ~label ~id () : 'a elt =
      let id' = id ^ "-input" in
      let icon =
        div
          ~a:[a_class [Textfield.CSS.icon]; a_role ["button"]; a_tabindex 0]
          [Icon.SVG.create_of_d Svg_icons.eye_off]
      in
      let input =
        Unsafe.coerce_elt
      ...
      Textfield.create ~attrs:[a_id id] ~trailing_icon:icon ~input ~label ~line_ripple ()

    let make_user_tabs () =
      let create_tab ?active user =
        let icon =
          Icon.SVG.(create_of_d ~classes:[Tab.CSS.icon] (user_icon_path user))
        in
        let username = Application_types.User.to_string user in
        let indicator =
          Tab_indicator.create ?active (Tab_indicator.create_content ()) ()
        in
        let username_human = Format.asprintf "%a" pp_user_human user in
    ...
    let make_helper_text ?(persistent = true) text =
      Textfield.create_helper_line
        [Textfield.Helper_text.create ~validation:true ~persistent ~text ()]
    ...
    let make ?(classes = []) ?(attrs = []) () =
      ...
      make_section
        ...
        ; Card.create_media
            [ div
                ~a:[a_class [CSS.Password.slider]]
                [make_form `Guest; make_form `Operator; make_form `Root] ]
            ()
