...
struct
  ...
  let create_textfield ?(autocomplete = "off") ?value ~name ~label ~id () :
      'a elt =
    let id' = id ^ "-input" in
    let icon =
      div
    ...
    let input =
      Unsafe.coerce_elt
      @@ Textfield_markup.textfield_input ~id:id'
           ~a:
             [
           ...
           ~typ:`Password ?value ()
    in
    Textfield_markup.textfield ~a:[ a_id id ] ~trailing_icon:icon ~input_id:id'
      ~input ~label ()
  ...
  let create_user_tabs () =
    let create_tab ?active user =
      ...
      Tab_markup.tab ?active ~icon ~text_label:username_human
  ...
  let create_helper_text ?(persistent = true) text =
    Textfield_markup.(
      textfield_helper_line
        ~children:
          [ Helper_text.helper_text ~validation:true ~persistent ~text () ]
        ())
    ...
    form
      ...
      [
        ...
        text_field_container
          [
            create_textfield ~id:(old_pass_id user) ~name:"current_password"
        ...
        text_field_container
          [
            create_textfield ~id:(new_pass_id user) ~name:"new_password"
              ~autocomplete:"new-password" ~label:"Новый пароль" ();
            create_helper_text "Минимум 4 символа";
          ];
        text_field_container
          [
            create_textfield ~id:(confirm_pass_id user) ~name:"confirm_password"
  ...
  let create ?(classes = []) ?(a = []) () =
    let submit =
      Button_markup.button ~classes:[ Card.CSS.action ] ~appearance:Raised
        ~button_type:`Submit ~label:"Сменить пароль" ()
    in
      ...
      ~children:
        [
          ...
          Card_markup.card_actions
            ~children:
              [ Card_markup.card_action_buttons ~children:[ submit ] () ]
            ();
        ]
      ()
end
