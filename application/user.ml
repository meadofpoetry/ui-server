...
type users = { root : userdata; operator : userdata; guest : userdata }
[@@deriving yojson, eq]
...
module Config = struct
  ...
  let of_string s =
    Yojson.Safe.from_string s |> users_of_yojson |> function
    | Ok v -> v
    | Error e -> failwith e
end
...
let set_pass (storage : passwd) (pass_entry : pass) =
  ...
  | `Root ->
      storage#set { table with root = { pass = encrypt pass_entry.password } }
  | `Operator ->
      storage#set
        { table with operator = { pass = encrypt pass_entry.password } }
  | `Guest ->
      storage#set { table with guest = { pass = encrypt pass_entry.password } }
...
let validate (storage : passwd) ~name ~pass =
  let open Lwt.Infix in
  Application_types.User.of_string name |> function
  | Error e -> Lwt.return_error (`Unknown e)
  | Ok user ->
      get_pass storage user >>= fun actual_pass ->
      if String.equal (encrypt pass) actual_pass.pass then Lwt.return_ok user
