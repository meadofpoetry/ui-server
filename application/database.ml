...
module SID = struct
  ...
  let of_stream_id (id : ID.t) = ID.to_string id

  let to_stream_id (t : string) = ID.of_string t
  ...
  let select db ?(boards = []) ?(cpu = []) ?(inputs = []) ?(streams = []) ?limit
    ...
    let inputs =
      let to_string (i : topo_input) =
        Printf.sprintf "(%d, %d)" i.id (input_to_enum i.input)
      in
      is_null_or_in "(input_id, input_type)" to_string inputs
    in
    let limit =
      match limit with
      | None -> max_limit
    ...
    let select' =
      R.collect
        ...
        log_entry
        (sprintf
           {|SELECT date,level,message,info,board,cpu,
