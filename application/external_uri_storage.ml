...
type lst =
  (Application_types.Stream.marker * Application_types.Stream.Table.setting list)
  list
[@@deriving yojson]
...
let equal_settings l r =
  ...
  try
    List.iter2
      (fun l r -> if not (equal_setting l r) then raise_notrace Exit)
      l r;
    true
  with _ -> false
...
let to_string c =
  Yojson.Safe.to_string @@ lst_to_yojson @@ List.of_seq @@ Map.to_seq c

let of_string s =
  let open Result in
  Yojson.Safe.from_string s |> lst_of_yojson |> function
  | Error e -> failwith e
  | Ok lst -> Map.of_seq @@ List.to_seq lst
