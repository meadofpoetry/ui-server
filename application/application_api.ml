...
let set_streams (app : Application.t) _user body _env _state =
  ...
  | Ok s -> (
      ...
      | Error ejs ->
          ...
          Lwt.return
            (`Error
              (Yojson.Safe.to_string @@ Stream.Table.set_error_to_yojson ejs)) )

let get_topology (app : Application.t) _user _body _env _state =
  app.topo |> React.S.value |> Topology.to_yojson |> fun v ->
  Lwt.return (`Value v)

let get_streams (app : Application.t) _user _body _env _state =
  app.hw.streams |> React.S.value |> Stream.stream_table_to_yojson |> fun v ->
  Lwt.return (`Value v)
...
let get_log (app : Application.t) boards cpu inputs streams limit from till
    duration order _user _body _env _state =
  match Interval.make ?from ?till ?duration () with
  | Ok (`Range (from, till)) ->
      Database.Log.select app.db ~boards ~cpu ~inputs ~streams ?limit ?order
        ~from ~till ()
      |> Lwt.map
           (Api.rows_to_yojson
              (Util_json.List.to_yojson Stream.Log_message.to_yojson) (fun () ->
                `Null))
