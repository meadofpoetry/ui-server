...
let common_tabs () =
  [
    ( Netlib.Uri.Path.of_string "log",
      Api_template.make_template_props ~title:"Лог" () );
  ]
  ...
  List.fold_left
    (fun acc (_, (board : Board.t)) ->
      let has_input =
        List.exists
          (function
            | `Input i, _ -> Topology.equal_topo_input input i | _ -> false)
          board.gui_tabs
      in
      if has_input then
...
let make_template (input : Topology.topo_input) (cpu : Data_processor.t option)
    (boards : Board.t Board.Map.t) (_topology : Topology.t) =
  let path =
    Option.value ~default:[] (Topology.board_list_for_input input _topology)
  in
  let boards =
  ...
  in
  let variables =
    Printf.sprintf "var input = `%s`; var boards = `%s`;"
  ...
  let eq a b =
    ...
    | _ -> false
  in
  Api_template.make_template_props ~title
    ~stylesheets:(dedup ~eq:String.equal stylesheets)
    ~pre_scripts:(dedup ~eq pre_scripts) ~post_scripts:(dedup ~eq post_scripts)
