...
let error_page_props error =
  let error_code, status, message =
    ...
    | `Forbidden ->
        ( "403"
        , "Доступ запрещён"
        , "Похоже, что доступ к данной странице Вам \
           запрещён... Не расстраивайтесь, Вы можете \
           вернуться на " )
  in
  Api_template.make_template_props
...
let application_ws (app : Application.t) =
  ...
  make
    ~prefix:"application"
    [ event_node
        ~doc:"Pushes device topology to the client"
        ~path:Path.Format.("topology" @/ empty)
        ~query:Query.empty
        (Application_api.Event.get_topology app)
    ; event_node
        ~doc:"Pushes stream table to the client"
        ~path:Path.Format.("stream-table" @/ empty)
        ~query:Query.empty
