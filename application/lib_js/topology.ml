...
let get_node_depth t =
  ...
  | `Boards x ->
    succ @@ find_max
      (List.map (fun x ->
           succ @@ find_max
             (List.map (fun x -> get_entry_depth 0 x.child) x.ports)) x)
...
let grid_template_areas t =
  ...
  match t with
  | `CPU x    ->
    concat (List.map (fun x -> get_entry_areas ". CPU" 1 x.conn) x.ifaces)
  | `Boards x ->
    concat (List.map (fun board ->
...
let iter_paths f nodes =
  List.iter (function
      | `Board b ->
        List.iter (fun p -> f (b :> Topo_node.t) p) b#paths
      | `CPU c   ->
        List.iter (fun p -> f (c :> Topo_node.t) p) c#paths
      | _        -> ()) nodes
...
let update_nodes nodes (t : Common.Topology.t) =
  ...
  List.iter (function
      | `Board b ->
        (match List.find_opt (f b) boards with
