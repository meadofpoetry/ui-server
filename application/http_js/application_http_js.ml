...
module Body = struct
  ...
  let of_event evt = of_string @@ Js_of_ocaml.Js.to_string evt##.data
end
...
module Event = struct
  ...
  let get_topology sock =
    Api_js.Websocket.JSON.subscribe
      ...
      ~query:Query.empty Topology.of_yojson sock

  let get_streams sock =
    Api_js.Websocket.JSON.subscribe
      ~path:Path.Format.("application/stream-table" @/ empty)
      ~query:Query.empty Stream.stream_table_of_yojson sock
...
let get_log ?(boards = []) ?(cpu = []) ?(inputs = []) ?(streams = []) ?limit
    ?from ?till ?duration ?order () =
  Api_http.perform
    ...
    boards cpu inputs streams limit from till duration order
    (fun _env -> function Error e -> Lwt.return_error e
      | Ok x -> (
          let of_json =
            Api.rows_of_yojson
              (Util_json.List.of_yojson Stream.Log_message.of_yojson) (fun _ ->
                Ok [])
          in
