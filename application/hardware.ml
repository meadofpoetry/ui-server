...
let equal_list compare l r =
  ...
  try
    List.iter2 (fun l r -> if not (compare l r = 0) then raise_notrace Exit) l r;
    true
  with _ -> false
...
module Input_map = Map.Make (struct
  open Topology
  ...
  let compare (li, lid) (ri, rid) =
    ...
    if ci <> 0 then ci else Stdlib.compare lid rid
end)
...
let topo_to_signal topo (boards : Board.t Board.Ports.t) :
    Topology.t React.signal =
  ...
  let build_board b connection ports =
    ...
    S.l2 ~eq
      (fun a p -> Board { b with connection = a; ports = p })
      connection ports
  in
  let merge_ports lst =
    let eq = equal_topo_port in
    List.map
      (fun (port, sw, list, sync, child) ->
        S.l3 ~eq
          (fun l s c ->
            { port; listening = l; has_sync = s; child = c; switchable = sw })
          list sync child)
      lst
    |> S.merge ~eq:(equal_list Topology.compare_topo_port) cons []
  in
  let rec entry_to_signal = function
    ...
    | Board b ->
        let connection, port_list, sync_list =
          ...
          | None ->
              Printf.printf "board with control=%d not found\n" b.control;
              raise Not_found
          | Some state ->
              ( state.connection,
                get_port state.ports_active,
                get_port state.ports_sync )
        in
        let ports =
          merge_ports
          @@ List.map
  ...
  match topo with
  | `CPU c ->
      React.S.map ~eq:Topology.equal (fun x -> `CPU x) @@ cpu_to_signal c
  | `Boards l ->
      ...
      |> React.S.map ~eq:Topology.equal (fun x ->
             `Boards
               (List.filter_map
                  (function Input _ -> None | Board b -> Some b)
                  x))
...
let get_sources (topo : Topology.topo_entry list) uri_table boards =
        ...
        let s, push =
          ...
          let (init : Stream.Table.stream list) =
            ...
            >|= List.map (fun (x : Stream.Table.setting) ->
                    Stream.Table.
                      { url = Some x.url; stream = x.stream; present = false })
            |> function
            | None -> []
            | Some x -> x
          in
          React.S.create ~eq init
        in
        let push' xs =
          List.map
            (fun ({ url; stream } : Stream.Table.setting) ->
              Stream.Table.{ url = Some url; stream; present = false })
            xs
          |> push
        in
        let controls =
          {
            controls with
            inputs = Input_map.add (i.input, i.id) push' controls.inputs;
          }
        in
        let signals =
          S.map ~eq:eq_row (fun s -> (input, `Unlimited, s)) s :: signals
        in
        get_sources' controls signals tl
    | Board b :: tl -> (
        let stream_handler =
          Board.Ports.find_opt b.control boards >>= fun (b : Board.t) ->
          b.stream_handler
        in
        ...
        | Some h ->
            ...
            let controls =
              {
                controls with
                boards = Board.Ports.add b.control h controls.boards;
              }
            in
            let signals =
              React.S.l2 ~eq:eq_row
                (fun s st -> (board, st, s))
  ...
  let controls, signals =
    get_sources'
      { inputs = Input_map.empty; boards = Board.Ports.empty }
      [] topo
  in
  let signals' =
    S.merge ~eq:Stream.equal_stream_table (fun acc s -> s :: acc) [] signals
  in
  (controls, signals')
...
let store_external_uris (storage : Uri_storage.t) streams =
  ...
  let streams =
    List.map
      ...
      streams
  in
  List.to_seq streams |> External_uri_storage.Map.of_seq |> storage#set
...
let create kv db (topo : Topology.t) =
  ...
  Uri_storage.create ~default:External_uri_storage.default kv
    [ "application"; "uri_storage" ]
  >>=? fun uri_storage ->
  ...
  | boards ->
      Usb_device.create ~sleep:step_duration () >>= fun (usb, loop) ->
      Lwt_list.fold_left_s
        (fun m (b : topo_board) ->
          match m with
          | Error e -> Lwt.return_error e
          | Ok m ->
              create_board db usb b m kv >>=? fun (board : Board.t) ->
...
let set_stream ?(port = 1234) (hw : t) (ss : Stream.stream_setting) =
  ...
  let gen_uris (ss : Stream.stream_setting) :
    ...
    let split = function
      ...
      | `Board id, sl -> (
          try
            let p = Board.Ports.find id hw.sources.boards in
            let Board.{ range; state = _ } = p#constraints in
            `Left (List.map (fun s -> ((`Board id, s), range)) sl)
          with Not_found ->
            raise_notrace
              (Constraints (`Internal_error "set_stream: no control found")) )
    in
    let rec rebuild_boards port' acc streams = function
      ...
      | (`Board id, _) :: tl ->
          let same, rest =
            partition_map
              (function
                | ((`Board bid, bs), buri) as v ->
                    if id = bid then
                      let url =
                        ( ( Uri.empty |> fun uri ->
                            Uri.with_host uri (Some (Ipaddr.V4.to_string buri))
                          )
              ...
              streams
          in
          rebuild_boards port' ((`Board id, same) :: acc) rest tl
    in
    let input_add_uri (`Input i, sl) =
      ...
      let s_to_uri s =
        ...
        | TSoIP x ->
            let url =
              ( ( Uri.empty |> fun uri ->
                  Uri.with_host uri (Some (Ipaddr.V4.to_string x.addr)) )
        ...
        | _ ->
            raise_notrace
              (Constraints
                 (`Internal_error
                   "set_stream: expected ip stream from an input"))
      in
      (`Input i, List.map s_to_uri sl)
    in
    ...
    let check_inputs l =
      let rec check = function
        | [] -> ()
        | x :: tl ->
            if List.exists (Stream.equal x) tl then
              raise_notrace
                (Constraints
                   (`Internal_error "set_stream: input streams duplication"));
            check tl
      in
      List.iter (fun (_, l) -> check l) l
    in
    let boards, inputs = partition_map split ss in
    check_inputs inputs;
    let inputs = List.map input_add_uri inputs in
    let forbidden = grep_input_uris [] inputs |> List.filter_map Uri.host_v4 in
    let boards =
      (*match Ipaddr.V4.gen_in_ranges ~forbidden (List.concat boards) with*)
      Ipaddr.V4.gen_in_ranges ~forbidden ~allowed:Netlib.Ipaddr.V4.multicast
        (List.concat boards)
  ...
  let check_constraints range state streams =
    ...
    if range = [] then ()
      ...
      List.iter
        (fun ({ url; _ } : Stream.Table.setting) ->
          if
            not
            @@ List.exists
                 (fun r ->
                   ...
                   | None -> false
                   | Some ip -> Ipaddr.V4.in_range r ip)
                 range
          then raise_notrace (Constraints `Not_in_range))
        streams
  in
  let rec loop l _res =
    match l with
    ...
    | (`Input k, streams) :: tl -> (
        try
          let p = Input_map.find k hw.sources.inputs in
          p streams;
          Lwt.return_ok () >>= loop tl
        with Not_found ->
          Lwt.return_error (`Internal_error "set_stream: no control found")
          >>= loop tl )
    | (`Board k, streams) :: tl -> (
        try
          let p = Board.Ports.find k hw.sources.boards in
...
let finalize hw =
  ...
  (match hw.usb with None -> () | Some usb -> Usb_device.finalize usb);
  Board.Map.fold
    (fun _ (b : Board.t) acc -> acc >>= b.state#finalize)
    hw.boards Lwt.return_unit
