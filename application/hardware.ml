...
let topo_to_signal topo (boards : Board.t Board.Ports.t) : Topology.t React.signal =
  ...
  let build_board b connection ports =
    ...
    S.l2 ~eq (fun a p -> Board {b with connection = a; ports = p}) connection ports
  in
  let merge_ports lst =
    ...
    List.map
      (fun (port, sw, list, sync, child) ->
        S.l3
          ~eq
          (fun l s c -> {port; listening = l; has_sync = s; child = c; switchable = sw})
          list
          sync
          child)
      lst
    ...
    | Board b ->
        let connection, port_list, sync_list =
          ...
          | None ->
              ...
              raise Not_found
          | Some state ->
              state.connection, get_port state.ports_active, get_port state.ports_sync
        in
        let ports =
          merge_ports
          @@ List.map
  ...
  in
  match topo with
  | `CPU c -> React.S.map ~eq:Topology.equal (fun x -> `CPU x) @@ cpu_to_signal c
  | `Boards l ->
...
let get_sources (topo : Topology.topo_entry list) uri_table boards =
  ...
  let rec get_sources' controls signals = function
    ...
    | Input i :: tl ->
        ...
        let s, push =
          ...
          let (init : Stream.Table.stream list) =
            ...
            >|= List.map (fun (x : Stream.Table.setting) ->
                    Stream.Table.{url = Some x.url; stream = x.stream; present = false})
            |> function
            | None -> []
            | Some x -> x
          in
        ...
        in
        let controls =
          {controls with inputs = Input_map.add (i.input, i.id) push' controls.inputs}
        in
        let signals = S.map ~eq:eq_row (fun s -> input, `Unlimited, s) s :: signals in
        get_sources' controls signals tl
    | Board b :: tl -> (
        let stream_handler =
          Board.Ports.find_opt b.control boards >>= fun (b : Board.t) -> b.stream_handler
        in
        ...
        | Some h ->
             ...
             * |> ignore; *)
            let controls =
              {controls with boards = Board.Ports.add b.control h controls.boards}
            in
            let signals =
              React.S.l2
                ~eq:eq_row
  ...
  let controls, signals =
    get_sources' {inputs = Input_map.empty; boards = Board.Ports.empty} [] topo
  in
  let signals' =
...
let store_external_uris (storage : Uri_storage.t) streams =
  ...
  let filter_streams =
    List.filter_map (function
        | ({url = Some url; stream; _} : stream) -> Some ({url; stream} : setting)
        | _ -> None)
  in
  let streams =
    List.map
...
let set_stream ?(port = 1234) (hw : t) (ss : Stream.stream_setting) =
    ...
    let input_add_uri (`Input i, sl) =
      ...
      let s_to_uri s =
        ...
        | _ ->
            raise_notrace
              (Constraints
                 (`Internal_error "set_stream: expected ip stream from an input"))
      in
    ...
    let check_inputs l =
      let rec check = function
        ...
        | x :: tl ->
            ...
            then
              raise_notrace
                (Constraints (`Internal_error "set_stream: input streams duplication"));
            check tl
      in
  ...
  let check_constraints range state streams =
    ...
    | `Limited l ->
        ...
        if l >= len then () else raise_notrace (Constraints (`Limit_exceeded (l, len)))
    | _ -> ());
    if range = []
    then ()
    else
  ...
  let rec loop l _res =
    ...
    | (`Input k, streams) :: tl -> (
      ...
      with Not_found ->
        Lwt.return_error (`Internal_error "set_stream: no control found") >>= loop tl)
    | (`Board k, streams) :: tl -> (
      try
