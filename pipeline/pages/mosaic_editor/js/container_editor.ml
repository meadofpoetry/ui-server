...
let init_top_app_bar_icon (scaffold : Scaffold.t) =
  ...
  | Some x -> (
      let icons =
        Element.query_selector_all x @@ Printf.sprintf ".%s" Icon.CSS.root
      in
      match icons with
...
let set_top_app_bar_icon (scaffold : Scaffold.t) typ icon =
  ...
  | Some x ->
      ...
      (match prev with None -> () | Some prev -> Dom.removeChild x prev);
      Element.add_class icon class_;
      Element.insert_child_at_index x index icon;
      prev

let filter_available_widgets (widgets : (string * Wm.widget) list)
...
class t ~(scaffold : Scaffold.t) (streams : Stream.t list)
  ...
  object (self)
    ...
    val back_icon =
      Tyxml_js.To_dom.of_element @@ Icon.SVG.(D.icon ~d:Path.arrow_left ())
    ...
    method! layout () : unit =
      ...
      in
      ( if frame_aspect < content_aspect then (
        (* Letterbox *)
        let height =
          Js.math##round (frame_height *. frame_aspect /. content_aspect)
        in
        ...
        (* Pillarbox *)
        let width =
          Js.math##round (frame_width *. content_aspect /. frame_aspect)
        in
    ...
    method private switch_to_widget_mode (cell : Dom_html.element Js.t) =
      ...
      in
      let editor =
        Widget_editor.make ~scaffold ~resolution ~position ~list_of_widgets
    ...
    method private handle_dragenter e _ =
      let target = Dom_html.eventTarget e in
      if
        Element.has_class target Grid.CSS.cell
    ...
    method private handle_drop e _ =
      ...
      Js.Opt.iter drag_target (fun dragged ->
          if not @@ Element.equal dragged target then (
            swap dragged target;
            let action =
              {
    ...
    method private handle_selected cells =
      ...
      | cells -> (
          ...
          in
          let restore =
            Actions.transform_top_app_bar ~title
    ...
    method private create_top_app_bar_menu () : Components_lab.Overflow_menu.t =
      Actions.Containers.make_menu (fst s_state)
        ...
        undo_manager wizard_dialog scaffold grid

    method private create_main_actions () =
      let submit =
        Button.make ~label:"Сохранить"
          ~on_click:(fun btn _ _ ->
            ...
            | Ok _ ->
                ...
                let snackbar =
                  Snackbar.make ~dismiss:`True ~label:(`Text label) ()
                in
    ...
    method private update_widget_elements (widgets : Dom_html.element Js.t list)
        (cell : Dom_html.element Js.t) : unit =
      let wrapper =
        match Element.query_selector cell Selector.widget_wrapper with
        | None ->
            let elt =
              Tyxml_js.To_dom.of_element @@ D.create_widget_wrapper []
            in
