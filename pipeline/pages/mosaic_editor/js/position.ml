...
type absolute = { x : float; y : float; w : float; h : float }
...
module Make (Pos : S) : Position with type t = Pos.t = struct
  ...
  let show (x : t) =
    Printf.sprintf "left=%g, top=%g, width=%g, height=%g" (left x) (top x)
      (width x) (height x)

  let right (t : t) = Pos.left t +. Pos.width t

  let bottom (t : t) = Pos.top t +. Pos.height t
  ...
  let validate_width ?max_width ?(min_width = 0.) ?parent_width (p : t) =
    ...
    let w =
      ...
      | Some max ->
          if w > max then max else if w < min_width then min_width else w
      | None -> if w < min_width then min_width else w
    in
    let left = Pos.left p in
    let w =
      match parent_width with
      | None -> w
      | Some parent_width ->
          if left +. w > parent_width then parent_width -. left
          else if left < 0.0 then w +. left
          else w
    in
  ...
  let validate_height ?max_height ?(min_height = 0.) ?parent_height (p : t) =
    ...
    let h =
      ...
      | Some max ->
          if h > max then max else if h < min_height then min_height else h
      | None -> if h < min_height then min_height else h
    in
    let top = Pos.top p in
    let h =
      match parent_height with
      | None -> h
      | Some parent_height ->
          if top +. h > parent_height then parent_height -. top
          else if top < 0.0 then h +. top
          else h
    in
    let h = if h < min_height then min_height else h in
    Pos.set_height p h

  (** Changes width and height to correspond provided constraints *)
  let validate_size ?max_width ?min_width ?max_height ?min_height ?parent_width
      ?parent_height =
  ...
  let validate_left_top ?min_left ?min_top ?max_left ?max_top
    ...
    in
    let x =
      match max_left with Some max -> if x > max then max else x | None -> x
    in
    let x =
      match min_left with Some min -> if x < min then min else x | None -> x
    in
    let y =
      match max_top with Some max -> if y > max then max else y | None -> y
    in
    let y =
      match min_top with Some min -> if y < min then min else y | None -> y
    in
    Pos.set_top (Pos.set_left p x) y

  let validate ?min_left ?min_top ?max_left ?max_top ?max_width ?min_width
      ?max_height ?min_height ?parent_width ?parent_height =
    validate_left_top ?min_left ?min_top ?max_left ?max_top ?parent_width
      ?parent_height
    % validate_size ?max_width ?min_width ?max_height ?min_height ?parent_width
        ?parent_height
  ...
  let compare (a : t) (b : t) =
    ...
    if c <> 0 then c
      ...
      if c <> 0 then c
        ...
        if c <> 0 then c else compare a.h b.h
...
module Absolute = struct
  ...
  *)
  let line_find_closest_align (pos : t) (siblings : t list)
      (min_distance : float) (line_align : Snap_line.direction) =
    let rec count_aligns line_align_val distance = function
      | [] -> distance
      | (hd : t) :: tl ->
          let distance =
            match line_align with
            | Htop ->
                let dist1 = pos.y -. hd.y in
                let dist2 = pos.y -. hd.y -. (hd.h /. 2.0) in
                let dist3 = pos.y -. hd.y -. hd.h in
                if
                  abs_float dist1 < min_distance
  ...
  let line_align_count (pos : t) (siblings : t list) (min_distance : float)
      line_align_val =
            ...
            if
              ...
              || line_align_val = Hcenter
                 && ( abs_float (pos.y +. (pos.h /. 2.0) -. hd.y) < min_distance
                    || abs_float
                         (pos.y +. (pos.h /. 2.0) -. hd.y -. (hd.h /. 2.0))
                       < min_distance
                    || abs_float (pos.y +. (pos.h /. 2.0) -. hd.y -. hd.h)
                       < min_distance )
              || line_align_val = Hbottom
                 && ( abs_float (pos.y +. pos.h -. hd.y) < min_distance
                    || abs_float (pos.y +. pos.h -. hd.y -. (hd.h /. 2.0))
                       < min_distance
              ...
              || line_align_val = Vcenter
                 && ( abs_float (pos.x +. (pos.w /. 2.0) -. hd.x) < min_distance
                    || abs_float
                         (pos.x +. (pos.w /. 2.0) -. hd.x -. (hd.w /. 2.0))
                       < min_distance
                    || abs_float (pos.x +. (pos.w /. 2.0) -. hd.x -. hd.w)
                       < min_distance )
              || line_align_val = Vright
                 && ( abs_float (pos.x +. pos.w -. hd.x) < min_distance
                    || abs_float (pos.x +. pos.w -. hd.x -. (hd.w /. 2.0))
                       < min_distance
  ...
  let get_snap (coord : float) (min_distance : float) items =
    let rec aux (snap : float) (snap_min_delta : float) = function
      ...
      | (_, aligns_count, distance__align_other) :: tl ->
          ...
          in
          let snap =
            if abs_float snap_min_delta <= min_distance then
              coord -. snap_min_delta
            else snap
          in
          aux snap snap_min_delta tl
    in
    aux coord (min_distance +. 1.0) items

  (* FIX + 1.0; (1.0 != 1 pixel) *)

  let get_item_snap_y (pos : t) min_distance siblings =
    get_snap pos.y min_distance
    @@ hlines_for_move_action pos min_distance siblings

  let get_item_snap_x (pos : t) min_distance siblings =
    get_snap pos.x min_distance
    @@ vlines_for_move_action pos min_distance siblings
  ...
  let snap_to_siblings_resize (pos : t) min_distance siblings =
    let make_line align =
      make_line_properties align pos min_distance siblings
    in
    function
    | NW ->
  ...
  let get_snap_lines (pos : t) siblings
    ...
    let rec create_lines action acc = function
      ...
      | (direction, aligns_count, (align_other, _)) :: tl ->
          let acc =
            if aligns_count > 0 then
              let is_vertical =
                match direction with
                | Snap_line.Vleft | Vright | Vcenter -> true
                | Nill | Htop | Hbottom | Hcenter -> false
              in
              let is_center =
                match align_other with
          ...
          in
          create_lines action acc tl
    in
    create_lines action [] snap_list

  let clip_to_parent ~parent_size:(parent_width, parent_height)
      ~(min_width : float) ~(min_height : float) ({ w; h; x; y } as pos : t) =
    function
    | `Move ->
        validate_left_top ~min_left:0. ~min_top:0. ~parent_width ~parent_height
          pos
    | `Resize direction ->
        ...
        in
        validate ?min_left ?max_left ?min_top ?max_top ~min_width ~min_height
          ~parent_width ~parent_height pos
  ...
  let snap_to_grid_resize (direction : direction) (pos : t) (grid_step : float)
      : t =
    match direction with
    | NW ->
        let x = Js.math##round (pos.x /. grid_step) *. grid_step in
  ...
  let move_children (rect_position : t) (children : t list) : t list =
    let pos_left =
      List.hd
        (List.sort Stdlib.compare (List.map (fun (v : t) -> v.x) children))
    in
    let pos_top =
      List.hd
        (List.sort Stdlib.compare (List.map (fun (v : t) -> v.y) children))
    in
    List.map
      (fun (v : t) ->
  ...
  let get_float_aspect (aspect : int * int) =
    let asp =
      if fst aspect = 0 then 1.0
      else float_of_int (snd aspect) /. float_of_int (fst aspect)
    in
    if asp <= 0.0 then 1.0 else asp

  let get_min_rect_size_for_aspect ~(aspect : float) ~(min_width : float)
      ~(min_height : float) = function
    | [] -> (min_width, min_height)
    | children ->
        let bound = bounding_rect children in
        if min_width > min_height then
          let child_min_w =
            List.hd
            @@ List.sort (fun (a : t) b -> Stdlib.compare a.w b.w) children
          in
          ...
          let child_min_h =
            List.hd
            @@ List.sort (fun (a : t) b -> Stdlib.compare a.h b.h) children
          in
  ...
  let fix_aspect_min (dir : direction) (pos : t) (orig_pos : t) (asp : float)
      (children : t list) (min_width : float) (min_height : float) : t =
    let min_w, min_h =
      get_min_rect_size_for_aspect ~aspect:asp ~min_width ~min_height children
    in
    if pos.h < min_h || pos.w < min_w then
      match dir with
      | SE -> { x = orig_pos.x; y = orig_pos.y; w = min_w; h = min_h }
      | NW ->
          {
  ...
  let get_max_wh_for_aspect (dir : direction) (x, y, w, h) (* input orig_pos *)
      (max_w : float) (max_h : float) (asp : float) =
    let w1, h1, w2, h2 =
      match dir with
      | NW ->
          let w1 = x +. w in
  ...
  let fix_aspect_max (dir : direction) (pos : t) (orig_pos : t) (asp : float)
      (max_width : float) (max_height : float) : t =
    let max_w, max_h =
      get_max_wh_for_aspect dir
        (orig_pos.x, orig_pos.y, orig_pos.w, orig_pos.h)
        max_width max_height asp
    in
    if pos.h > max_h || pos.w > max_w then
      match dir with
      | NW ->
          {
            x = orig_pos.x +. orig_pos.w -. max_w;
      ...
      | SE -> { x = orig_pos.x; y = orig_pos.y; w = max_w; h = max_h }
      | SW ->
          {
            x = orig_pos.x +. orig_pos.w -. max_w;
            y = orig_pos.y;
  ...
  let fix_aspect_after_snap (dir : direction) (orig_pos : t) (before_pos : t)
      (after_pos : t) (aspect : int * int) =
    ...
    let p1 =
      ...
      | NW ->
          if
            abs_float (before_pos.w -. after_pos.w)
      ...
      | W -> { after_pos with y = orig_pos.y; h = after_pos.w *. asp }
      | E ->
          {
            after_pos with
  ...
  let fix_aspect2 (aspect_ratio : (int * int) option)
      (action : [ `Resize of direction | `Move ]) (children : t list)
      (orig_pos : t) (before_pos : t) (after_pos : t) (min_width : float)
      (min_height : float) (max_width : float) (max_height : float) =
    match aspect_ratio with
    | None -> after_pos
    | Some x -> (
        match action with
        | `Move ->
            let p1 = fix_aspect_after_snap SE orig_pos before_pos after_pos x in
            let p2 =
              fix_aspect_min SE p1 orig_pos (get_float_aspect x) children
                min_width min_height
            in
            fix_aspect_max SE p2 orig_pos (get_float_aspect x) max_width
              max_height
        | `Resize resz ->
            let p1 =
              fix_aspect_after_snap resz orig_pos before_pos after_pos x
            in
            let p2 =
              fix_aspect_min resz p1 orig_pos (get_float_aspect x) children
                min_width min_height
            in
            fix_aspect_max resz p2 orig_pos (get_float_aspect x) max_width
              max_height )

  let resize_children ({ x; y; w; h } : t) (children : t list) : t list =
    let bound = bounding_rect children in
    (* FIXME what is this? *)
  ...
  let adjust ?aspect_ratio ?(snap_lines = true) ?(min_width = 20.)
      ?(min_height = 20.) ?(min_distance = 12.) ?grid_step
      ~(action : [ `Resize of direction | `Move ]) ~(siblings : t list)
      ~(* widget positions int coordinatrs to float [0;1.0] *)
      (parent_size : float * float)
      ~(* need if input positions is int pixel coordinates *)
      (frame_position : t) (positions : t list) =
    let parent_w, parent_h = parent_size in
    let position = frame_position in
    let original_position = bounding_rect positions in
    let position =
      match (grid_step, action) with
      | None, _ -> position
      | Some step, `Move -> snap_to_grid_move position step
      | Some step, `Resize dir -> snap_to_grid_resize dir position step
    in
    let position_asp =
      fix_aspect2 aspect_ratio action positions original_position position
        position min_width min_height parent_w parent_h
    in
    let position_snap =
      match (snap_lines, action) with
      | false, _ -> position_asp
      | true, `Move -> snap_to_siblings_move position_asp min_distance siblings
      | true, `Resize resz ->
          snap_to_siblings_resize position_asp min_distance siblings resz
    in
    let position_asp =
      fix_aspect2 aspect_ratio action positions original_position position_asp
        position_snap min_width min_height parent_w parent_h
    in
    let min_width, min_height =
      get_min_rect_size ~min_width ~min_height positions
    in
    let position_clip_parent =
      match aspect_ratio with
      | None ->
          clip_to_parent ~parent_size ~min_width ~min_height position_asp action
      | Some _ ->
          clip_to_parent ~parent_size ~min_width:0.0 ~min_height:0.0
            position_asp action
      (* not calc min sizes *)
    in
    let snap_lines =
      if snap_lines then get_snap_lines position_clip_parent siblings action
      else []
    in
    let children =
      match action with
      | `Move ->
          move_children position_clip_parent
...
let absolute_to_normalized ~(parent_size : float * float) (pos : Absolute.t) :
    Normalized.t =
  let w, h = (pos.w /. fst parent_size, pos.h /. snd parent_size) in
