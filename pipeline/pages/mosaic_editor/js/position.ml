...
module Make (Pos : S) : Position with type t = Pos.t = struct
  ...
  let validate_width ?max_width ?(min_width = 0.) ?parent_width (p : t) =
    ...
    let w =
      ...
      | Some max -> if w > max then max else if w < min_width then min_width else w
  ...
  let validate_height ?max_height ?(min_height = 0.) ?parent_height (p : t) =
    ...
    let h =
      ...
      | Some max -> if h > max then max else if h < min_height then min_height else h
  ...
  let validate_left_top
    ...
    let x =
      if x < 0. then 0. else if x +. w > parent_width then parent_width -. w else x
    in
    let y =
      if y < 0. then 0. else if y +. h > parent_height then parent_height -. h else y
    in
    let x =
      match max_left with
  ...
  let validate
      ...
      ?parent_width
      ?parent_height =
    validate_left_top ?min_left ?min_top ?max_left ?max_top ?parent_width ?parent_height
    % validate_size
        ?max_width
        ?min_width
        ?max_height
...
module Abs = struct
  ...
  let make ~left ~top ~width ~height = {x = left; y = top; w = width; h = height}
end
...
module Norm = struct
  ...
  let make ~left ~top ~width ~height : t = {x = left; y = top; w = width; h = height}
end
...
module Normalized = struct
  ...
  let apply_to_element (pos : t) (elt : #Dom_html.element Js.t) =
    ...
    let (pos : t) =
      {x = pos.x *. 100.; y = pos.y *. 100.; w = pos.w *. 100.; h = pos.h *. 100.}
    in
...
module Absolute = struct
  ...
  *)
            ...
            if (line_align_val = Snap_line.Htop
               ...
               || (line_align_val = Hcenter
                  && (abs_float (pos.y +. (pos.h /. 2.0) -. hd.y) < min_distance
                     || abs_float (pos.y +. (pos.h /. 2.0) -. hd.y -. (hd.h /. 2.0))
                        < min_distance
               ...
               || (line_align_val = Vcenter
                  && (abs_float (pos.x +. (pos.w /. 2.0) -. hd.x) < min_distance
                     || abs_float (pos.x +. (pos.w /. 2.0) -. hd.x -. (hd.w /. 2.0))
                        < min_distance
      ...
      | (direction, aligns_count, (align_other, _)) :: tl ->
          let acc =
            ...
            then
              ...
              in
              let line_ret =
                ({is_vertical; is_multiple = aligns_count > 1; is_center; origin}
                  : Snap_line.t)
              in
              line_ret :: acc
            else acc
  ...
  let clip_to_parent
    ...
    | `Move ->
        validate_left_top ~min_left:0. ~min_top:0. ~parent_width ~parent_height pos
    | `Resize direction ->
        let max_left, max_top, min_left, min_top =
          match direction with
          | NW -> Some (x +. w -. min_width), Some (y +. h -. min_height), None, None
  ...
  let move_children (rect_position : t) (children : t list) : t list =
    let pos_left =
      List.hd (List.sort Stdlib.compare (List.map (fun (v : t) -> v.x) children))
    in
    let pos_top =
      List.hd (List.sort Stdlib.compare (List.map (fun (v : t) -> v.y) children))
    in
    List.map
      (fun (v : t) ->
  ...
  let get_min_rect_size_for_aspect
    ...
    | children ->
        ...
        then
          let child_min_w =
            List.hd @@ List.sort (fun (a : t) b -> Stdlib.compare a.w b.w) children
          in
          ...
          let child_min_h =
            List.hd @@ List.sort (fun (a : t) b -> Stdlib.compare a.h b.h) children
          in
  ...
  let get_min_rect_size ~(min_width : float) ~(min_height : float) = function
    ...
    | children ->
        ...
        in
        let bound = bounding_rect children in
        bound.w *. min_width /. child_min_w.w, bound.h *. min_height /. child_min_h.h
  ...
  let fix_aspect_min
    ...
    then
      ...
      | NW ->
          ...
          ; h = min_h }
      | S | W | SW ->
          {x = orig_pos.x +. orig_pos.w -. min_w; y = orig_pos.y; w = min_w; h = min_h}
      | N | E | NE ->
          {x = orig_pos.x; y = orig_pos.y +. orig_pos.h -. min_h; w = min_w; h = min_h}
    else pos
  ...
  let fix_aspect_max
    ...
    then
      ...
      | NW ->
          ...
          ; h = max_h }
      | NE ->
          {x = orig_pos.x; y = orig_pos.y +. orig_pos.h -. max_h; w = max_w; h = max_h}
      | SE -> {x = orig_pos.x; y = orig_pos.y; w = max_w; h = max_h}
      | SW ->
          {x = orig_pos.x +. orig_pos.w -. max_w; y = orig_pos.y; w = max_w; h = max_h}
      | N -> {x = orig_pos.x; y = orig_pos.y +. orig_pos.h -. max_h; w = max_w; h = max_h}
      | S -> {x = orig_pos.x +. orig_pos.w -. max_w; y = orig_pos.y; w = max_w; h = max_h}
      | W -> {x = orig_pos.x +. orig_pos.w -. max_w; y = orig_pos.y; w = max_w; h = max_h}
      | E -> {x = orig_pos.x; y = orig_pos.y +. orig_pos.h -. max_h; w = max_w; h = max_h}
    else pos
  ...
  let fix_aspect_after_snap
    ...
    let p1 =
      ...
      | E -> {after_pos with y = orig_pos.y +. orig_pos.h -. h; h = after_pos.w *. asp}
    in
    p1
  ...
  let fix_aspect2
    ...
    | Some x -> (
      ...
      | `Resize resz ->
          let p1 = fix_aspect_after_snap resz orig_pos before_pos after_pos x in
          let p2 =
            fix_aspect_min
              resz
              p1
  ...
  let adjust
    ...
    let position_clip_parent =
      match aspect_ratio with
      | None -> clip_to_parent ~parent_size ~min_width ~min_height position_asp action
      | Some _ ->
          clip_to_parent ~parent_size ~min_width:0.0 ~min_height:0.0 position_asp action
      (* not calc min sizes *)
    in
    let snap_lines =
      if snap_lines then get_snap_lines position_clip_parent siblings action else []
    in
    let children =
      match action with
      | `Move ->
