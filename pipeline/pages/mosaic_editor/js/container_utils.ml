...
let cell_position_to_wm_position ~cols ~rows {Grid.row; col; row_span; col_span} :
    Wm.position =
...
let points_to_frs (deltas : float list) : Grid.value list =
  ...
  try List.map (fun x -> Grid.Fr (x *. len)) deltas with Division_by_zero -> []
...
let get_cell_pos_part (edge : float) points =
  let rec aux cnt = function
    ...
    | _ :: tl -> aux (cnt + 1) tl
  in
  if equal_float edge 0. then first_grid_index else aux (first_grid_index + 1) points
...
let get_cell_positions ~lefts ~tops =
  List.map (fun (id, _, ({Wm.Annotated.position = x; _} as c)) ->
      ...
      id, (c, {Grid.col; row; col_span = col_end - col; row_span = row_end - row}))
