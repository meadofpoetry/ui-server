...
module Event = struct
  ...
  module Typ = struct
    let (input : resize Js.t Dom_html.Event.typ) = Dom_html.Event.make (name ^ ":input")

    let (change : resize Js.t Dom_html.Event.typ) = Dom_html.Event.make (name ^ ":change")
  end
...
module Util = struct
      ...
      (fun () ->
        ...
        let rec aux acc i =
          ...
          then acc
            ...
            match touch_id with
            | None -> Some touch
            | Some id -> if touch##.identifier = id then Some touch else aux acc (succ i)
        in
  ...
  let cell_of_event (items : Dom_html.element Js.t list) (e : #Dom_html.event Js.t) :
  ...
  let get_cell_position (cell : Dom_html.element Js.t) =
    ...
    {col; row; col_span = parse_span col col_end; row_span = parse_span row row_end}
  ...
  let set_cell_position {col; row; col_span; row_span} (cell : Dom_html.element Js.t) =
  ...
  let find_first_cell dir n cells =
    ...
    @@ List.fold_left
         (fun acc cell ->
           ...
           | Some (aux', _) -> if n = main && aux < aux' then Some (aux, cell) else acc)
         None
         cells
  ...
  let get_matched_css_rules (elt : #Dom_html.element Js.t) : #stylish Js.t list =
    ...
    in
    List.filter (fun x ->
        try Element.matches elt (Js.to_string x##.selectorText) with _ -> false)
    @@ List.concat
    ...
    | cell :: _ as cells ->
        List.fold_left
          (fun (acc : cell_position) v ->
            ...
            let x2 =
              if ci.col + ci.col_span > acc.col + acc.col_span
              then ci.col + ci.col_span
              else acc.col + acc.col_span
            in
            let y2 =
              if ci.row + ci.row_span > acc.row + acc.row_span
  ...
  let is_coord_in_cells cells (x, y) =
    List.exists
      (fun v ->
        ...
        x >= c.col && x < c.col + c.col_span && y >= c.row && y < c.row + c.row_span)
      cells
  ...
  let get_visual_table_size (cells : Dom_html.element Js.t list) : int * int =
    let x, y =
      List.fold_left
        (fun acc v ->
          let c = get_cell_position v in
          let x = if c.col + c.col_span > fst acc then c.col + c.col_span else fst acc in
          let y = if c.row + c.row_span > snd acc then c.row + c.row_span else snd acc in
          x, y)
        (0, 0)
        cells
    in
    ...
    let list_before_uniq =
      List.sort_uniq
        (fun a b ->
          ...
          then 0
          else if pa.col + (pa.row * len_x) > pb.col + (pb.row * len_x)
          then 1
          else -1)
        list_before
    in
...
class t
  ...
  object (self)
          ...
          let col, col_end, row, row_end =
            List.fold_left
              (fun (cs, ce, rs, re) cell ->
                ...
                min col cs, max (col + col_span) ce, min row rs, max (row + row_span) re)
    ...
    method cells' ?include_subgrids ?(grid = super#root) () : Dom_html.element Js.t list
        =
    ...
    method private cells_ ?(include_subgrids = true) grid : Dom_html.element Js.t list =
      ...
      | false ->
          List.filter (fun x -> Element.has_class x CSS.cell) @@ Element.children grid
    ...
    method private add_row_or_column
      ...
      let _, after_list, line_list =
        Util.get_before_after_line_lists
          cell
          direction
          before
          (self#cells' ~include_subgrids:false ~grid ())
      ...
      List.iter
        (fun v ->
          ...
          let (elt : Dom_html.element Js.t) =
            Tyxml_js.To_dom.of_element @@ D.create_cell (make_cell_position ~col ~row ())
          in
    ...
    method private handle_drag_start (e : Dom_html.event Js.t) _ : unit Lwt.t =
      ...
      let direction =
        Element.(
          if has_class target CSS.col_handle
          then Some `Col
          else if has_class target CSS.row_handle
          then Some `Row
          else if has_class target CSS.mul_handle
          then Some `Mul
          else None)
      in
    ...
    method private get_dimensions ~col ~row grid direction : dimensions =
      ...
      in
      let a_track_start =
        start +. Util.get_size_at_track ~gap (Array.sub track_values_px 0 a_track)
      in
      let b_track_end =
        start +. Util.get_size_at_track ~gap (Array.sub track_values_px 0 (succ b_track))
      in
      { a_track
      ; b_track
      ; a_track_start
    ...
    method private update_position grid position (dimensions : dimensions) =
      ...
      in
      let min_position = dimensions.a_track_start +. min_size_start +. dimensions.gap in
      let max_position = dimensions.b_track_end -. min_size_end -. dimensions.gap in
      ...
      let position =
        ...
        |> min max_position
      in
      let a_track_size = position -. dimensions.a_track_start -. dimensions.gap in
...
let make ?drag_interval ?snap_offset ?min_size_start ?min_size_end ?on_cell_insert () =
  let elt = Dom_html.(createDiv document) in
  new t ?drag_interval ?snap_offset ?min_size_start ?min_size_end ?on_cell_insert elt ()
