...
module Event = struct
  ...
  class type resize = [detail] Dom_html.customEvent
...
module Util = struct
  ...
  let get_cursor_position ?touch_id (event : Dom_html.event Js.t) =
    Js.Opt.case
      ...
      (fun () ->
        ...
        let rec aux acc i =
          if i >= touches##.length then acc
            ...
            let touch =
              Js.Optdef.get (touches##item i) (fun () -> assert false)
            in
            match touch_id with
            | None -> Some touch
            | Some id ->
      ...
      (fun event ->
        match Js.Optdef.(to_option event##.pageX, to_option event##.pageY) with
        | Some page_x, Some page_y -> (page_x, page_y)
        | _ -> failwith "no page coordinates in mouse event")

  let cell_of_event (items : Dom_html.element Js.t list)
    ...
    | None -> None
    | Some parent ->
        if not @@ Element.matches parent ("." ^ CSS.cell) then None
  ...
  let get_parent_grid (cell : Dom_html.element Js.t) =
    let rec aux elt =
      Js.Opt.case (Element.get_parent elt)
        ...
        (fun elt -> if Element.has_class elt CSS.root then elt else aux elt)
    in
    aux cell
  ...
  let get_cell_position (cell : Dom_html.element Js.t) =
    let parse_span n s =
      ...
      | [ s ] -> int_of_string s - n
      | _ -> failwith "unknown cell span value"
    in
  ...
  let find_first_cell dir n cells =
    ...
    @@ List.fold_left
         (fun acc cell ->
           ...
           | Some (aux', _) ->
               if n = main && aux < aux' then Some (aux, cell) else acc)
         None cells
  ...
  let first_non_zero f a =
    let rec aux = function
      ...
      | n -> (
          match f a.(n) with
          | None -> aux (pred n)
          | Some v -> if v <> 0. then Some (n, v) else aux (pred n) )
    in
    aux (pred @@ Array.length a)

  let fr_to_pixels track_values computed_values =
    match
      first_non_zero (function Fr x -> Some x | _ -> None) track_values
    with
    | None -> 0.
  ...
  let percentage_to_pixels track_values computed_values =
    match
      first_non_zero (function Pc x -> Some x | _ -> None) track_values
    with
    | None -> 0.
    | Some (track, v) -> computed_values.(track) /. v
  ...
  let get_matched_css_rules (elt : #Dom_html.element Js.t) : #stylish Js.t list
    ...
    List.filter (fun x ->
        ...
        with _ -> false)
    @@ List.concat
    @@ List.map (fun x -> try make_list x##.cssRules with _ -> [])
  ...
  let get_styles (rule : string) (elt : Dom_html.element Js.t) =
    ...
    List.filter_map
      (fun (x : Js.js_string Js.t Js.optdef) ->
        Js.Optdef.case x
          (fun () -> None)
          (fun x -> match Js.to_string x with "" -> None | s -> Some s))
      styles
    ...
    let rec gen_rows acc row =
      let rec gen_cols acc col =
        if col = 0 then acc
          ...
          gen_cols (elt :: acc) (pred col)
      in
      if row = 0 then acc else gen_rows (gen_cols acc cols) (pred row)
    in
    gen_rows [] rows
    ...
    | cell :: _ as cells ->
        List.fold_left
          (fun (acc : cell_position) v ->
            ...
            let x2 =
              if ci.col + ci.col_span > acc.col + acc.col_span then
                ci.col + ci.col_span
              else acc.col + acc.col_span
            in
            let y2 =
              if ci.row + ci.row_span > acc.row + acc.row_span then
                ci.row + ci.row_span
              else acc.row + acc.row_span
            in
            { col = x1; row = y1; col_span = x2 - x1; row_span = y2 - y1 })
  ...
  let generate_check_coords (rect : cell_position) : (int * int) list =
    let rec generate_coords_list acc x y x_beg x_end y_beg y_end =
      if x >= x_beg && y >= y_beg then
        let acc = (x, y) :: acc in
    ...
    generate_coords_list []
      ...
      rect.col (rect.col + rect.col_span) rect.row (rect.row + rect.row_span)
  ...
  let get_visual_table_size (cells : Dom_html.element Js.t list) : int * int =
    let x, y =
      List.fold_left
        (fun acc v ->
          let c = get_cell_position v in
          let x =
            if c.col + c.col_span > fst acc then c.col + c.col_span else fst acc
          in
          let y =
            if c.row + c.row_span > snd acc then c.row + c.row_span else snd acc
          in
          (x, y))
        (0, 0) cells
    in
  ...
  let perpendicular_direction = function Col -> Row | Row -> Col

  let get_cell_at_coord ~col ~row cells =
    List.find_opt (fun v -> is_coord_in_cells [ v ] (col, row)) cells

  let is_cells_same_at_coords (x1 : int) (y1 : int) (x2 : int) (y2 : int)
  ...
  let get_before_after_line_lists (selected_cell : Dom_html.element Js.t)
      (what_add : direction) (before : bool)
        ...
        (fun acc v ->
          let x =
            generate_check_coords
              ( match what_add with
              | Row ->
                  { col = fst v; row = 1; col_span = 1; row_span = len_y + 1 }
              | Col ->
                  { col = 1; row = snd v; col_span = len_x + 1; row_span = 1 }
              )
          in
          x :: acc)
        [] (List.rev cross_line)
            ...
            List.sort
              (fun a b ->
                let c =
                  ...
                  | Col -> if before then sel.col else sel.col + sel.col_span
                in
                match what_add with
              ...
              steps
          in
          match steps_sorted with [] -> acc | hd :: _ -> hd :: acc)
        ...
        (fun acc (cr, h_l) ->
          let one_col_before, one_col_after =
            List.fold_left
              ...
              ([], []) cr
          in
          (one_col_before @ fst acc, one_col_after @ snd acc))
        ([], []) check_lines_with_insert_line
    in
    let list_before_uniq =
      List.sort_uniq
        (fun a b ->
...
class t ?drag_interval ?(snap_offset = 0.) ?(min_size_start = 20.)
  ?(min_size_end = 20.) ?(on_cell_insert = fun _ _ -> ())
  ...
  object (self)
    ...
    method rows : value array =
      Array.map value_of_string @@ self#raw_tracks super#root Row

    method cols : value array =
      Array.map value_of_string @@ self#raw_tracks super#root Col
    ...
    method reset ?cells ~(cols : property) ~(rows : property) () =
      ...
      let cells =
        ...
        | None ->
            ...
            Util.gen_cells
              ...
              ~cols:(property_to_num cols) ~rows:(property_to_num rows)
      in
      List.iter
        (fun cell ->
    ...
    method insert_table ?(col_size = Fr 1.) ?(row_size = Fr 1.) ~(cols : int)
        ~(rows : int) (cell : Dom_html.element Js.t) : unit =
    ...
    method merge (cells : Dom_html.element Js.t list)
      ...
      | x :: tl ->
          ...
          let col, col_end, row, row_end =
            List.fold_left
              (fun (cs, ce, rs, re) cell ->
                let { col; row; col_span; row_span } =
                  Util.get_cell_position cell
                in
                ( min col cs,
                  max (col + col_span) ce,
                  min row rs,
                  max (row + row_span) re ))
              (col, col + col_span, row, row + row_span)
              tl
          in
          let position =
          ...
          let (merged : Dom_html.element Js.t) =
            Tyxml_js.To_dom.of_element @@ D.create_cell position
          in
    ...
    method cells' ?include_subgrids ?(grid = super#root) ()
        : Dom_html.element Js.t list =
      self#cells_ ?include_subgrids grid

    method cells : Dom_html.element Js.t list = self#cells_ super#root
    ...
    method private remove_row_or_column (direction : direction)
      ...
      List.iter (fun cell ->
          ...
          else if n' = n then Element.remove_child_safe grid cell)
      @@ self#cells' ~include_subgrids:false ~grid ();
      (* Update style *)
      let style = String.concat " " @@ Util.remove_at_idx (n - 1) tracks in
      self#set_style grid direction style;
      if self#empty then self#clear_styles_ ()

    method private add_row_or_column ?(size = Fr 1.) ?(before = false)
        (direction : direction) (cell : Dom_html.element Js.t) : unit =
      let grid = Util.get_parent_grid cell in
      let tracks = Array.to_list @@ self#raw_tracks grid direction in
      ...
      List.iter
        (fun v ->
          ...
          let (elt : Dom_html.element Js.t) =
            Tyxml_js.To_dom.of_element
            @@ D.create_cell (make_cell_position ~col ~row ())
          in
          on_cell_insert self elt;
          Element.append_child grid elt)
        line_list;
      let style =
        String.concat " " @@ Util.insert_at_idx 1 (value_to_string size) tracks
      in
      self#set_style grid direction style
          ...
          let first_cells, row, col =
            ...
            | `Mul ->
                ...
                ( [ col_cell; row_cell ],
                  ...
                  Some (self#get_dimensions ~col ~row grid Row) )
          in
          let resize_properties = { grid; cell; row; col; first_cells } in
              ...
              _move_listeners <-
                Lwt_js_events.
                  [
                    ...
                    touchends wnd
                      (self#handle_drag_stop resize_properties % coerce);
                    touchcancels wnd
                      (self#handle_drag_stop resize_properties % coerce);
                  ])
            (fun _ ->
    ...
    method private get_dimensions ~col ~row grid direction : dimensions =
      let start =
        ...
        | Col -> grid##getBoundingClientRect##.left
      in
      ...
      let a_track_start =
        start
        +. Util.get_size_at_track ~gap (Array.sub track_values_px 0 a_track)
      in
      let b_track_end =
        start
        +. Util.get_size_at_track ~gap
             (Array.sub track_values_px 0 (succ b_track))
      in
      {
        a_track;
        b_track;
        ...
        total_fr =
          Array.fold_left
            (fun acc -> function Fr _ -> succ acc | _ -> acc)
            0 track_values;
        percentage_to_pixels =
          Util.percentage_to_pixels track_values track_values_px;
    ...
    method private update_position grid position (dimensions : dimensions) =
      ...
      in
      let min_position =
        dimensions.a_track_start +. min_size_start +. dimensions.gap
      in
      let max_position =
        dimensions.b_track_end -. min_size_end -. dimensions.gap
      in
      ...
      in
      let a_track_size =
        position -. dimensions.a_track_start -. dimensions.gap
      in
      let b_track_size = dimensions.b_track_end -. position -. dimensions.gap in
      let drag_interval =
        match drag_interval with
        | None -> 1.
        | Some (Fr x) -> x *. dimensions.fr_to_pixels
        | Some (Px x) -> x
        | Some (Pc x) -> x *. dimensions.percentage_to_pixels
        | Some Auto -> fail "`auto` interval is not supported"
      in
      let a_track_size, b_track_size =
        if drag_interval > 1. then
          let a_track_size_interleaved =
            Js.math##round (a_track_size /. drag_interval) *. drag_interval
          in
          ( a_track_size_interleaved,
            b_track_size -. (a_track_size_interleaved -. a_track_size) )
        else (a_track_size, b_track_size)
      in
    ...
    method private adjust_position ~a_track_size ~b_track_size
        ({
           a_track;
           b_track;
           tracks;
    ...
    method private track_values_px grid direction : float array =
      ...
      let v =
        ...
        | Col -> (Js.Unsafe.coerce style)##.gridTemplateColumns
      in
      Js.Optdef.case v
        (fun () -> [||])
        ( Array.of_list
    ...
    method private gap grid direction : float =
      ...
      let v =
        ...
        | Col -> (Js.Unsafe.coerce style)##.gridColumnGap
      in
      Js.Optdef.case v
        (fun () -> 0.)
        (fun x ->
          let s = Js.to_string x in
          match value_of_string_opt s with Some (Px v) -> v | _ -> 0.)
  end

let make ?drag_interval ?snap_offset ?min_size_start ?min_size_end
    ?on_cell_insert () =
  let elt = Dom_html.(createDiv document) in
  new t
    ?drag_interval ?snap_offset ?min_size_start ?min_size_end ?on_cell_insert
    elt ()

let attach ?drag_interval ?snap_offset ?min_size_start ?min_size_end
    ?on_cell_insert (elt : Dom_html.element Js.t) : t =
  new t
    ?drag_interval ?snap_offset ?min_size_start ?min_size_end ?on_cell_insert
    elt ()
