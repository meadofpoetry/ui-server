  ...
  object (self)
    ...
    method private transform_top_app_bar items =
      let title =
        ...
        | x -> Printf.sprintf "Выбрано виджетов: %d" @@ List.length x
      in
      let restore =
    ...
    method private handle_transform_action e _ =
      ...
      let aspect_ratio =
        ...
        | true, None ->
            ...
            in
            let aspect =
              Some (resolution_to_aspect (int_of_float width, int_of_float height))
            in
    ...
    method private move_ghost
      ...
      fun ?aspect event ->
        ...
        let (position : Position.Absolute.t) =
          {x = fst point; y = snd point; w = 100. (* FIXME *); h = 100. (* FIXME *)}
        in
