...
class t (scaffold : Scaffold.t) (elt : Dom_html.element Js.t) =
  object (self)
    ...
    method remove_by_id (id : string) =
      let selector = Printf.sprintf ".%s[%s=\"%s\"]" CSS.item Widget_utils.Attr.id id in
    ...
    method private handle_change () : unit =
      ...
      match self#items with
      | [] -> if placeholder_hidden then placeholder#remove_class CSS.placeholder_hidden
      | _ -> if not placeholder_hidden then placeholder#add_class CSS.placeholder_hidden
