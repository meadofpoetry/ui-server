...
let () =
  ...
  let thread =
    ...
    >>=? fun (_v_id, v_ev) -> Http_measurements.Event.get_audio socket
    >>=? fun (_a_id, a_ev) -> Http_structure.get_annotated ()
    >>=? fun structures ->
        ...
        [ E.map (fun data ->
              let data = List.map
                  (fun ({ stream
                        ...
                        ; pid
                        ; data
                        ; _ } : Qoe_errors.Video_data.t) ->
                    let (source : data_source) =
                      { stream
                      ; service = channel
                      ; pid
                      } in
                    source, `Video data) data in
              `Data data) v_ev
        ; E.map (fun data ->
              let data = List.map
                  (fun ({ stream
                        ...
                        ; pid
                        ; data
                        ; _ } : Qoe_errors.Audio_data.t) ->
                    let (source : data_source) =
                      { stream
                      ; service = channel
                      ; pid
                      } in
                    source, `Audio data) data in
              `Data data) a_ev
        ] in
    List.iter (fun chart ->
