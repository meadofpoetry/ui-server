...
struct
  let create_hotkeys ?classes ?a () : 'a Html.elt =
    ...
    let create_section (title, (gp : hotkeys_group)) =
      ...
      M.create_section ~title ~options ()
    in
    M.create ?classes ?a ~sections:(List.map create_section hotkeys) ()
end
