...
module RTC = struct
  ...
  module MP = struct
    ...
    let track_to_create_req (track : track) : Mp_create.t =
      ...
      in
      {type_ = Rtp rtp; admin_key = None; secret = None; pin = None; permanent = true}

    let create (plugin : Plugin.t) (req : Mp_create.t) :
        (Mp_create.r, string) Lwt_result.t =
      Plugin.send_message ~message:(Js.Unsafe.obj @@ request_to_obj (Create req)) plugin
      >>= function
      | Ok None -> Lwt.return_error "empty response"
      | Ok (Some d) -> Lwt.return @@ Mp_create.of_js_obj d
      | Error _ as e -> Lwt.return e

    let watch ?secret (plugin : Plugin.t) (id : int) : (unit, string) Lwt_result.t =
      let req = Mp_watch.{id; secret} in
      Plugin.send_message ~message:(Js.Unsafe.obj @@ request_to_obj (Watch req)) plugin
      >>= function
      | Ok _ -> Lwt.return_ok ()
      | Error _ as e -> Lwt.return e

    let start ?jsep (plugin : Plugin.t) =
      Plugin.send_message ?jsep ~message:(Js.Unsafe.obj @@ request_to_obj Start) plugin
      >>= function
      | Ok _ -> Lwt.return_ok ()
      | Error _ as e -> Lwt.return e
  end

  let handle_jsep (jsep : Webrtc._RTCSessionDescriptionInit Js.t) (plugin : Plugin.t) =
    ...
    Lwt.Infix.(
      ...
      | Ok session -> (
          ...
          | Ok (plugin : Plugin.t) ->
              ...
              |> Lwt.join
              >>= fun () ->
              MP.watch plugin main.id
              >>= fun _ -> Lwt.return_ok {session; video = plugin; audio = plugin}
          | Error e -> Lwt_result.fail e))
end
...
let tie_side_sheet_with_toggle (scaffold : Scaffold.t) =
  ...
  | Some i, Some side_sheet ->
      Lwt.async (fun () ->
          Js_of_ocaml_lwt.Lwt_js_events.clicks i (fun _ _ -> side_sheet#toggle ()))
  | _ -> ()
...
let make_wizard (scaffold : Scaffold.t) =
  ...
  let show () =
    ...
    | Accept -> (
        ...
        | Ok wizard -> submit_wizard scaffold wizard#value >>= fun _ -> Lwt.return_unit)
  in
  let listeners =
    Js_of_ocaml_lwt.Lwt_js_events.
