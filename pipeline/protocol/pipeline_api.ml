  ...
  | inputs ->
      List.filter
        (fun (_, stream) ->
          ...
          | None -> false
          | Some input -> List.exists (Topology.equal_topo_input input) inputs)
        streams
...
let filter_data ids inputs streams data =
  ...
  | _ ->
      let uris =
        streams |> filter_by_stream_id ids |> filter_by_input inputs |> uris
      in
      filter_by_uris uris data
...
module Event = struct
  ...
  let filter_event ids inputs streams event =
    ...
    | _ ->
        let uris =
          streams |> filter_by_stream_id ids |> filter_by_input inputs |> uris
        in
        E.map (filter_by_uris uris) event
  ...
  let get_annotated (state : Protocol.state) _user =
    let signal =
      React.S.l3
        ...
        state.notifs.applied_structs state.notifs.streams
        state.options.structures#s
    in
    let event = S.changes signal |> E.map Structure.Annotated.to_yojson in
    Lwt.return event
  ...
  let get_settings (state : state) _user =
    let event =
      S.changes state.options.settings#s |> E.map Settings.to_yojson
    in
    Lwt.return event
  ...
  let get_wm_layout (state : Protocol.state) _user =
    let signal =
      React.S.l2
        (fun active stored -> Wm.Annotated.annotate ~active ~stored)
        state.notifs.wm state.options.wm#s
    in
    let event = S.changes signal |> E.map Wm.Annotated.to_yojson in
    Lwt.return event
  ...
  let get_status (state : Protocol.state) ids _user =
    ...
    let event =
      ...
      | ids ->
          React.E.map
            (List.filter (fun (x : Qoe_status.t) ->
...
let get_streams (state : Protocol.state) ids inputs _user _body _env _state =
  ...
  let streams =
    state.sources |> filter_by_stream_id ids |> filter_by_input inputs
  in
  let json =
    Util_json.List.to_yojson
...
let get_annotated (state : Protocol.state) _user _body _env _state =
  ...
  | Some backend -> (
      Protocol.Qoe_backend.Stream_parser.get_structure backend >>= fun avail ->
      Protocol.Qoe_backend.Graph.get_structure backend >>= fun active ->
      state.options.structures#get >>= fun stored ->
...
let get_applied_structures (state : Protocol.state) ids inputs _user _body _env
    _state =
  match state.backend with
  | None -> Lwt.return (`Error "not ready")
  | Some backend -> (
      ...
      |> Lwt_result.map
           (Structure.Many.to_yojson % filter_data ids inputs state.sources)
      >>= function
...
let apply_structures (state : Protocol.state) _user body _env _state =
  ...
  | Error e -> Lwt.return (`Error e)
  | Ok x -> (
      Protocol.reset state state.sources (* TODO remove reset *) >>= function
      | Error (`Qoe_backend e) -> Lwt.return (`Error e)
      | Ok () -> (
          match state.backend with
          | None -> Lwt.return (`Error "not ready")
          | Some backend -> (
              Protocol.Qoe_backend.Graph.apply_structure backend x >>= function
              | Error (`Qoe_backend e) -> Lwt.return (`Error e)
              | Ok () ->
                  state.options.structures#set x >>= fun () -> Lwt.return `Unit
              ) ) )
  ...
  set body Settings.of_yojson
    Protocol.(
      fun x ->
        ...
        | Some backend ->
            Qoe_backend.Analysis_settings.apply_settings backend x >>= fun () ->
...
let get_settings (state : Protocol.state) _user _body _env _state =
  ...
  | None -> Lwt.return (`Error "not ready")
  | Some backend -> (
      Protocol.Qoe_backend.Analysis_settings.get_settings backend >>= function
      | Error (`Qoe_backend e) -> Lwt.return (`Error e)
...
let filter_wm (wm : Wm.t) : Wm.t =
  let layout =
    List.map
      (fun (id, (c : Wm.container)) ->
        let widgets =
          List.filter
            (fun (id, _) ->
              List.exists (fun (id', _) -> String.equal id' id) wm.widgets)
            c.widgets
        in
        (id, { c with widgets }))
      wm.layout
  ...
  set body Wm.of_yojson
    Protocol.(
      fun x ->
        ...
        | None -> Lwt.return_error (`Qoe_backend "not ready")
        | Some backend ->
            Protocol.Qoe_backend.Mosaic.get_layout backend >>= fun active ->
            let x = { x with widgets = active.widgets } in
            let filtered = filter_wm x in
            Qoe_backend.Mosaic.apply_layout backend filtered >>= fun () ->
...
let get_wm_layout (state : Protocol.state) _user _body _env _state =
  ...
  | Some backend -> (
      Protocol.Qoe_backend.Mosaic.get_layout backend >>= fun active ->
      state.options.wm#get >>= fun stored ->
      match active with
      | Error (`Qoe_backend e) -> Lwt.return (`Error e)
      | Ok active ->
