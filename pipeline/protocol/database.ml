...
module Streams = struct
  ...
  let init db streams =
    ...
    let insert_new = R.exec Caqti_type.(tup3 string ptime ptime)
                       (sprintf "INSERT INTO %s (streams,date_start,date_end) VALUES (?,?,?)" table) in
    let now = Ptime_clock.now () in
    let streams = Yojson.Safe.to_string @@ (Util_json.List.to_yojson Stream.to_yojson streams) in
    Conn.request db Db.Request.(exec insert_new (streams,now,now))

  let bump db =
    let open Printf in
    let table = (Conn.names db).streams in
    let update_last = R.exec Caqti_type.ptime
  ...
  let select_streams db ?(limit = 500) ~from ~till =
    let open Printf in
    let table = (Conn.names db).streams in
    let select = R.collect Caqti_type.(tup3 ptime ptime int) Caqti_type.(tup3 string ptime ptime)
                   (sprintf {|SELECT * FROM %s WHERE date_end >= ? AND date_start <= ? 
                             ORDER BY date_end LIMIT ?|} table) in
    Conn.request db Db.Request.(list select (from,till,limit) >>= fun l ->
...
module Pid_state = struct
  ...
  let init db pids =
    ...
    let insert_new = R.exec Caqti_type.(tup4 SID.typ int int (tup2 ptime ptime))
    ...
    Conn.request db Db.Request.(with_trans (List.fold_left (fun acc (s,c,p,_) ->
  ...
  let bump db pids =
     let open Printf in
     let table = (Conn.names db).pid_state in
     let update_last = R.exec Caqti_type.(tup4 SID.typ int int ptime)
  ...
  let req_select_span db ~stream ~channel ~pid ~from ~till =
    ...
    in Db.Request.(find r (stream,channel,pid,(from,till)) >>= function
                | Some span ->
...
module Structure = struct
  ...
  let select_structures db ?(uris=[]) ?(limit=500) ~from ~till =
    ...
    let select = 
      R.collect Caqti_type.(tup3 ptime ptime int) Caqti_type.(tup2 string ptime)
    ...
    in Conn.request db Db.Request.(list select (from,till,limit) >>= fun l ->
...
module Stream_status = struct
  ...
  let init db pids =
    ...
    let now = Ptime_clock.now () in
    Logs.err (fun m -> m "DB pids len: %d" @@ List.length pids);
    Conn.request db Db.Request.(with_trans (List.fold_left (fun acc {stream;channel;pid;_} ->
                                             acc >>= fun () ->
