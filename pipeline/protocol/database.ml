...
module SID = struct
  ...
  let of_stream_id (id : Stream.ID.t) = Stream.ID.to_string id

  let to_stream_id (t : string) = Stream.ID.of_string t
...
module Model = struct
  ...
  let streams_keys =
    Db.make_keys ~time_key:"date_end"
      [
        ("streams", key "JSONB");
  ...
  let init db streams =
    ...
    let insert_new =
      R.exec
        ...
        (sprintf "INSERT INTO %s (streams,date_start,date_end) VALUES (?,?,?)"
           table)
    in
    let now = Ptime_clock.now () in
    let streams =
      Yojson.Safe.to_string @@ Util_json.List.to_yojson Stream.to_yojson streams
    in
    Conn.request db Db.Request.(exec insert_new (streams, now, now))

  let bump db =
    let open Printf in
    let table = (Conn.names db).streams in
    let update_last =
      R.exec Caqti_type.ptime
        (sprintf
           {|UPDATE %s SET date_end = ? WHERE 
                                  date_start = (SELECT date_start FROM %s ORDER BY date_start DESC LIMIT 1)|}
           table table)
      (* TODO optimize out*)
    in
    let now = Ptime_clock.now () in
    Conn.request db Db.Request.(exec update_last now)
  ...
  let select_streams db ?(limit = 500) ~from ~till =
    ...
    let select =
      R.collect
        ...
        Caqti_type.(tup3 string ptime ptime)
        (sprintf
           {|SELECT * FROM %s WHERE date_end >= ? AND date_start <= ? 
                             ORDER BY date_end LIMIT ?|}
           table)
    in
    Conn.request db
      Db.Request.(
        ...
        try
          ...
          return
            (Ok
               (Raw
                  { data; has_more = limit <= List.length data; order = `Desc }))
...
module Pid_state = struct
  let init db pids =
    ...
    let insert_new =
      R.exec
        Caqti_type.(tup4 SID.typ int int (tup2 ptime ptime))
        (sprintf
           "INSERT INTO %s (stream,channel,pid,date_start,date_end) VALUES \
            (?,?,?,?,?)"
           table)
    in
    let now = Ptime_clock.now () in
  ...
  let bump db pids =
    ...
    let update_last =
      R.exec
        Caqti_type.(tup4 SID.typ int int ptime)
        (sprintf
           {|UPDATE %s SET date_end = $4
  ...
  let req_select_distinct_pids db ~from ~till =
    ...
    let r =
      R.collect
        Caqti_type.(tup2 ptime ptime)
        Caqti_type.(tup3 SID.typ int int)
        (sprintf
           {|SELECT DISTINCT stream,channel,pid FROM %s 
                        WHERE date_start <= $2 AND date_end >= $1|}
           table)
    in
    Db.Request.(list r (from, till))
  ...
  let req_select_intervals db ?(limit = 500) ~stream ~channel ~pid ~from ~till =
    ...
    let r =
      R.collect
        Caqti_type.(tup4 SID.typ int int (tup3 ptime ptime int))
        Caqti_type.(tup2 ptime ptime)
        (sprintf
           {|SELECT date_start,date_end FROM %s 
                        WHERE stream = $1 AND channel = $2 AND pid = $3
                        AND date_end >= $4 AND date_start <= $5 ORDER BY date_end DESC LIMIT $6|}
           table)
    in
    Db.Request.(list r (stream, channel, pid, (from, till, limit)))
  ...
  let req_select_span db ~stream ~channel ~pid ~from ~till =
    ...
    let r =
      R.find
        ...
        (sprintf
           {|SELECT SUM(date_end - date_start) FROM
                        ((SELECT date_start,date_end FROM %s 
                         ...
                         (SELECT $4 as date_start, $5 as date_end FROM %s
                          ...
                          ORDER BY date_end DESC LIMIT 1)) as temp|}
           table table table table)
    in
    Db.Request.(
      find r (stream, channel, pid, (from, till)) >>= function
      | Some span ->
...
module Structure = struct
  ...
  let insert_structures db _streams : unit Lwt.t =
    ...
    let entries = [] in
    (*Structure_conv.dump_structures streams in*)
    let insert =
      R.exec
    ...
    Conn.request db
      Db.Request.(
        with_trans
          (List.fold_left
             (fun acc e -> acc >>= fun () -> exec insert e)
             (return ()) entries))
  ...
  let select_structures db ?(uris = []) ?(limit = 500) ~from ~till =
    ...
    let uris =
      is_in "id"
        (fun x ->
          Yojson.Safe.to_string @@ Stream.container_id_to_yojson (TSoIP x))
        uris
    in
    let select =
      R.collect
        ...
        (sprintf
           {|(SELECT structs, date FROM %s WHERE %s date > $1 AND date <= $2)
                  UNION
                  (SELECT structs, date FROM %s WHERE %s date < $1 ORDER BY date DESC LIMIT 1)
                  ORDER BY date DESC LIMIT $3|}
           table uris table uris)
    in
    Conn.request db
      Db.Request.(
        list select (from, till, limit) >>= fun l ->
        try
          ...
          return
            (Ok
               (Raw
                  { data; has_more = List.length data >= limit; order = `Desc }))
        with Failure e -> return (Error e))
end
...
module Errors = struct
  (* TODO
       ...
       let insert =
         R.exec data_t
           (sprintf {|INSERT INTO %s(stream,channel,pid,error,counter,size,min,max,avg,peak_flag,cont_flag,date)
                     VALUES (?,?,?,?,?,?,?,?,?,?,?,?)|} table)
       in Conn.request db Db.Request.(with_trans (List.fold_left (fun acc x -> acc >>= fun () -> exec insert x)
                                                 (return ()) data))
     ...
     let select_has_any db ?(coords = []) ?(errors = []) ?(typ = `Both) ~from ~till =
       ...
       in
       let select = R.find Caqti_type.(tup2 ptime ptime) Caqti_type.bool
                      (sprintf {|SELECT EXISTS (SELECT 1 FROM %s
                                WHERE %s %s %s date >= ? AND date <= ?)|}
                         table typ errors coords)
       in Conn.request db Db.Request.(find select (from,till) >>= function
                                   | None   -> return false
                                   | Some x -> return x)
     ...
     let select_percent db ?(coords = []) ?(errors = []) ?(typ = `Both) ~from ~till =
       ...
       let typ      = match typ with
         ...
         | `Cont -> "cont_flag = TRUE AND"
       in
       let select = R.find Caqti_type.(tup4 SID.typ int int (tup2 ptime ptime)) Caqti_type.int
                      (sprintf {|SELECT count(DISTINCT date_trunc('second',date)) FROM %s
                         ...
                         table typ errors)
       in Conn.request db Db.Request.(begin match coords with
     ...
     let select_errors db ?(limit = 500) ?(coords = []) ?(errors = []) ?(typ = `Both) ~from ~till =
       ...
       let typ      = match typ with
         ...
         | `Cont -> "cont_flag = TRUE AND"
       in
       let select = R.collect Caqti_type.(tup3 ptime ptime int) data_t
                      (sprintf {|SELECT stream,channel,pid,error,counter,size,min,max,avg,peak_flag,cont_flag,date
                         ...
                         table typ errors coords)
       in Conn.request db Db.Request.(list select (from,till,limit) >>= fun data ->
     ...
     let select_states db ?(limit = 500) ~error ~stream ~channel ~pid ~from ~till =
       let open Printf in
       let table    = (Conn.names db).errors in
       let select = R.collect Caqti_type.(let (&) =tup2 in SID.typ & int & int & int & ptime & ptime & int) Caqti_type.(tup4 float float float ptime)
                      (sprintf {|SELECT min,max,avg,date FROM %s
                         ...
                         table)
       in Conn.request db Db.Request.(list select (stream,(channel,(pid,(error,(from,(till,limit)))))) >>= fun data ->
                                   return @@ Raw { data; has_more = List.length data >= limit; order = `Desc })
     ...
     let select_states_compress db ~error ~stream ~channel ~pid ~from ~till =
       ...
       let table    = (Conn.names db).errors in
       let intvals  = Time.split ~from ~till in
       let select   = R.find Caqti_type.(let (&) =tup2 in SID.typ & int & int & int & ptime & ptime) Caqti_type.(tup3 float float float)
