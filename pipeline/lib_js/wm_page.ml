...
module Widget_item : Item with type item = Wm.widget = struct
  ...
  let t_of_layout_item (k, (v:item)) =
    let path =
      ...
      | "audio" -> music
      | _       -> help in
    let t = { icon     = Icon.SVG.(create_simple path)#widget
            ; name     = k
            ; unique   = true
            ; min_size = None
            ; item     = v } in
...
let create_widgets_grid
  ...
  let _ =
    React.E.map (fun _ ->
        let eq      =
          Widget_item.equal in
        let added   =
          List.filter (fun x -> not @@ List.mem ~eq x init) w.ig#items in
        let removed =
          List.filter (fun x -> not @@ List.mem ~eq x w.ig#items) init in
...
let create ~(init: Wm.t)
  ...
  let cont =
    Cont.make ~title
      ...
      () in
  let _ =
    wizard#listen Widget.Event.click (fun _ _ ->
        wz_show () |> Lwt.ignore_result; true) in
  let _ =
    ...
    React.(
      E.map (fun selected ->
          switch ~grid:cont.ig
            ...
            ())
      @@ E.select
           [ cont.ig#e_item_dblclick
           ; E.map (fun _ -> S.value cont.ig#s_selected |> Option.get_exn)
               edit#e_click ]) in
  let _ =
    save#listen Widget.Event.click (fun _ _ ->
  ...
  let _ =
    size#listen Widget.Event.click (fun _ _ ->
        let open Lwt.Infix in
