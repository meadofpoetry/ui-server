...
let make_player_action ?classes ?disabled ?on_path path =
  let make_icon ?(on = false) path =
    let classes = [Icon_button.CSS.icon] |> Utils.cons_if on Icon_button.CSS.icon_on in
  ...
  Player.Controls.(
    ...
    let fullscreen =
      make_player_action
        ...
        Player.Path.fullscreen
    in
    let volume_panel = Player.Controls.create_volume_panel [volume; slider] () in
...
let make_player () : 'a Html.elt =
  let audio = Player.create_audio ~autoplay:false ~playsinline:true ~controls:true () in
  let video = Player.create_video ~autoplay:true ~controls:false ~playsinline:true () in
