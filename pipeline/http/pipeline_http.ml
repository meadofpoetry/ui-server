...
let make_top_app_bar_row () =
  Tyxml.Html.toelt @@ Top_app_bar.F.top_app_bar_row ()
...
let pages () : Api_template.topmost Api_template.item list =
  ...
  let video_page_props =
    make_template_props ~title:"Мозаика"
      ~side_sheet:(make_side_sheet_props ~clipped:false ())
      ~top_app_bar_content:
        [
          Tyxml.Html.toelt
          @@ Top_app_bar.F.top_app_bar_section ~align:`End
  ...
  let editor_page_props =
    make_template_props ~title:"Редактор мозаики"
      ~top_app_bar_content:
      ...
      ~content:[] ()
  in
  simple ~priority:(`Index 2) ~title:"Мозаика"
    ~icon:(make_icon Components_tyxml.Svg_icons.view_grid)
    ~path:(Path.of_string video_path)
    video_page_props
  @ simple ~priority:(`Index 3) ~title:"Редактор мозаики"
  ...
  [
    merge ~prefix:"pipeline"
      [
        make ~prefix:"streams"
          [
            node ~doc:"List of streams" ~meth:`GET ~path:Path.Format.empty
              ~query:
                Query.
                  [
        ...
        make ~prefix:"structures"
          [
            node ~doc:"Annotated structures" ~meth:`GET
              ~path:Path.Format.("annotated" @/ empty)
              ~query:Query.empty
              (Pipeline_api.get_annotated state);
            node ~doc:"Applied structs" ~meth:`GET
            ...
            node ~doc:"Apply structs" ~meth:`POST ~restrict:[ `Guest ]
        ...
        make ~prefix:"wm"
          [
            node ~doc:"Wm" ~meth:`GET ~path:Path.Format.empty ~query:Query.empty
              (Pipeline_api.get_wm_layout state);
            node ~doc:"Post wm" ~restrict:[ `Guest ] ~meth:`POST
              ~path:Path.Format.empty ~query:Query.empty
              (Pipeline_api.apply_wm_layout state);
          ];
        make ~prefix:"settings"
          [
            node ~doc:"Settings" ~meth:`GET ~path:Path.Format.empty
              ~query:Query.empty
              (Pipeline_api.get_settings state);
            node ~doc:"Post settings" ~meth:`POST ~restrict:[ `Guest ]
              ~path:Path.Format.empty ~query:Query.empty
              (Pipeline_api.apply_settings state);
          ];
        make ~prefix:"status"
          [
            node ~doc:"Status" ~meth:`GET ~path:Path.Format.empty
              ~query:Query.[ ("id", (module List (Stream.ID))) ]
              (Pipeline_api.get_status state);
          ];
        make ~prefix:"history"
          [
            node ~doc:"Streams archive" ~meth:`GET
            ...
            node ~doc:"Structure archive" ~meth:`GET
  ...
  [
    merge ~prefix:"pipeline"
      [
        ...
        make ~prefix:"wm"
          [
            event_node ~doc:"WM socket" ~path:Path.Format.empty
              ~query:Query.empty
              (Pipeline_api.Event.get_wm_layout state);
          ];
        make ~prefix:"settings"
          [
            event_node ~doc:"Settings socket" ~path:Path.Format.empty
              ~query:Query.empty
              (Pipeline_api.Event.get_settings state);
          ];
        make ~prefix:"status"
          [
            event_node ~doc:"Stream status socket" ~path:Path.Format.empty
              ~query:Query.[ ("id", (module List (Stream.ID))) ]
