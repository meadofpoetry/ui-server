...
let default : t = { resolution = (1280, 720); widgets = []; layout = [] }
...
module Annotated = struct
  ...
  let annotate ~(active : raw) ~(stored : raw) : t =
    let merge_widgets ?(active : (string * widget) option)
    ...
    let merge_containers ?(active : (string * raw_container) option)
      ...
      let widgets =
        ...
        @@ List.map
             (fun (active, stored) -> merge_widgets ?active ?stored ())
    ...
    let active_conts, stored_conts' =
      match_and_filter
        ~pred:(fun (n1, _) (n2, _) -> String.equal n1 n2)
        ~main:active.layout stored.layout
    in
    ...
    in
    { resolution = active.resolution; widgets = active.widgets; layout }
  ...
  let update_stored ~(active : raw) ~(stored : raw) =
    ...
    let for_each_widg ?(active_widget : (string * widget) option)
    ...
    let for_each_cont ?(active : (string * raw_container) option)
        ?(stored : (string * raw_container) option) () =
      let active_widgets, stored_widgets' =
        match_and_filter ~pred:equal_widget
          ~main:(opt_get_list active ~deref:(fun (_, x) -> x.widgets))
          (opt_get_list stored ~deref:(fun (_, x) -> x.widgets))
      in
      ...
      let widgets =
        List.filter_map Fun.id
        @@ List.map
             (fun (active_widget, stored_widget) ->
      ...
      | Some (id, c), _ | _, Some (id, c) ->
          if widgets = [] then None else Some (id, { c with widgets })
    in
  ...
  let filter ~select (annotated : t) : raw =
    let rec filter_widgets = function
      | [] -> []
      | (id, state, widget) :: tl ->
          if state <> select then filter_widgets tl
          else (id, widget) :: filter_widgets tl
    in

    let rec filter_layout :
        (string * state * container) list -> (string * raw_container) list =
      function
      | [] -> []
      | (id, state, cont) :: tl ->
          if state <> select then filter_layout tl
          else
