...
module Packed = struct
  ...
  type nonrec t = { source : Stream.t; structure : t }

  let to_yojson { source; structure } =
    `Assoc
      [
        ("source", Stream.to_yojson source); ("structure", to_yojson structure);
      ]
  ...
  let of_yojson = function
    | `Assoc [ ("source", src); ("structure", str) ] ->
        Stream.of_yojson src >>= fun source ->
        of_yojson str >>= fun structure -> Ok { source; structure }
      ...
      let channels =
        List.fold_left
          (fun acc c ->
            let pids =
              List.fold_left
                (fun acc p -> (s.id, c.number, p.pid) :: acc)
                [] c.pids
            in
            pids @ acc)
          [] s.channels
...
module Annotated = struct
  ...
  let opt_map_any ~f opt1 opt2 opt3 =
    ...
    | None -> (
        ...
        | None -> (
            match opt3 with Some v -> Some (f `Third v) | None -> None ) )

  let opt_get_list opt ~deref = match opt with None -> [] | Some l -> deref l
  ...
  let annotate ~(active : raw) ~(avail : raw) ~(stored : raw) : t =
    let merge_pids ?active ?avail ?stored () =
      opt_map_any active avail stored ~f:(fun prior (pid : raw_pid) ->
          match prior with
          | `Third -> (`Stored, pid)
          | `Second -> (`Avail, pid)
    ...
    let merge_channels ?active ?avail ?stored () =
      ...
      let active_args, avail', stored' =
        match_and_filter ~pred:eq_pid_num
      ...
      in
      opt_map_any active avail stored ~f:(fun prior (ch : raw_channel) ->
          let c =
            {
              number = ch.number;
    ...
    let merge_structures ?active ?avail ?stored () : (state * structure) option
      ...
      let active_args, avail', stored' =
        match_and_filter ~pred:eq_chan_num
          ~rearrange:(fun a b c -> (a, b, c))
          ~main:(opt_get_list active ~deref)
          (opt_get_list avail ~deref)
          (opt_get_list stored ~deref)
      ...
      let channels =
        ...
        |> List.filter_map Fun.id
      in
      opt_map_any active avail stored ~f:(fun prior (s : raw_structure) ->
    ...
    let active_structs, avail', stored' =
      match_and_filter ~pred:eq_struct_id
        ~rearrange:(fun a b c -> (a, b, c))
        ~main:active avail stored
    in
  ...
  let update_stored ~(active : raw) ~(avail : raw) ~(stored : raw) =
    ...
    let for_each_channel ?active_struct ?stored_struct ch =
      ...
      in
      let active_chan =
        opt_get_list active_struct ~deref:(fun (c : raw_structure) ->
            c.channels)
        |> List.find_opt (eq_chan ch)
      in
      let stored_chan =
        opt_get_list stored_struct ~deref:(fun (c : raw_structure) ->
            c.channels)
        |> List.find_opt (eq_chan ch)
      in
    ...
    let for_each_structure s =
      let eq_struct (s1 : raw_structure) (s2 : raw_structure) = s1.id = s2.id in
      let active_struct = List.find_opt (eq_struct s) active in
      let stored_struct = List.find_opt (eq_struct s) stored in
      let channels =
        List.map (for_each_channel ?active_struct ?stored_struct) s.channels
      in
      { s with channels }
    in

    let res = List.map for_each_structure avail in
    match !updated with true -> `Changed res | false -> `Kept active
  ...
  let filter ~select (annotated : t) : raw =
    ...
    let rec filter_structs = function
      ...
      | (_, s) :: tl ->
          let s' : raw_structure =
            { id = s.id; uri = s.uri; channels = filter_channels s.channels }
          in
          s' :: filter_structs tl
    in
...
module Many = struct
  ...
  let equal l r =
    try
      let in_r, not_in_r =
        List.partition (fun el -> List.exists (fun er -> equal el er) r) l
      in
      if not_in_r <> [] then raise_notrace Not_found;
      List.iter
        (fun er ->
          if not @@ List.exists (fun el -> equal el er) in_r then
    ...
    | `List l -> (
        try
          Ok
            (List.map
               (fun x ->
                 let[@warning "-8"] (Ok res) = of_yojson x in
                 res)
               l)
