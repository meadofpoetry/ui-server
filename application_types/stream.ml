...
module ID : sig
  ...
  let to_yojson (x : t) : Yojson.Safe.t = `String (Uuidm.to_string x)
...
module Multi_TS_ID : sig
  ...
  (* ensure zeros at right places *)

  let make ~source_id ~stream_id : t = Parsed { source_id; stream_id }
  ...
  let to_yojson (t : t) = Util_json.Int32.to_yojson (to_int32_pure t)
...
let compare l r = if equal l r then 0 else compare l r
...
type stream_setting = (marker * t list) list [@@deriving yojson, eq]

type stream_table_row = marker * Table.source_state * Table.stream list
[@@deriving yojson, eq]

type stream_table = stream_table_row list [@@deriving yojson, eq]

type stream_list = (board_num * t list) list
