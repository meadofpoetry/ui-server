...
type pass = { user : t; password : string } [@@deriving yojson]

type pass_change = { user : t; old_pass : string; new_pass : string }
[@@deriving yojson, eq]

let to_int = function `Root -> 0 | `Operator -> 1 | `Guest -> 2

let of_int = function 0 -> `Root | 1 -> `Operator | _ -> `Guest

let is_root = function `Root -> true | _ -> false

type 'a user_table = { root : 'a; operator : 'a; guest : 'a }

let empty_table = { root = []; operator = []; guest = [] }
