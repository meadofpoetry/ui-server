...
let env_of_yojson : Yojson.Safe.t -> (env, string) result = function
  ...
  | j ->
      let e =
        Printf.sprintf "env_of_yojson: got bad value - %s" (Yojson.Safe.to_string j)
      in
      Error e
...
let iter_boards f =
  ...
  and iter_cpu f c = List.iter (fun iface -> iter_entry f iface.conn) c.ifaces in
  function
  | `CPU c -> iter_cpu f c
...
let get_entries = function
  | `Boards l ->
      List.fold_left (fun acc b -> List.map (fun p -> p.child) b.ports @ acc) [] l
...
let get_board_name (b : topo_board) = Printf.sprintf "%s %s" b.manufacturer b.model
...
let board_list_for_input input : t -> 'a =
  ...
  | `Boards bs ->
      fmap_first
        (fun board -> fmap_first (fun port -> traverse [] port.child) board.ports)
        bs
