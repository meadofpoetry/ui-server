...
type identity += Device of int * int | (* id * type *) Stream of int32
...
class device id typ =
  object
    ...
    method check = function `OK -> is_sent <- true | _ -> is_sent <- false

    method get_data _ =
      Lwt.return_some
        (`Assoc
          [ ("serial", `Int id); ("type", `Int typ); ("name", `String "ATS") ])
