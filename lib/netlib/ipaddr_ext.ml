module V4 = struct
  ...
  let range_of_pair (min, max) = if min <= max then Some (min, max) else None

  let range_to_pair (min, max) = (min, max)

  let equal (t1 : t) (t2 : t) = Int32.equal (to_int32 t1) (to_int32 t2)
  ...
  let succ ip = Ipaddr.V4.of_int32 @@ Int32.succ @@ Ipaddr.V4.to_int32 ip

  let pred ip = Ipaddr.V4.of_int32 @@ Int32.pred @@ Ipaddr.V4.to_int32 ip

  let max_ip = Ipaddr.V4.of_string_exn "255.255.255.255"

  let min_ip = Ipaddr.V4.of_string_exn "0.0.0.0"
  ...
  let gen_in_ranges ?(forbidden : t list = []) ~allowed
      (vs : ('a * (t * t) list) list) : ('a * t) list =
    (* Debug
    ...
    let random_in ~ranges =
      ...
      if max > min then
        let min, max = (to_int32 min, to_int32 max) in
        Int32.add (Random.int32 Int32.(sub max min)) min |> of_int32
      else min
    in
    let split_ranges ~excluded ranges =
      let exclude (min, max) =
        if in_range (min, max) excluded then
          [ (min, pred excluded); (succ excluded, max) ]
          (* TODO check if overlap *)
        else [ (min, max) ]
      in
      List.concat @@ List.map exclude ranges
    in
    let constraint_by_allowed ~allowed_ranges ranges =
      ...
      let rec constrain ~allowed = function
        | [] -> [ allowed ]
        | [ h ] -> [ intersect allowed h ]
        | h :: tl -> intersect allowed h :: constrain ~allowed tl
      in
      List.fold_left
        (fun ranges allowed -> constrain ~allowed ranges)
    ...
    let rec solve = function
      ...
      | (id, ranges) :: rest ->
          ...
          while is_none !result && !tries > 0 do
            ...
            let rest' =
              List.map
                (fun (id, ranges) -> (id, split_ranges ~excluded:ip ranges))
                rest
            in
            match solve rest' with
            | Some result' -> result := Some ((id, ip) :: result')
            | None -> decr tries
          done;
          !result
    in
    ...
    let allowed =
      List.fold_left
        (fun ranges excluded -> split_ranges ~excluded ranges)
        [ allowed ] forbidden
    in
    let constrained, unconstrained =
      List.partition (fun (_, r) -> not @@ (r = [])) vs
    in
    ...
    let constrained =
      List.map
        ...
        constrained
    in
    let unconstrained = List.map (fun (id, _) -> (id, allowed)) unconstrained in
