...
let cons_maybe = function None -> fun l -> l | Some v -> fun l -> v :: l

let opt_map f = function None -> None | Some v -> Some (f v)

let result_map f = function Error _ as e -> e | Ok v -> Ok (f v)
...
module Scheme = struct
  ...
  let is uri s =
    match scheme uri with None -> false | Some scheme -> String.equal scheme s
end
...
module Path = struct
  ...
  let split s = String.split_on_char '/' s |> List.filter (not % String.equal "")
  ...
  let rec templ_compare l r =
    match (l, r) with
    | `S l :: ls, `S r :: rs ->
        let c = String.compare l r in
        if c = 0 then templ_compare ls rs else c
    | _ :: ls, _ :: rs -> templ_compare ls rs
    | [], [] -> 0
    | _, [] -> 1
    | [], _ -> -1

  let next = function [] as l -> (None, l) | h :: tl -> (Some h, tl)
  ...
  module Format = struct
    ...
    let ( @/ ) s fmt =
      let toks = split s in
      let rec merge = function [] -> fmt | h :: tl -> S (h, merge tl) in
      merge toks
    ...
    let rec scan_unsafe : type a b. string list -> (a, b) t -> a -> b =
     fun path fmt f ->
      match (path, fmt) with
      ...
      | h :: tl, F (Uuid, fmt) ->
          scan_unsafe tl fmt (f @@ get_exn @@ Uuidm.of_string h)
    ...
    let rec kprint : type a b. (string list -> b) -> (a, b) t -> a =
     fun k -> function
      ...
      | F (String, rest) ->
          let f x = kprint (fun lst -> k (x :: lst)) rest in
          f
      | F (Int, rest) ->
          let f x = kprint (fun lst -> k (string_of_int x :: lst)) rest in
          f
      | F (Int32, rest) ->
          let f x = kprint (fun lst -> k (Int32.to_string x :: lst)) rest in
          f
      | F (Uuid, rest) ->
          let f x = kprint (fun lst -> k (Uuidm.to_string x :: lst)) rest in
          f
      | F (Bool, rest) ->
          let f x = kprint (fun lst -> k (string_of_bool x :: lst)) rest in
          f
      | F (Any, rest) ->
          let f () = kprint (fun lst -> k ("*" :: lst)) rest in
          f

    let doc : type a b. (a, b) t -> string =
     fun fmt ->
...
module Query = struct
  type err = Key_not_found of string | Parser_error of string
  ...
  let grep_arg (name : string) lst =
    let rec grep' acc = function
      | [] -> (None, lst)
      | (title, arg) :: tl ->
          if String.equal title name then (Some arg, List.rev acc @ tl)
  ...
  module Either (L : Show) (R : Show) = struct
    ...
    let of_string s =
      try `Left (L.of_string s)
      with _ -> (
        try `Right (R.of_string s) with _ -> raise_notrace (Failure "Neither") )
  ...
  module List (E : Show) : Convert with type t = E.t list = struct
    ...
    let of_query = function None -> [] | Some l -> List.map E.of_string l

    let to_query v =
      match v with [] -> None | v -> Some (List.map E.to_string v)
  end
   ...
   fun k -> function
    ...
    | (q, (module C)) :: rest ->
        let f x =
          make_q
            (fun lst ->
              k (cons_maybe (opt_map (fun x -> (q, x)) (C.to_query x)) lst))
            rest
        in
        f
   ...
   fun k -> function
    ...
    | (q, (module C)) :: rest ->
        fun sl ->
          ...
          parse_q
            (k
               ( try C.of_query arg with
               | Not_found -> raise_notrace (Key_not_found_exn q)
               | exn -> raise_notrace exn ))
            rest args

  let parse_query' lst f queries =
    try Ok (parse_q f lst queries) with
    | Key_not_found_exn key -> Error (Key_not_found key)
    | exn -> Error (Parser_error (Printexc.to_string exn))

  let parse_query lst f queries = parse_query' lst f queries |> result_map fst
  ...
  let doc queries =
    let rec loop : type a b. (a, b) format -> (string * string) list = function
      | [] -> Stdlib.List.[]
      | (name, (module C)) :: tl -> Stdlib.List.cons (name, C.typ) (loop tl)
    in
    loop queries
end

let handle_uri ~path ~query f uri =
            ...
            M.fold
              (fun path node m ->
                ...
                let handler uri =
                  ...
                  in
                  let path' =
                    Path.merge @@ drop len @@ Path.of_string @@ Uri.path uri
                  in
 ...
 *)
  ...
  let concat (l : 'a t list) =
    ...
    | [] -> M.empty
    | [ x ] -> x
    | acc :: l' -> List.fold_left (M.merge merge_fun) acc l'

  let dispatch ~default (m : 'a t) (uri : uri) =
    let templ = Path.to_templ (Path.of_string @@ path uri) in
    try (M.find templ m).handler uri with _ -> default

  let doc (m : 'a t) : string list =
    let gen node =
      let queries q =
        String.concat ""
        @@ List.map (fun (name, typ) -> Printf.sprintf "\t%s : %s\n" name typ) q
      in
      let doc = match node.docstring with None -> "Absent" | Some s -> s in
      Printf.sprintf "\t%s\nDoc: %s\n%s"
        (Path.templ_to_string node.templ)
        doc (queries node.query_typ)
    in
    M.fold (fun _ node acc -> gen node :: acc) m []
end
...
let host_v4 uri =
  ...
  | None -> None
  | Some host -> (
      match Ipaddr_ext.V4.of_string host with Error _ -> None | Ok x -> Some x )

let with_host_v4 uri ip = with_host uri @@ Some (Ipaddr_ext.V4.to_string ip)

let with_path_parsed uri p = with_path uri @@ Path.to_string p
