...
end) : RO_V with type value := S.t = struct
  let create ?default kv keys =
    let open Lwt.Infix in
    Kv.RO.read_opt kv keys >>= function
    | None -> Lwt.return default
    | Some v -> (
        try Lwt.return_some @@ S.of_string v with _ -> Lwt.return_none )
end

type 'value rw =
  < get : 'value Lwt.t ; set : 'value -> unit Lwt.t ; s : 'value React.S.t >
...
module type RW_V = sig
  ...
  val create :
    ?default:value -> Kv.RW.t -> Kv.RW.key list -> (t, [> error ]) result Lwt.t
end
...
module RW (S : sig
  ...
  let construct kv keys v =
    ...
    Lwt.return_ok
      (object
         ...
         method set x =
           Kv.RW.write kv keys (S.to_string x) >>= fun _ -> Lwt.return_unit
  ...
  let create ?default kv keys =
    ...
    | Some v, _ | None, Some v -> construct kv keys v
end
