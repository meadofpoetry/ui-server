...
let delay f =
  ignore @@ Dom_html.setTimeout f 0.
...
let update_tabbable_nodes (t : t) : unit =
  ...
  let first_tabbable = match tabbable with
    | [] -> get_initial_focus_node t
    | x :: _ -> x in
  let last_tabbable = match List.rev tabbable with
    | [] -> get_initial_focus_node t
    | x :: _ -> x in
...
let deactivate ?return_focus (t : t) : unit =
  if t.state.active then (
    let return_focus = match return_focus with
    ...
    (match t.options.on_deactivate with
     ...
     | Some f -> f ());
    if return_focus
    then delay (fun () -> try_focus t t.state.focused_before))
...
let check_focus_in (t : t) (e : Dom_html.event Js.t) =
  ...
  then (
    let node = match t.state.recently_focused with
      | Some x -> Some x
      | None -> Some (get_initial_focus_node t) in
