...
let touch e =
  Js.Optdef.get (e##.changedTouches##item 0) (fun () -> failwith "touch fail")

let rec find_touch id num source =
  (* given a touchList Js.t, finds a touch with needed identifier among *)
  if num < 0
  then None
  else if Js.Optdef.test (source##item num)
  then
    let touch =
      Js.Optdef.get (source##item num) (fun () -> failwith "No touch with such id")
    in
    if touch##.identifier = id then Some touch else find_touch id (num - 1) source
...
let dispatch ?data touch typ target : unit =
  ...
  let () =
    Js.Unsafe.set evt "dataTransfer" (Js.Unsafe.pure_js_expr "new DataTransfer()")
  in
...
class t ~data ~typ elt () =
  object (self)
    ...
    method private handle_touch_end e _ =
      ...
      Js.Opt.iter
        (elt_from_point touch##.clientX touch##.clientY)
    ...
    method private handle_touch_move e _ =
      ...
      | true, Some id -> (
        match find_touch id (e##.changedTouches##.length - 1) e##.changedTouches with
        | Some touch ->
            Js.Opt.iter
              (elt_from_point touch##.clientX touch##.clientY)
            ...
            Option.iter
              (fun cln ->
                ...
                cln##.style##.top
                := Js.string @@ string_of_int (touch##.pageY - dy) ^ "px")
              clone
        | None -> ())
      | _, _ -> ());
      Lwt.return_unit
