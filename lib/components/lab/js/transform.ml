...
let get_z_index (x : Dom_html.element Js.t) : int option =
  int_of_string_opt
  @@ Js.to_string (Dom_html.window##getComputedStyle x)##.zIndex
...
type position = { left : float; top : float; width : float; height : float }
...
module Event = struct
  ...
  class type select = [Dom_html.element Js.t] Dom_html.customEvent

  class type input = [detail Js.t] Dom_html.customEvent

  class type event = [Dom_html.clientRect Js.t] Dom_html.customEvent
...
class t ?(transformables = []) ?(move_threshold = 10.)
  ...
  object (self)
            ...
            (fun () ->
              ...
              let touch =
                ...
                @@ Js.Optdef.map
                     ...
                     (fun touch -> touch##.identifier)
              in
              (None, touch))
            (fun e -> (Some e##.button, None))
        in
        let action =
          match button with
          | None | Some 0 ->
              if Element.has_class target CSS.resizer then
                match direction_of_event event with
                | None -> None
    ...
    method private handle_delayed_drag_move
      ...
      fun ({ point; touch_id; _ } as state) event _ ->
        ...
        if Float.abs (x +. y -. (fst point +. snd point)) >= move_threshold then (
    ...
    method private handle_drag_end
      ...
      fun state event _ ->
        let f () =
          ...
          List.iter Lwt.cancel _temp_listeners;
          _temp_listeners <- [];
          if state.single_click then self#handle_single_click state event
          else (
            self#notify_change ();
            Lwt.return_unit )
        in
        Js.Opt.case
          ...
          (fun () ->
            ...
            | Some id -> (
                ...
                | Some _ -> f () ))
          (fun (e : Dom_html.mouseEvent Js.t) ->
            match e##.button with 0 -> f () | _ -> Lwt.return_unit)
    ...
    method private resize
        : 'a. direction -> state -> (#Dom_html.event as 'a) Js.t -> unit Lwt.t =
      fun direction
          ( {
              touch_id;
              point = x, y;
...
let attach ?transformables elt =
  new t ?transformables (elt :> Dom_html.element Js.t) ()
