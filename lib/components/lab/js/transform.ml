...
class t ?(transformables = []) ?(move_threshold = 10.) (elt : Dom_html.element Js.t) () =
  object (self)
    ...
    method private move : 'a. state -> (#Dom_html.event as 'a) Js.t -> unit Lwt.t =
      fun ({touch_id; point = x, y; position = {left; top; _} as position; _} as state) e ->
    ...
    method private resize
      ...
      fun direction
          ({touch_id; point = x, y; position = {left; top; width; height} as position; _}
          as state)
          e ->
        let page_x, page_y = get_cursor_position ?touch_id e in
        let dx, dy = page_x -. x, page_y -. y in
        let position =
          ...
          | NW ->
              ...
              ; top = top +. dy }
          | NE ->
              {position with width = width +. dx; height = height -. dy; top = top +. dy}
          | SW ->
              { position with
                width = width -. dx
              ; height = height +. dy
