...
class t
  ...
  method private handle_single_click (_state : state)
    ...
    let rec includes el =
      ...
      then Some el
      else Js.Opt.case (Element.get_parent el) (fun () -> None) includes in
    (* Traverse dom upwards to check if target is selectable *)
    match includes target with
    | None -> self#notify_event `Outside_click e
    | Some target ->
      self#notify_event `Start e;
      let shift_key = Js.Opt.case (Dom_html.CoerceTo.mouseEvent e)
          (fun () -> false)
          (fun x -> Js.to_bool x##.shiftKey) in
