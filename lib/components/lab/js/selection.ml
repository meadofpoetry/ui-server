  ...
  List.fold_left
    (fun acc -> function
      | Query q ->
          ...
          Dom.list_of_nodeList nodes @ acc | Node e -> e :: acc)
    [] elts

let coerce e = (e :> Dom_html.event Js.t)

let parse_event (e : Dom_html.event Js.t) :
    Dom_html.element Js.t * float * float =
  let tap =
    if
      Js.Optdef.test (Js.Unsafe.coerce e)##.touches
...
let event_path (e : Dom_html.event Js.t) =
  ...
  let path =
    if Js.Optdef.test path then path
    else if Js.Optdef.test (Js.Unsafe.coerce e)##.composedPath then
      (Js.Unsafe.coerce e)##composedPath
    else Js.undefined
  in
...
let collides (a : Dom_html.clientRect Js.t) (b : Dom_html.clientRect Js.t) =
  function
  | Cover ->
      b##.left >= a##.left
      && b##.top >= a##.top
...
class t ?(multiple = true) ?(single_click = true) ?(start_threshold = 10)
  ?(scroll_speed_divider = 10.) ?(start_areas = [ Query "html" ])
  ?(boundaries = [ Query "html" ]) ?(selectables = []) ?(mode = Touch)
  ?(class_ = "selection-area") ?(container = Query "body") ?before_start
  ?on_start ?on_move ?on_stop ?on_outside_click (clip : Dom_html.element Js.t)
  () =
  object (self)
    ...
    method select (items : Dom_html.element Js.t list) : unit =
      let items =
        List.filter
          (fun x ->
            (not @@ List.memq x _selected) && (not @@ List.memq x _stored))
          items
      in
    ...
    method private notify_event_bool typ (e : Dom_html.event Js.t) : bool =
      let f = match typ with `Before_start -> before_start in
      match f with None -> true | Some f -> f (self :> t) (self#make_detail e)
    ...
    method private notify_event typ (e : Dom_html.event Js.t) : unit =
      ...
      in
      match f with None -> () | Some f -> f (self :> t) (self#make_detail e)
    ...
    method private handle_drag_start (e : Dom_html.event Js.t) _ : unit Lwt.t =
      ...
      let target_container =
        List.find_opt
          (fun x -> collides x##getBoundingClientRect rect _mode)
          boundaries
      in
      ...
      let contains_start_areas =
        List.exists
          (fun x ->
            List.exists (( == ) (x :> Dom_html.eventTarget Js.t)) evt_path)
          start_areas
      in
      ...
      | Some target_container, true, true ->
          if self#notify_event_bool `Before_start e then (
            let state =
              (* Area start point *)
              {
                ax1 = x;
            ...
            _temp_listeners <-
              Lwt_js_events.
                [
                  ...
                  dragends doc
                    (self#handle_drag_end ~no_event:true state % coerce);
    ...
    method private handle_single_click (_state : state)
      ...
      let rec includes el =
        if List.memq el _selectables then Some el
      ...
      | Some target -> (
          ...
          | true, _ :: _ -> () (* TODO *)
          | _ ->
              if List.memq target _stored then self#remove_from_selection target
    ...
    method private init_multiple_selection e ({ target_container; _ } as state)
      ...
      if scroll_available then (
        (* Detect mouse scrolling *)
        _temp_listeners <-
          Lwt_js_events.(
            seq_loop
              (make_event @@ Dom_html.Event.make "wheel")
              ~passive:false Dom_html.window (self#manual_scroll state)
            :: _temp_listeners);
        ...
        _selectables <-
          List.filter (Element.contains target_container) _selectables;
      ...
      self#handle_drag_move state e Lwt.return_unit >>= fun () ->
      self#notify_event `Start e;
      Lwt.return_unit
    ...
    method private handle_delayed_drag_move ({ ax1; ay1; _ } as state : state)
      ...
      then (
        ...
        if _multiple then self#init_multiple_selection e state
        else Lwt.return_unit )
      else Lwt.return_unit )
      >>= fun () ->
      Dom.preventDefault e;
    ...
    method private handle_drag_move (state : state) (e : Dom_html.event Js.t) _
        : unit Lwt.t =
      let _, x, y = parse_event e in
      state.ax2 <- x;
      state.ay2 <- y;
      ...
      | true, (Some _, None | None, Some _) ->
          let rec scroll _ =
            ...
            | None, None ->
                Lwt.return_unit (* Scrolling is not anymore required *)
            | ss_x, ss_y ->
                ...
                | Some ss_y ->
                    let v =
                      Float.(to_int @@ ceil (ss_y /. scroll_speed_divider))
                    in
                    scon##.scrollTop := scon##.scrollTop + v;
                    state.ay1 <-
                      state.ay1 -. float_of_int scon##.scrollTop -. scroll_top
                );
                ( match ss_x with
                | None -> ()
                | Some ss_x ->
                    let v =
                      Float.(to_int @@ ceil (ss_x /. scroll_speed_divider))
                    in
    ...
    method private handle_drag_end ?(no_event = false) (state : state)
      ...
      _delayed_listeners <- [];
      _temp_listeners <- [];
      if _is_single_click && _single_click then self#handle_single_click state e
      else if (not _is_single_click) && not no_event then (
        self#update_touching_elements ();
        self#notify_event `Stop e );
      ...
      let added, touched =
        List.fold_left
          (fun ((added, touched) as acc) item ->
            if collides area_rect item##getBoundingClientRect _mode then
              let added =
    ...
    method private manual_scroll state e _ =
      ...
      state.scroll_speed <-
        (let dy = scroll_speed_divider *. (float_of_int e##.deltaY *. -1.) in
         ...
         ( Some (add dx (fst state.scroll_speed)),
           Some (add dy (snd state.scroll_speed)) ));
    ...
    method private redraw_area (state : state) : unit =
      ...
      let ss_x, x =
        if x < left then
          let ss =
            if scroll_left <> 0 then Some Float.(neg (abs (left -. x)))
            else None
          in
          (ss, left)
        else if x > left +. width then
          let ss =
            if scroll_width - scroll_left - client_width <> 0 then
              Some Float.(abs (top +. height -. y))
            else None
          in
          (ss, left +. width)
        else (None, x)
      ...
      let ss_y, y =
        if y < top then
          let ss =
            if scroll_top <> 0 then Some Float.(neg (abs top -. y)) else None
          in
          (ss, top)
        else if y > top +. height then
          let ss =
            if scroll_height - scroll_top - client_height <> 0 then
              Some Float.(abs (top +. height -. y))
            else None
          in
          (ss, top +. height)
...
let make ?multiple ?single_click ?start_threshold ?scroll_speed_divider
    ?start_areas ?boundaries ?selectables ?mode ?class_ ?container ?before_start
    ?on_start ?on_move ?on_stop ?on_outside_click () =
  let elt = Dom_html.(createDiv document) in
  new t
    ?multiple ?single_click ?before_start ?start_threshold ?scroll_speed_divider
    ?start_areas ?boundaries ?selectables ?mode ?class_ ?container ?on_start
    ?on_move ?on_stop ?on_outside_click elt ()
