...
let focusable_candidate_selector =
  String.concat "," ("iframe" :: selectors)
...
let coerce_to_input (elt : node) : Dom_html.inputElement Js.t option =
  ...
  | "INPUT" -> Some (Js.Unsafe.coerce elt) | _ -> None
  ...
  | Some i ->
     ...
     | name ->
        let (owner : Dom_html.document Js.t) =
          (Js.Unsafe.coerce i)##.ownerDocument in
        let (selector : string) =
          Printf.sprintf "input[type=\"radio\"][name=\"%s\"]" name in
...
let get_tab_index (elt : node) : int =
  let attr = match Element.get_attribute elt "tabindex" with
    | None -> None
    | Some x -> int_of_string_opt x in
  match attr with
  | Some x -> x
  | None ->
     (* Browsers do not return `tabIndex` correctly for contentEditable nodes;
          ...
          assume it's 0. *)
     if is_content_editable elt then 0
     else (Js.Unsafe.coerce elt)##.tabIndex
...
let is_node_matching_selector_focusable (elt : node) : bool =
  let disabled = match Js.Optdef.to_option (Js.Unsafe.coerce elt)##.disabled with
    | None -> false
    | Some x -> Js.to_bool x in
