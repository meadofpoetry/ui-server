(** @see <https://github.com/davidtheclark/focus-trap> . [focus-trap] JavaScript
    library rewritten in OCaml. *)

(** When a focus trap is activated, this is what should happen:

    - Some element within the focus trap receives focus. By default, this will
      be the first element in the focus trap's tab order. Alternately, you can
    ...
    - Clicks within the focus trap behave normally; but clicks outside the focus
      trap are blocked.
    - The [Escape] key will deactivate the focus trap. When the focus trap is
      deactivated, this is what should happen:
