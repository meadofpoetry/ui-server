...
val make_position :
  ?top:float -> ?right:float -> ?left:float -> ?bottom:float -> unit -> position
...
module Event : sig
  val opened : unit Dom_html.customEvent Js.t Dom_html.Event.typ
  (** Event emitted after the menu surface is opened. *)

  val closed : unit Dom_html.customEvent Js.t Dom_html.Event.typ
...
class t :
  ...
  -> object
       ...
       method private adjust_position_for_hoisted_element :
         layout -> (string * float) list -> (string * float) list
...
val attach :
  ?body:#Dom_html.element Js.t -> ?viewport:viewport -> Dom_html.element Js.t -> t
