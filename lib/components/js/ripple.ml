...
module Util = struct
    ...
    | true, _ | _, None ->
        ...
        if not supports_function_present then false
          ...
          let we_are_feature_detecting_safary_10_plus =
            let a =
              Js.to_bool
              @@ window ##. CSS##supports (Js.string "(--css-vars: yes)")
            in
            let b =
              Js.to_bool
              @@ window ##. CSS##supports
                   (Js.string "color") (Js.string "#00000000")
            in
            a && b
          in
          let supports =
            if
              explicitly_supports_css_vars
  ...
  let get_normalized_event_coords (event : Dom_html.event Js.t)
      (page_offset : point) (client_rect : Dom_html.clientRect Js.t) =
...
let update_css_variable node name value =
  (Js.Unsafe.coerce node##.style)##setProperty
    (Js.string name)
    (match value with None -> Js.string "" | Some s -> Js.string s)

let make_default_adapter ?(is_unbounded = false) (elt : #Dom_html.element Js.t)
    : adapter =
  let elt = Element.coerce elt in
  {
    ...
    is_surface_disabled =
      (fun () ->
        if Js.Optdef.test (Js.Unsafe.coerce elt)##.disabled then
          Js.to_bool (Js.Unsafe.coerce elt)##.disabled
        else false);
...
let (pointer_deactivation_event_types :
      Dom_html.event Js.t Dom_html.Event.typ list) =
...
class t (adapter : adapter) () =
  object (self)
    ...
    method destroy () =
      if self#supports_press_ripple () then (
    ...
    method set_unbounded (x : bool) : unit =
      if x then self#add_class CSS.unbounded
      else self#remove_class CSS.unbounded
    ...
    (* Private methods *)
    method private handle_focus (_ : Dom_html.event Js.t) (_ : unit Lwt.t)
        : unit Lwt.t =
      Lwt_js_events.request_animation_frame () >>= fun _ ->
      Lwt.return @@ self#add_class CSS.bg_focused

    method private handle_blur (_ : Dom_html.event Js.t) (_ : unit Lwt.t)
        : unit Lwt.t =
      Lwt_js_events.request_animation_frame () >>= fun () ->
      Lwt.return @@ self#remove_class CSS.bg_focused
    ...
    (** We compute this property so that we are not querying information about
        ...
        too early, such as when components are rendered on the server and then
    ...
    method private register_root_handlers (supports_press_ripple : bool) : unit
      ...
      Lwt_js_events.(
        if supports_press_ripple then
          let handler e _ =
            self#activate ~event:(e :> Dom_html.event Js.t) ()
          in
          (* ;
           * if adapter.is_unbounded () then (
    ...
    method activate_ (event : Dom_html.event Js.t option) : unit Lwt.t =
      ...
      if adapter.is_surface_disabled () then Lwt.return ()
        ...
        if was_element_made_active then
          _activation_thread <- Some (self#animate_activation ());
        Lwt_js_events.request_animation_frame () >>= fun () ->
        (* Reset array on next frame after the current event has had a chance to
           bubble to prevent ancestor ripples. *)
        _activated_targets <- [];
        if
          (not was_element_made_active)
          &&
          match event with
          | None -> false
        ...
        then (
          let was_element_made_active = self#check_element_made_active event in
          if was_element_made_active then
            Lwt.ignore_result @@ self#animate_activation ();
          _activation_state <-
            { _activation_state with was_element_made_active } );
        (* Reset activation state immediately if element was not made active. *)
        if not _activation_state.was_element_made_active then
    ...
    method private animate_activation () : unit Lwt.t =
      ...
      Lwt.catch
        (fun () ->
          Lwt_js.sleep Const.deactivation_timeout_s >>= fun () ->
    ...
    method private run_deactivation_ux_logic_if_ready () : unit Lwt.t =
      ...
      if activation_has_ended && _activation_animation_has_ended then (
        self#rm_bounded_activation_classes ();
        self#add_class CSS.fg_deactivation;
        let t =
          Lwt_js.sleep Const.fg_deactivation_s >>= fun () ->
          Lwt.return @@ self#remove_class CSS.fg_deactivation
        in
    ...
    method private reset_activation_state () : unit Lwt.t =
      ...
      Lwt_js.sleep Const.tap_delay_s >>= fun () ->
    ...
    method private animate_deactivation (a : activation_state) : unit Lwt.t =
      if a.was_activated_by_pointer || a.was_element_made_active then
        self#run_deactivation_ux_logic_if_ready ()
      else Lwt.return ()
    ...
    method private update_layout_css_vars () : unit =
      ...
      if self#unbounded then (
        ...
        let y =
          Float.(round ((_frame.height /. 2.) -. (_initial_size /. 2.)))
        in
