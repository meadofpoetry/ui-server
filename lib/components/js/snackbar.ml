...
type dismiss_reason = Action | Dismiss | Timeout | Custom of string
...
module Const = struct
  ...
  (** Number of seconds to wait between temporarily clearing the label text in
      the DOM and subsequently restoring it. This is necessary to force IE 11 to
...
let announce ?(label_elt : Element.t option) (aria_elt : Element.t) =
  let label_elt = match label_elt with Some x -> x | None -> aria_elt in
  ...
  | Some label_text, Some priority ->
      ...
      label_elt##.innerHTML :=
        Js.string
          "<span style=\"display: inline-block; width: 0; height: \
           1px;\">&nbsp;</span>";
      ...
      Js_of_ocaml_lwt.Lwt_js.sleep Const.aria_live_delay_s >>= fun () ->
...
class t ?(auto_dismiss_timeout = Const.def_auto_dismiss_timeout_s)
  ...
  object (self)
    ...
    val label_element : Element.t =
      Element.query_selector_exn elt Selector.label

    val surface_element : Element.t =
      Element.query_selector_exn elt Selector.surface
    ...
    method set_timeout (x : float) : unit =
      if
        x <= Const.max_auto_dismiss_timeout_s
        && x >= Const.min_auto_dismiss_timeout_s
      then auto_dismiss_timeout <- x
      else
        let s =
          Printf.sprintf "timeout must be in the range (%g - %g), but got %g"
            Const.min_auto_dismiss_timeout_s Const.max_auto_dismiss_timeout_s x
        in
        failwith s
    ...
    method label_text : string =
      Js.Opt.map label_element##.textContent Js.to_string |> fun x ->
      Js.Opt.get x (fun () -> "")
    ...
    method action_button_text : string option =
      ...
      | None -> None
      | Some button ->
          Js.Opt.to_option @@ Js.Opt.map button##.textContent Js.to_string
    ...
    method private handle_surface_click (e : #Dom_html.event Js.t)
      ...
      Js.Opt.map e##.target (fun (elt : Dom_html.element Js.t) ->
          if Js.Opt.test @@ Element.closest elt Selector.action then
            self#handle_action_button_click ()
          else if Js.Opt.test @@ Element.closest elt Selector.dismiss then
            self#handle_action_icon_click ()
...
let attach ?auto_dismiss_timeout ?close_on_escape (elt : #Dom_html.element Js.t)
    : t =
  new t ?auto_dismiss_timeout ?close_on_escape (Element.coerce elt) ()

let make ?classes ?a ?leading ?stacked ?dismiss ?action ?actions ?label ?surface
    ?children ?auto_dismiss_timeout ?close_on_escape () =
  D.snackbar ?classes ?a ?leading ?stacked ?dismiss ?action ?actions ?label
    ?surface ?children ()
