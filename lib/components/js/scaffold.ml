...
type drawer_elevation = Full_height | Clipped
...
module Selector = struct
  ...
  let by_class (elt : Dom_html.element Js.t) (c : string) :
    ...
    match by_class_opt elt c with Some x -> x | None -> failwith (not_found c)
end
...
class t ?(drawer : #Drawer.t option)
  ...
  ?(top_app_bar : #Top_app_bar.t option) ?(body : #Dom_html.element Js.t option)
  ?on_navigation_icon_click (elt : Dom_html.element Js.t) () =
  let drawer_frame_full_height =
    Selector.by_class elt CSS.drawer_frame_full_height
  in
  ...
  object (self)
    ...
    val mutable drawer =
      match drawer with None -> attach_drawer elt () | Some x -> Some x
    ...
    method set_drawer
        : 'a. ?elevation:drawer_elevation ->
          ?breakpoints:Side_sheet.typ Breakpoint.t -> (#Drawer.t as 'a) -> unit
        =
    ...
    method set_side_sheet
        : 'a. ?elevation:drawer_elevation ->
          ?breakpoints:Side_sheet.typ Breakpoint.t -> (#Side_sheet.t as 'a) ->
          unit =
    ...
    method show_snackbar_await (snackbar : Snackbar.t)
      ...
      snackbar#open_await () >>= fun reason ->
    ...
    method private drawer_elevation_ (drawer : #Widget.t option)
      ...
      | Some (d : #Side_sheet.t) -> (
          ...
          | None -> None
          | Some parent ->
              if Element.has_class parent CSS.drawer_frame_full_height then
                Some Full_height
              else if Element.has_class parent CSS.drawer_frame_clipped then
                Some Clipped
              else None )
    (** Determines drawer or side sheet elevation *)

    method private render_drawer_ ~(is_leading : bool) (typ : Side_sheet.typ)
        (elevation : drawer_elevation) (drawer : #Side_sheet.Parent.t) : unit =
      let create_scrim, scrim_class =
        if is_leading then (Drawer.D.drawer_scrim, Drawer.CSS.scrim)
    ...
    method private set_drawer_properties_ ~(is_leading : bool)
        (typ : Side_sheet.typ) (elevation : drawer_elevation)
    ...
    method private handle_drawer_resize_ ~(is_leading : bool) (screen : int)
        (elevation : drawer_elevation) (drawer : #Side_sheet.Parent.t)
        : unit Lwt.t =
      let typ, breakpoints =
        if is_leading then (drawer_type, drawer_breakpoints)
        else (side_sheet_type, side_sheet_breakpoints)
      in
      let cur = Breakpoint.current screen breakpoints in
      if equal_drawer_type typ cur then Lwt.return_unit
      else
        Lwt.Infix.(
          drawer#toggle ~force:false () >>= fun () ->
    ...
    method private handle_content_loaded () =
      ...
      | Some drawer ->
          ...
          let elv =
            ...
            | Some x -> x
            | None -> (
                match self#drawer_elevation with None -> Clipped | Some x -> x )
          in
      ...
      listeners <-
        Js_of_ocaml_lwt.Lwt_js_events.
          [
            limited_onresizes ~elapsed_time:0.05 (fun _ _ ->
                self#handle_resize ());
          ];
      ( match leading with
      | None -> ()
    ...
    method private handle_resize () : unit Lwt.t =
      ...
      let side_sheet_lwt =
        ...
        | Some side_sheet, Some elevation ->
            self#handle_drawer_resize_ ~is_leading:false screen elevation
              side_sheet
        | _ -> Lwt.return_unit
      in
...
let make ?classes ?a ?drawer ?drawer_elevation ?drawer_breakpoints ?side_sheet
    ?side_sheet_elevation ?side_sheet_breakpoints
    ?(top_app_bar : #Top_app_bar.t option)
    ?(body : #Dom_html.element Js.t option) ?on_navigation_icon_click () =
  let elt =
    ...
    @@ D.(
         scaffold ?classes ?a
           ~children:
             [
               scaffold_drawer_frame ~full_height:true
                 ~children:
                   [
                     scaffold_app_content ~outer:true
                       ~children:
                         [
                           scaffold_drawer_frame ~clipped:true
                             ~children:
                               [
                                 scaffold_app_content ~inner:true ~children:[]
                                   ();
                               ]
                             ();
                         ]
  ...
  in
  new t
    ?drawer ?drawer_elevation ?drawer_breakpoints ?side_sheet
    ?side_sheet_elevation ?side_sheet_breakpoints ?top_app_bar ?body
    ?on_navigation_icon_click elt ()
