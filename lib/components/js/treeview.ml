...
let get_node_checked_state (item : Dom_html.element Js.t) =
  ...
  | Some x ->
      ...
      if Js.to_bool (Js.Unsafe.coerce checkbox)##.indeterminate then
        `Indeterminate
      else if Js.to_bool checkbox##.checked then `Checked
      else `Unchecked
  ...
  let rec find = function
    ...
    | i ->
        let item =
          Js.Opt.bind
            (list##item (i - 1))
            (fun e -> if f e then Js.some e else Js.null)
        in
...
let tree_node_of_event (items : Dom_html.element Dom.nodeList Js.t)
  ...
  @@ Js.Opt.bind e##.target (fun (target : Dom_html.element Js.t) ->
         ...
         Js.Opt.bind nearest_parent (fun (parent : Dom_html.element Js.t) ->
             if not @@ Element.matches parent ("." ^ CSS.node) then Js.null
...
let focus_next_node (active : Dom_html.element Js.t)
    (nodes : Dom_html.element Dom.nodeList Js.t) =
  ...
  let rec aux ?(found_active = false) = function
    ...
    | i -> (
        ...
        if (not found_active) && (not @@ Element.equal item active) then
          ...
          | None -> None
          | Some next ->
              if Tabbable.is_focusable next then (
                next##focus;
                Some next )
...
let focus_first_node (active : Dom_html.element Js.t)
    (nodes : Dom_html.element Dom.nodeList Js.t) =
  let rec aux i =
    match Js.Opt.to_option @@ nodes##item i with
    | None -> None
    | Some x ->
        if Element.equal active x then None
        else if Tabbable.is_focusable x then Some x
        else aux (succ i)
  in
  let first = aux 0 in
...
let focus_last_node (active : Dom_html.element Js.t)
    (nodes : Dom_html.element Dom.nodeList Js.t) =
  let rec aux i =
    match Js.Opt.to_option @@ nodes##item i with
    | None -> None
...
class t elt () =
  object (self)
    ...
    method! layout () : unit =
      ...
      Js.Opt.iter
        (nodes##item 0)
        (fun x -> Element.set_attribute x "tabindex" "0");
      super#layout ()
    ...
    method initialize_tree_type () : unit =
      ...
      if Js.Opt.test single_selected_node then (
        let item = Js.Opt.get single_selected_node (fun () -> assert false) in
        if Element.has_class item CSS.node_activated then
    ...
    method is_empty : bool =
      match Element.children super#root with [] -> true | _ -> false
    ...
    method selected_leafs : Dom_html.element Js.t list =
      ...
      @@ List.filter self#is_leaf selected_items
    ...
    method node_parent (node : Dom_html.element Js.t) =
      let rec aux node =
        Js.Opt.bind (Element.get_parent node) (fun parent ->
            if Element.has_class parent CSS.node then Js.some parent
            else aux parent)
      in
      aux node
    ...
    method node_children (node : Dom_html.element Js.t)
      ...
      let children_wrapper =
        node##querySelector (Js.string Selector.children)
      in
          ...
          | Some active ->
              let next, stop =
                ...
                | ArrowLeft, true | ArrowUp, false -> (
                    ...
                    if self#node_expanded active then (
                ...
                | ArrowRight, true | ArrowDown, false -> (
                    prevent_default_event e;
                    match
                      (self#node_expanded active, self#node_children active)
                    with
                    | _, [] -> (None, false)
                    | false, _ ->
                ...
                | (Enter as k), _ | (Space as k), _ ->
                    if Element.has_class item CSS.node then
    ...
    method private handle_action ?(is_checkbox = false)
        (node : Dom_html.element Js.t) =
      if self#is_leaf node || is_checkbox then
    ...
    method private get_node_checked_state (node : Dom_html.element Js.t)
      ...
      let rec aux ((checked, unchecked, indeterminate) as acc) = function
        | [] ->
            if (not indeterminate) && not unchecked then `Checked
            else if (not indeterminate) && not checked then `Unchecked
            else `Indeterminate
        | hd :: tl -> (
    ...
    method private update_children (checked : bool)
    ...
    method private toggle_checkbox ?(origin = false) ?checked ?(toggle = true)
        (node : Dom_html.element Js.t) : unit =
      ...
      | Some state ->
          ...
          if checked then (
            if not @@ List.exists (Element.equal node) selected_items then
              selected_items <- node :: selected_items )
          else
            selected_items <-
              List.filter (not % Element.equal node) selected_items
  end
