...
let children (elt : #Dom_html.element Js.t) : t list =
  List.filter_map (fun (x : Dom.node Js.t) ->
      match x##.nodeType with ELEMENT -> Some (Js.Unsafe.coerce x) | _ -> None)
...
let toggle_class ?(force : bool option) (elt : #Dom_html.element Js.t)
    (_class : string) : bool =
  match force with
...
let query_selector_all (elt : #Dom_html.element Js.t) (selector : string) :
    t list =
...
let get_attribute (elt : #Dom_html.element Js.t) (a : string) : string option =
  elt##getAttribute (Js.string a) |> Js.Opt.to_option |> function
  | None -> None
...
let set_attribute (elt : #Dom_html.element Js.t) (a : string) (v : string) :
    unit =
...
let set_style_property (elt : #Dom_html.element Js.t) (prop : string)
    (value : string) : unit =
  (Js.Unsafe.coerce elt##.style)##setProperty (Js.string prop) (Js.string value)

let get_parent (elt : #Dom_html.element Js.t) : t Js.opt =
  if Js.Optdef.test (Js.Unsafe.coerce elt)##.parentElement then
    (Js.Unsafe.coerce elt)##.parentElement
  else
...
let matches (e : #Dom_html.element Js.t) (selector : string) : bool =
  let native_matches =
    if Js.Optdef.test (Js.Unsafe.coerce e)##.matches then
      (Js.Unsafe.coerce e)##.matches
    else if Js.Optdef.test (Js.Unsafe.coerce e)##.webkitMatchesSelector then
      (Js.Unsafe.coerce e)##.webkitMatchesSelector
...
let closest (e : t) (selector : string) : t Js.opt =
  if Js.Optdef.test (Js.Unsafe.coerce e)##.closest then
    (Js.Unsafe.coerce e)##closest (Js.string selector)
  else
...
let is_focused (elt : #Dom_html.element Js.t) : bool =
  Js.Opt.map Dom_html.document##.activeElement (equal elt) |> fun x ->
  Js.Opt.get x (fun () -> false)
...
let emit ?(should_bubble = false) ?detail evt_type
    (element : Dom_html.element Js.t) =
  let event =
    Dom_html.createCustomEvent ?detail ~bubbles:should_bubble evt_type
  in
  element##dispatchEvent (event :> Dom_html.event Js.t)

let remove (x : #Dom.node Js.t) =
  if Js.Optdef.test (Js.Unsafe.coerce x)##.remove then
    (Js.Unsafe.coerce x)##remove
