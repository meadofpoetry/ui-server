...
let is_tab_event (e : Dom_html.keyboardEvent Js.t) : bool =
  let key =
    ...
    | Some x -> Some (Js.to_string x)
  in
  match (key, e##.keyCode) with Some "Tab", _ | _, 9 -> true | _ -> false
  ...
  | None -> (
      ...
      | None -> (
          ...
          | None -> (
              ...
              | None ->
                  failwith
                    "You can't have a focus-trap without at least one \
                     focusable element" ) ) )

let update_tabbable_nodes (t : t) : unit =
  let tabbable = Tabbable.get_tabbable t.container in
  let first_tabbable =
    match tabbable with [] -> get_initial_focus_node t | x :: _ -> x
  in
  let last_tabbable =
    match List.rev tabbable with [] -> get_initial_focus_node t | x :: _ -> x
  in
  t.state.first_tabbable <- Some first_tabbable;
  t.state.last_tabbable <- Some last_tabbable

let pause (t : t) : unit =
  if (not t.state.paused) && t.state.active then (
    t.state.paused <- true;
    remove_listeners t )
...
let deactivate_trap (x : t) : unit =
  ...
  match !queue with [] -> () | y :: _ -> y.unpause ()
...
let deactivate ?return_focus (t : t) : unit =
  if t.state.active then (
    ...
    deactivate_trap t;
    (match t.options.on_deactivate with None -> () | Some f -> f ());
    if return_focus then delay (fun () -> try_focus t t.state.focused_before) )

(** This needs to be done on mousedown and touchstart instead of click so that
    it precedes the focus event. *)
let check_pointer_down (t : t) (e : #Dom_html.event Js.t) =
  ...
  | target ->
      let return_focus =
        match target with None -> false | Some x -> Tabbable.is_focusable x
      in
      if t.options.click_outside_deactivates then deactivate ~return_focus t
...
let check_focus_in (t : t) (e : Dom_html.event Js.t) =
  ...
  if
    (not @@ contains t.container target)
...
(** Hijack Tab events on the first and last focusable nodes of the trap, in
    order to prevent focus from escaping. If it escapes for even a moment it can
    end up scrolling the page and causing confusion so we kind of need to
    capture the action at the keydown phase. *)
...
let check_key (t : t) (e : Dom_html.keyboardEvent Js.t) =
  if t.options.escape_deactivates && is_escape_event e then (
    Dom.preventDefault e;
    deactivate t )
  else if is_tab_event e then check_tab t e
...
let activate_trap (x : t) : unit =
  (match !queue with [] -> () | y :: _ -> if not @@ equal x y then y.pause ());
...
let unpause (t : t) : unit =
  if t.state.paused && t.state.active then (
    t.state.paused <- false;
    update_tabbable_nodes t;
    add_listeners t )
...
let activate (t : t) : unit =
  if not t.state.active then (
    ...
    (match t.options.on_activate with None -> () | Some f -> f ());
    add_listeners t )
...
let make ?(on_activate : (unit -> unit) option)
    ...
    ?(initial_focus : Dom_html.element Js.t option)
    ?(fallback_focus : Dom_html.element Js.t option)
    ?(escape_deactivates = true) ?(click_outside_deactivates = false)
    ?(return_focus_on_deactivate = true) (container : #Dom_html.element Js.t) :
    t =
  let options =
    {
      on_activate;
