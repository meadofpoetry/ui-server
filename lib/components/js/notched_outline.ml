...
class t (elt : Dom_html.element Js.t) () =
  object
    ...
    method! initial_sync_with_dom () : unit =
      ...
      | Some label ->
          (Js.Unsafe.coerce label##.style)##.transitionDuration
          := Js.string "0s";
...
let make ?classes ?a ?leading ?trailing ?notch ?label_for ?label ?children () :
    t =
  D.notched_outline ?classes ?a ?leading ?trailing ?notch ?label_for ?label
    ?children ()
