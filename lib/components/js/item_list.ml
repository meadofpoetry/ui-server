...
module Selector = struct
  ...
  let focusable_child_elements =
    Printf.sprintf
      ".%s button:not(:disabled), .%s a, .%s \
       input[type=\"radio\"]:not(:disabled), .%s \
...
module Lwt_js_events = struct
  ...
  let action ?use_capture ?passive t =
    make_event ?use_capture ?passive Event.action t
...
let list_item_of_event (items : Dom_html.element Dom.nodeList Js.t)
  ...
  @@ Js.Opt.bind e##.target (fun (target : Dom_html.element Js.t) ->
         ...
         Js.Opt.bind nearest_parent (fun (parent : Dom_html.element Js.t) ->
             if not @@ Element.matches parent ("." ^ CSS.item) then Js.null
...
let focus_prev_element ?(wrap = false) (active : Dom_html.element Js.t)
  ...
  let rec aux = function
    ...
    | i when i = 0 ->
        ...
        if not @@ Element.equal item active then aux (succ i)
        else if not wrap then None
        else
...
let focus_next_element ?(wrap = false) (active : Dom_html.element Js.t)
    (items : Dom_html.element Dom.nodeList Js.t) =
  let length = items##.length in
  let rec aux = function
    | i when i = length || length = 1 -> None
...
let has_radio_at_index (i : int) (items : Dom_html.element Dom.nodeList Js.t) :
    bool =
  Js.Opt.(
    test
    @@ bind
...
let has_checkbox_at_index (i : int) (items : Dom_html.element Dom.nodeList Js.t)
    : bool =
  Js.Opt.(
    test
    @@ bind
...
module Item = struct
  ...
  let make ?classes ?a ?graphic ?meta ?role ?tabindex ?activated ?selected
      ?checked ?primary_text ?secondary_text ?force_wrap ?children ?ripple () =
    D.list_item ?classes ?a ?graphic ?meta ?role ?tabindex ?activated ?selected
      ?checked ?primary_text ?secondary_text ?force_wrap ?children ()
...
class t (elt : Dom_html.element Js.t) () =
  object (self)
        ...
        Js_of_ocaml_lwt.Lwt_js_events.
          [
            ...
            seq_loop
              ...
              super#root self#handle_focus_in;
            seq_loop
              (make_event (Dom_html.Event.make "focusout"))
              super#root self#handle_focus_out;
          ]
        @ listeners;
    ...
    method! layout () : unit =
      ...
      loop_nodes (fun _ item -> Element.set_attribute item "tabindex" "-1")
      @@ super#root##querySelectorAll
           (Js.string Selector.focusable_child_elements);
      let items = self#items_ in
      match items##.length with
      | 0 -> ()
      | _ ->
          if has_checkbox_at_index 0 items then _is_checkbox_list <- true
          else if has_radio_at_index 0 items then _is_radio_list <- true
    ...
    method initialize_list_type () : unit =
      ...
      in
      if checkbox_list_items##.length > 0 then
      ...
      else if Js.Opt.test single_selected_list_item then (
        let item =
          Js.Opt.get single_selected_list_item (fun () -> assert false)
        in
        if Element.has_class item CSS.item_activated then
          self#set_use_activated true;
        self#set_single_selection true;
        _selected_items <- [ item ] )
      else if Js.Opt.test radio_selected_list_item then
        let item =
          Js.Opt.get radio_selected_list_item (fun () -> assert false)
        in
        _selected_items <- [ item ]
    ...
    method private set_selected (items : Dom_html.element Js.t list) : unit =
      self#layout ();
      if _is_checkbox_list then self#set_checkbox items
      else if _is_radio_list then (
        match items with
        | [] -> ()
        | [ x ] -> self#set_radio x
        | x :: _ ->
    ...
    method private set_aria_for_single_selection (item : Dom_html.element Js.t)
        : unit =
      ...
      | [] -> ()
      | l ->
          List.iter (fun e -> Element.set_attribute e aria_attribute "false") l
      );
      let value =
        match _aria_current_value with None -> "true" | Some x -> x
      in
      Element.set_attribute item aria_attribute value

    method private set_selected_item_on_action ?toggle
        (item : Dom_html.element Js.t) : unit =
      if _is_checkbox_list then self#toggle_checkbox ?toggle item
          ...
          | Some active ->
              let next, stop =
                ...
                | End, _ ->
                    prevent_default_event e;
                    let last =
                      Js.Opt.to_option (items##item (items##.length - 1))
                    in
                    Option.iter (fun x -> x##focus) last;
                    (last, false)
                | (Enter as k), _ | (Space as k), _ ->
                    if Element.has_class item CSS.item then
                      ...
                      in
                      if is_a_tag && is_enter then (None, true)
                      else (
                        prevent_default_event e;
                        if self#is_selectable_list then
                          self#set_selected_item_on_action active;
    ...
    method private handle_click (e : Dom_html.mouseEvent Js.t) (_ : unit Lwt.t)
        : unit Lwt.t =
      let items = self#items_ in
      Option.iter (fun item ->
          ...
          if self#is_selectable_list then
            self#set_selected_item_on_action ~toggle item;
    ...
    method private handle_focus_in (e : Dom_html.event Js.t) (_ : unit Lwt.t)
        : unit Lwt.t =
      Option.iter (set_tab_index_for_list_item_children 0)
      @@ list_item_of_event self#items_ e;
      Lwt.return_unit

    method private handle_focus_out (e : Dom_html.event Js.t) (_ : unit Lwt.t)
        : unit Lwt.t =
      let items = self#items_ in
      ...
      if not @@ Element.is_focus_inside super#root then
        set_tab_index_to_first_selected_item ~selected:_selected_items items;
      Lwt.return_unit
    ...
    method private toggle_checkbox ?(toggle = true)
      ...
      if checked then _selected_items <- add item _selected_items
      else
        _selected_items <-
          List.filter (fun x -> not @@ eq item x) _selected_items
  end

let attach (elt : #Dom_html.element Js.t) : t = new t (Element.coerce elt) ()

let make ?classes ?a ?avatar_list ?dense ?two_line ?non_interactive ?role
    ?children () =
  let two_line =
    ...
    | None, Some children ->
        ...
        @@ List.find_opt
             ...
             children
  in
  D.list ?classes ?a ?avatar_list ?dense ?two_line ?non_interactive ?role
    ?children ()
