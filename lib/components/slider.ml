...
object(self)
  ...
  method! destroy () : unit =
    ...
    Option.iter Lwt.cancel _keydown; _keydown <- None;
    Option.iter Lwt.cancel _mousedown; _mousedown <- None;
  ...
  method set_value (v : float) : unit =
    self#set_value_ ~fire_input:true v
  ...
  method private calculate_thumb_styles (percent : float) : unit =
    let func =
      if self#vertical then "translateY" else "translateX" in
    let value =
      if self#vertical || super#is_rtl ()
      then 100. -. percent else percent in
  ...
  method private calculate_track_styles
    ...
    then (
      ...
      let style =
        if self#vertical
        then Printf.sprintf "translateX(%s50%%) scaleY(%g)"
               (if super#is_rtl () then "" else "-") v
        else Printf.sprintf "translateY(-50%%) scaleX(%g)" v in
  ...
  method private handle_mouse_enter (e : Dom_html.mouseEvent Js.t) _
                 : unit Lwt.t =
    if (Js.Unsafe.coerce e)##.buttons = 0
    then self#handle_drag_end ();
    Lwt.return_unit
  ...
  method private handle_drag_end () : unit =
    ...
    Option.iter Lwt.cancel _mouseenter; _mouseenter <- None;
    Option.iter Lwt.cancel _mouseleave; _mouseleave <- None;
