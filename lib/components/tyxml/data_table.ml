...
module Make_fmt(Xml : Xml_sigs.NoWrap) = struct
  ...
  let rec of_string : type a. a t -> string -> a = fun fmt s ->
    match fmt with
    | Int -> int_of_string s
  ...
  let equal : type a. a t -> (a -> a -> bool) = fun t a b ->
    0 = compare t a b
end
...
module Make(Xml : Xml_sigs.NoWrap)
  ...
  let make_cell ?(classes = []) ?(attrs = []) ?colspan ?(numeric = false)
      content : 'a elt =
    let classes =
      classes
