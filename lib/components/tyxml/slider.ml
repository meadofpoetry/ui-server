...
let ( @? ) a b = match b with None -> a | Some b -> a @ [ b ]
...
struct
  ...
  let slider_thumb_container ?(classes = return []) ?(a = [])
      ?(discrete = false) ?pin ?(focus_ring = slider_focus_ring ())
      ?(thumb = slider_thumb ()) () =
  ...
  let slider_track_marker_container ?(classes = return []) ?(a = [])
      ?(children = nil ()) () =
    let classes = fmap (fun x -> CSS.track_marker_container :: x) classes in
    div ~a:(a_class classes :: a) children

  let slider_container ?(classes = return []) ?(a = []) ?(discrete = false)
      ?(markers = false) ?(track_before = slider_track_before ())
    ...
    let track_marker_container =
      ...
      | Some x -> Some (return x)
      | None ->
          if discrete && markers then
            Some (return @@ slider_track_marker_container ())
          else None
    in
    div ~a:(a_class classes :: a)
      ( return track_before
  ...
  let slider ?(classes = return []) ?(a = []) ?(discrete = false)
      ?(markers = false) ?(disabled = false) ?label ?step ?track_before
      ?track_after ?thumb_container ?track_marker_container
      ?(container =
        slider_container ~discrete ~markers ?track_before ?track_after
          ?thumb_container ?track_marker_container ()) ?(min = return 0.)
      ?(max = return 100.) ?(value = return 0.) () : 'a elt =
    let classes =
      fmap
    ...
    div
      ~a:
        ...
        |> Utils.map_cons_option
             (fun x -> a_aria "label" (fmap (fun x -> [ x ]) x))
             label
        |> Utils.(
             map_cons_option (a_user_data "step" % fmap string_of_float) step)
