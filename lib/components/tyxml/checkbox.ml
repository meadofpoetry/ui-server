module CSS = struct
  ...
  let anim_checked_indeterminate =
    BEM.add_modifier root "anim-checked-indeterminate"

  let anim_checked_unchecked = BEM.add_modifier root "anim-checked-unchecked"

  let anim_indeterminate_checked =
    BEM.add_modifier root "anim-indeterminate-checked"

  let anim_indeterminate_unchecked =
    BEM.add_modifier root "anim-indeterminate-unchecked"

  let anim_unchecked_checked = BEM.add_modifier root "anim-unchecked-checked"

  let anim_unchecked_indeterminate =
    BEM.add_modifier root "anim-unchecked-indeterminate"
end
...
struct
  ...
  let checkbox ?(classes = return []) ?(a = []) ?input_id ?(disabled = false)
      ?(checked = false) () =
    let classes =
    ...
    let input =
      input
        ~a:
          ( [
              a_input_type (return `Checkbox);
              a_class (return [ CSS.native_control ]);
            ]
