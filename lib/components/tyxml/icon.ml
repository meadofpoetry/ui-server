...
struct
  ...
  module SVG = struct
    let icon_path ?(classes = return []) ?(a = []) ?fill ~d ?(children = nil ())
      ...
      Svg.path
        ~a:
          ( Svg.a_class classes :: Svg.a_d d :: a
          |> Utils.map_cons_option Svg.a_fill fill )
        children

    let icon ?(classes = return []) ?(a = []) ?(size = return 24) ?fill ?d
        ?(children = nil ()) () =
      let sz = fmap (fun x -> (float_of_int x, None)) size in
      let viewbox =
        fmap (fun x -> (0., 0., float_of_int x, float_of_int x)) size
      in
      ...
      svg
        ~a:
          ...
          :: a )
        (match path with None -> children | Some x -> cons x children)
  end
end
