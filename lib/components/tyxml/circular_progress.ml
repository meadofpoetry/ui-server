...
struct
  ...
  let circular_progress ?(classes = return []) ?(a = []) ?(min = return 0.)
      ?(max = return 1.) ?(value = return 0.) ?(indeterminate = true)
      ?(thickness = return 3.6) ?(size = return 40) () : 'a elt =
    let open Xml.W in
    let classes =
      fmap
        (Utils.cons_if indeterminate CSS.indeterminate % List.cons CSS.root)
        classes
    in
    let style =
      fmap (fun x -> Printf.sprintf "width: %dpx; height: %dpx" x x) size
    in
    let circle =
      return
      @@ Svg.circle
    ...
    let svg =
      return
      @@ svg ~a:[ Svg.a_viewBox (return (0., 0., sz, sz)) ] (singleton circle)
    in
    div
      ~a:
        ( a_class classes
