module CSS = struct
  ...
  (** Class used to style the content below the standard and fixed top app bar
  ...
  (** Class used to style the content below the prominent top app bar to prevent
      the top app bar from covering it. *)
  ...
  (** Class used to style the content below the dense top app bar to prevent the
      top app bar from covering it. *)
  let dense_fixed_adjust = BEM.add_modifier root "dense-fixed-adjust"

  (** Class used to style the content below the top app bar when styled as both
      prominent and dense, to prevent the top app bar from covering it. *)
  ...
  (** Class used to style the content below the short top app bar to prevent the
      top app bar from covering it. *)
...
struct
  ...
  let top_app_bar_title ?(classes = return []) ?(a = []) ?title
      ?(children = nil ()) () =
  ...
  let top_app_bar_section ?(classes = return []) ?(a = []) ?align
      ?(children = nil ()) () =
    let align_class =
      match align with
      | None -> None
    ...
    in
    let classes =
      fmap (Utils.cons_option align_class % List.cons CSS.section) classes
    in
  ...
  let top_app_bar ?(classes = return []) ?(a = []) ?leading ?title ?actions
      ?children () =
    ...
    let children =
      ...
      | None ->
          ...
          let start_section =
            return
              (top_app_bar_section ~align:`Start
          ...
          let end_section =
            ...
            | None -> None
            | Some x ->
                Some (return (top_app_bar_section ~align:`End ~children:x ()))
          in
