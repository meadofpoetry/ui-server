...
struct
  ...
  let treeview_node_content ?(classes = return []) ?(a = []) ?graphic ?meta
      ?role ?primary_text ?secondary_text ?force_wrap ?children () =
    let classes =
      fmap (fun x -> Item_list.CSS.item :: CSS.node_content :: x) classes
    in
    let children =
      match children with
      | Some x -> x
      | None ->
          let text =
            Item_list_markup.list_item_text ?force_wrap ?primary_text
              ?secondary_text ()
          in
  ...
  let treeview_node ?(classes = return []) ?(a = []) ?value ?level
      ?(expanded = false) ?(tabindex = -1) ?(selected = false)
      ?(checked = false) ?(indeterminate = false) ?graphic ?meta ?role
      ?primary_text ?secondary_text ?force_wrap ?node_content ?child_nodes
      ?children_wrapper () : 'a elt =
    let node_content =
      ...
      | None ->
          ...
          in
          return
            (treeview_node_content ?graphic ?meta ?role ?primary_text
               ?secondary_text ?force_wrap ())
    in
    let checked =
    ...
    let children_wrapper =
      ...
      | None -> (
          ...
          | None -> None
          | Some nodes ->
              Some (return (treeview_children_wrapper ~children:nodes ())) )
    in
  ...
  let treeview ?(classes = return []) ?(a = []) ?multiselectable
      ?(dense = false) ?(two_line = false) ?(children = nil ()) () =
    let classes =
      fmap
