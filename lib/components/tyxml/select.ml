module CSS = struct
  ...
  (** Mandatory when using the enhanced select. This class should be placed on
      the mdc-menu element within the mdc-select element. *)
  let menu = BEM.add_element root "menu"

  (** Mandatory. Should be placed on an i element within the mdc-select element.
      Used for the dropdown arrow svg and animation. *)
  ...
  (** Optional. Styles the select as disabled. This class should be applied to
      the root element when the disabled attribute is applied to the <select>
      element. *)
...
struct
  ...
  module Native = struct
    let option ?(classes = return []) ?(a = []) ?value ?label
        ?(disabled = false) ?(selected = false) ~text () =
      option
        ~a:
          ( a_class classes :: a
    ...
    let optgroup ?(classes = return []) ?(a = []) ~label ?(disabled = false)
        ?(options = nil ()) () =
      optgroup
        ~a:(a_class classes :: a |> Utils.cons_if_lazy disabled a_disabled)
        ~label options

    let native_control ?(classes = return []) ?(a = []) ?id ?(disabled = false)
        ?(autofocus = false) ?(required = false) ?size
        ?(* Number of visible options in a drop-down list *) form
        ?(* Form the select field belongs to *) name
        ?((* Name of the drop-down list *) options = nil ()) () =
    ...
    let select ?(classes = return []) ?(a = []) ?label ?input_id ?line_ripple
        ?(disabled = false) ?(outlined = false) ?outline ?icon ?required
        ?autofocus ?size ?form ?name ?options
        ?(native_control =
          native_control ?id:input_id ?required ?autofocus ?size ?form ?name
            ?options ()) () : 'a elt =
      let outline =
      ...
      let floating_label =
        ...
        | Some x, None ->
            Some
              ( return
              @@ Floating_label_makrup.floating_label ?for_:input_id ~label:x ()
              )
      in
      let classes =
        fmap
      ...
      in
      let dropdown_icon =
        i ~a:[ a_class (return [ CSS.dropdown_icon ]) ] (nil ())
      in
  ...
  module Enhanced = struct
    let hidden_input ?(classes = return []) ?(a = []) ?(disabled = false) () :
        'a elt =
      input
        ~a:
    ...
    let select ?(classes = return []) ?(a = []) ?label ?line_ripple
        ?(disabled = false) ?(selected_text = return "") ?outline ?icon
        ?hidden_input ~menu () : 'a elt =
      let with_leading_icon =
        match icon with None -> false | Some _ -> true
      in
      let outlined = match outline with None -> false | Some _ -> true in
      let classes =
        fmap
