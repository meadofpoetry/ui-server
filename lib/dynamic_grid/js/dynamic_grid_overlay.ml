...
class overlay_grid ~parent ~s_col_w ~s_row_h ~s_cols ~s_rows ~s_im () =
  ...
  object(self)
    ...
    method show () : unit =
      Element.append_child parent super#root;
      Element.add_class parent Dynamic_grid_tyxml.CSS.with_overlay_grid

    method hide () : unit =
      Element.remove_child_safe parent super#root;
      Element.remove_class parent Dynamic_grid_tyxml.CSS.with_overlay_grid
