...
let listen ?save target typ f =
  ...
  Utils.Option.iter (fun f -> f @@ Some listener) save

let stop_listen = Utils.Option.iter Dom_events.stop_listen
...
class ['a] t ~s_grid (* grid props *)
  ...
  object(self)
    ...
    val resize_button =
      ...
      icon#add_class Dynamic_grid_tyxml.CSS.item_resize_handle;
      icon
    ...
    method set_selected (x : bool) : unit =
      ...
      | true ->
        ...
        self#add_class Dynamic_grid_tyxml.CSS.item_selected;
        selected <- true
      | false ->
        if self#selected
        then (super#remove_class Dynamic_grid_tyxml.CSS.item_selected;
              selected <- false;
              s_selected_push
    ...
    method private _set_draggable x =
      ...
      self#get_drag_target#toggle_class ~force:x
        Dynamic_grid_tyxml.CSS.item_drag_handle
    ...
    method private start_dragging (ev : action) =
      ...
      if self#draggable
      then
        (self#get_drag_target#add_class Dynamic_grid_tyxml.CSS.item_dragging;
         let ghost = self#make_ghost () in
         ghost#root##.style##.zIndex := Js.string "1";
         super#root##.style##.zIndex := Js.string "3";
    ...
    method private start_resizing (ev : action) =
      ...
      then (let ghost = self#make_ghost () in
            ghost#root##.style##.zIndex := Js.string "4";
            super#root##.style##.zIndex := Js.string "3";
            Option.iter (fun x -> Dom.appendChild x ghost#root)
            @@ Js.Opt.to_option @@ Element.get_parent super#root;
