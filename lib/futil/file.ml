...
let create_single_dir mode d =
  ...
  | Unix.Unix_error (Unix.EACCES, _, _) -> Error `Access_denied
  | Unix.Unix_error (Unix.ENOSPC, _, _) -> Error `No_space
  | _ -> Error `Unspecified

let create_file mode f =
  try
    Unix.openfile f [ Unix.O_CREAT ] mode |> Unix.close;
    Ok ()
  with
  | Unix.Unix_error (Unix.EEXIST, _, _) -> Error `File_already_exists
  | Unix.Unix_error (Unix.EACCES, _, _) -> Error `Access_denied
  | Unix.Unix_error (Unix.ENOSPC, _, _) -> Error `No_space
  | _ -> Error `Unspecified
...
let create_rec ?(dir = false) ?(dmode = default_dir_mode)
  ...
  let rec create' base = function
    ...
    | [ x ] when not dir ->
        ...
        if exists path then Ok () else create_file fmode path
    | h :: tl ->
        let path = Filename.concat base h in
        if exists path then create' path tl
        else create_single_dir dmode path >>= fun _ -> create' path tl
  in
...
let create_dir ?(dmode = default_dir_mode) path =
  ...
  | Error _ as e -> e
  | Ok path' ->
      if not (Info.exists path) then
        create_rec ~dir:true ~dmode (Path.to_string path')
      else Ok ()
...
let create ?(fmode = default_mode) ?(dmode = default_dir_mode) path =
  ...
  | Ok path' ->
      if not (Info.exists path) then
        create_rec ~dir:false ~fmode ~dmode (Path.to_string path')
      else Ok ()
...
let write ?(create = true) ?(fmode = default_mode) ?(dmode = default_dir_mode)
  ...
  | Ok path' -> (
      ...
      ( if create && not (Info.exists path) then create_rec ~fmode ~dmode path_s
      else Ok () )
      >>= fun () ->
      try
...
let read path =
  ...
  | Ok path' -> (
      let path_s = Path.to_string path' in
      if not (Info.exists path) then Error `No_such_file
      else
        try
          let ic = open_in_bin path_s in
          let buf = ref (Bytes.create page_size) in
          let len = ref 0 in
          finalizer ic ~fin:close_in (fun ic ->
              try
                while true do
                  (* resize *)
        ...
        with
        | Sys_error e -> Error (`Reading_error e)
