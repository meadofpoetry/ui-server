...
class transaction obj =
  object
    ...
    method cancel =
      OBus_method.call
        PK_trans_iface.Org_freedesktop_PackageKit_Transaction.m_Cancel obj ()

    method get_updates flags =
      OBus_method.call
        PK_trans_iface.Org_freedesktop_PackageKit_Transaction.m_GetUpdates obj
        flags
    ...
    method finished =
      OBus_signal.make
        PK_trans_iface.Org_freedesktop_PackageKit_Transaction.s_Finished obj
      |> OBus_signal.connect

    method error_code =
      OBus_signal.make
        PK_trans_iface.Org_freedesktop_PackageKit_Transaction.s_ErrorCode obj
      |> OBus_signal.connect

    method percentage =
      OBus_property.make
        PK_trans_iface.Org_freedesktop_PackageKit_Transaction.p_Percentage obj
      |> OBus_property.monitor

    method status =
      OBus_property.make
        PK_trans_iface.Org_freedesktop_PackageKit_Transaction.p_Status obj
      |> OBus_property.monitor
  end
...
class t obj =
  object
    ...
    method create_transaction =
      let* path =
        OBus_method.call PK_iface.Org_freedesktop_PackageKit.m_CreateTransaction
          obj ()
      in
      let trans = OBus_proxy.make ~peer:obj.OBus_proxy.peer ~path in
      Lwt.return (new transaction trans)

    method get_transaction_list =
      OBus_method.call PK_iface.Org_freedesktop_PackageKit.m_GetTransactionList
        obj ()
    ...
    method transaction_list_changed =
      OBus_signal.make
        PK_iface.Org_freedesktop_PackageKit.s_TransactionListChanged obj
      |> OBus_signal.connect
