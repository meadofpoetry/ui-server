...
let of_yojson (j : Yojson.Safe.t) : (t, string) result =
  let to_err j =
    Printf.sprintf "of_yojson: bad json value (%s)" @@ Yojson.Safe.to_string j
  in
  match j with
  | `String s -> (
  ...
  let sep_days ~sz (fd, fps) (td, tps) =
    ...
    let rec loop acc d =
      if d > td - sz then ((d, 0L), (td, tps)) :: acc
        ...
        let nd = d + sz in
        loop (((d, 0L), (nd, 0L)) :: acc) nd
    in
    let d, dps = (fd + 1, 0L) in
  ...
  let sep_seconds ~sz from til =
    ...
    let rec loop acc st =
      if compare st (sub_span til sz |> Option.get) > 0 then (st, til) :: acc
        ...
        let nst = Option.get @@ add_span st sz in
        loop ((st, nst) :: acc) nst
    in
    let s =
      Option.get @@ add_span (truncate ~frac_s:0 from) (Span.of_int_s 1)
    in
...
struct
  ...
  let of_int x =
    Option.get @@ Ptime.of_span @@ Option.get @@ Ptime.Span.of_d_ps (M.of_int x)

  let to_int x = M.to_int @@ Ptime.Span.to_d_ps @@ Ptime.to_span x
...
struct
  let () =
    if Int64.compare M.second ps_in_s > 0 then
      failwith "Time.Span.Conv64: second precision is more than 1ps"

  type t = Ptime.t

  let of_int64 (x : int64) : t =
    let s = Int64.to_float x /. Int64.to_float M.second in
    try Option.get @@ Ptime.of_span @@ Option.get @@ Ptime.Span.of_float_s s
    with e ->
...
module Seconds64 = Conv64 (struct
  let second = 1L
end)

module Useconds = Conv64 (struct
  let second = 1000_000L
end)
...
module Period = struct
  ...
  let of_yojson (j : Yojson.Safe.t) : (t, string) result =
    let to_err j =
      Printf.sprintf "span_of_yojson: bad json value (%s)"
