...
module Make (Body : Api.BODY) (Msg : Api.WS_BODY with type t := Body.t) = struct
  ...
  let request (type a) ?(timeout = 2.) socket (req : a req) =
    ...
    let rec loop () =
      Lwt_stream.next socket.control >>= fun resp ->
      match (req, resp) with
    ...
    Lwt.pick
      [
        ...
        ( Lwt_js.sleep timeout >>= fun () ->
          Lwt.return_error (`Msg "Request timed out") );
      ]
  ...
  let subscribe ?(reqid = Random.bits ()) ~path ~query =
    ...
    in
    make_uri ?secure:None ?host:None ?port:None ~insert_defaults:false ~f ~path
      ~query
  ...
  let open_socket :
   ...
   fun ?secure ?host ?port ~path ->
    let f uri () : (t, error) Lwt_result.t =
      ...
      | _ -> assert false
    in
    make_uri ?secure ?host ?port ~path ~insert_defaults:true
      ~query:Uri.Query.empty ~f:(fun uri ->
