...
module Make
    (User : Api.USER)
    (Body : Api.BODY)
    (Msg : Api.WS_BODY with type t = Body.t) =
struct
  open Netlib
  ...
  let transform not_allowed f user =
    if not_allowed user then Lwt.fail_with "not allowed" else f user
  ...
  let merge ?prefix handlers =
    match prefix with
    | None -> Uri.Dispatcher.concat handlers
    | Some prefix ->
        assert (String.length prefix <> 0);
  ...
  let find_free_connection () =
    let rec loop ind =
      if ind >= Array.length connections then
        failwith "too much socket connections"
      else
        match connections.(ind) with None -> ind | Some _ -> loop (succ ind)
    in
    loop 0

  let store_connection ind data = connections.(ind) <- Some data

  let free_connection ind = connections.(ind) <- None

  let conn_mutex = Lwt_mutex.create ()

  let open_connection (events : t) (ping : unit React.event) user _body _env
      state =
    Lwt_mutex.with_lock conn_mutex (fun () ->
        ...
        let unsubscribe (subid : int) =
          ...
          | Some (n, e) -> (
              ...
              | 0 ->
                  React.E.stop e;
                  Hashtbl.remove subscriptions subid;
                  Ok ()
              | _ ->
                  decr n;
                  Ok () )
        ...
        let parse_frame s =
          match Body.of_string s with Error _ -> None | Ok v -> Msg.parse v
        in

        Websocket_cohttp_lwt.upgrade_connection (fst state) (fun frame ->
            match frame.opcode with
            | Frame.Opcode.Close -> free_connection conn_id
            | Frame.Opcode.Ping ->
                let frame =
                  Frame.create ~opcode:Pong ~content:frame.content ()
                in
            ...
            | Frame.Opcode.Text -> (
                ...
                | Some (reqid, `Unsubscribe id) -> (
                    ...
                    | Error e ->
                        push_msg @@ `Body (Msg.compose reqid (`Error e))
                    | Ok () ->
        ...
        let event_stream =
          ...
          @@ React.E.select
               [
                 msg_stream;
                 React.E.map
                   (fun () -> `Frame (Frame.create ~opcode:Ping ()))
                   ping;
               ]
        in
  ...
  let to_http ?doc ~prefix ~ping nodes =
    ...
    Api_http.make
      [
        Api_http.node_raw ?doc ~meth:`GET
          ~path:Uri.Path.Format.(prefix @/ empty)
          ~query:Uri.Query.empty make_conn;
      ]
end
