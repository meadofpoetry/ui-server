...
let env_of_headers headers : env =
  let env : type a. a Api.key -> a option = function
    | Api.Key k -> Cohttp.Header.get headers k
    | Api.Auth -> (
...
let to_resp_action resp = Lwt.return (`Response resp)

let respond_need_auth ?headers ~auth () =
  let open Lwt.Infix in
  Cohttp_lwt_unix.Server.respond_need_auth ?headers ~auth () >>= to_resp_action

let respond_string ?(status = `OK) ?headers body () =
  let open Lwt.Infix in
  Cohttp_lwt_unix.Server.respond_string ~status ?headers ~body () >>= to_resp_action

let respond_error ?(status = `Forbidden) error () =
  let open Lwt.Infix in
  Cohttp_lwt_unix.Server.respond_error ~status ~body:error () >>= to_resp_action

let respond_redirect uri () =
  let open Lwt.Infix in
  Cohttp_lwt_unix.Server.respond_redirect ~uri () >>= to_resp_action
...
module Make (User : Api.USER) (Body : Api.BODY) : sig
  ...
  val make : ?prefix:string -> node list -> t
  ...
  val node :
    ...
    -> query:('b, user -> body -> env -> state -> answer Lwt.t) Netlib.Uri.Query.format
    -> 'a
    -> node
  ...
  val node_raw :
    ...
    -> meth:meth
    -> path:('a, 'b) Netlib.Uri.Path.Format.t
    -> query:('b, user -> string -> env -> state -> answer Lwt.t) Netlib.Uri.Query.format
    -> 'a
    -> node
  ...
  type 'a handler = Cohttp.Code.meth * 'a Netlib.Uri.Dispatcher.node

  type node = (user -> string -> env -> state -> answer Lwt.t) handler

  type t =
    (user -> string -> env -> state -> answer Lwt.t) Netlib.Uri.Dispatcher.t Meth_map.t
  ...
  let handle (tbl : t) ~state ?(meth = `GET) ?forbidden ?default ~env ~redir uri body =
    ...
    let forbidden =
      ...
      | Some v -> fun (user : user) -> v user
    in
    let default =
      match default with
      | None -> fun (_user : user) _body _env _state -> Lwt.return (`Error "Not_found")
      | Some v -> fun (user : user) _body _env _state -> Lwt.return (`Instant (v user))
    in
    redir env
    >>= function
  ...
  let make ?prefix nodes =
    let add_node map (meth, node) =
      ...
      in
      let update = function
        | None -> Some Uri.Dispatcher.(add empty node)
        | Some disp -> Some Uri.Dispatcher.(add disp node)
      in
      Meth_map.update meth update map
    in
    nodes |> List.fold_left add_node Meth_map.empty
  ...
  let doc (t : t) =
    List.map (fun (meth, v) -> Cohttp.Code.string_of_method meth, Uri.Dispatcher.doc v)
    @@ Meth_map.bindings t
end
