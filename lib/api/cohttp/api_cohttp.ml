...
module Make (User : Api.USER) (Body : Api.BODY) : sig
  ...
  val node :
    ...
    query:
      ('b, user -> body -> env -> state -> answer Lwt.t) Netlib.Uri.Query.format ->
    'a ->
    node
  ...
  type t =
    (user -> string -> env -> state -> answer Lwt.t) Netlib.Uri.Dispatcher.t
    Meth_map.t

  let handle (tbl : t) ~state ?(meth = `GET) ?forbidden ?default ~env ~redir uri
      body =
    let open Lwt.Infix in
    let forbidden =
    ...
    let default =
      match default with
      | None ->
          fun (_user : user) _body _env _state ->
            Lwt.return (`Error "Not_found")
      | Some v ->
          fun (user : user) _body _env _state -> Lwt.return (`Instant (v user))
    in
    redir env >>= function
    ...
    | Ok user -> (
        ...
        | `Value body ->
            respond_string
              ~headers:
                (Cohttp.Header.of_list [ ("Content-Type", Body.content_type) ])
              (Body.to_string body) ()
        | `Unit -> respond_string "" ()
  ...
  let transform not_allowed f : user -> string -> env -> state -> answer Lwt.t =
   fun user body env state ->
    ...
    | Ok body ->
        if not_allowed user then Lwt.return (`Error "access denied")
  ...
  let doc (t : t) =
    List.map (fun (meth, v) ->
        (Cohttp.Code.string_of_method meth, Uri.Dispatcher.doc v))
    @@ Meth_map.bindings t
end
