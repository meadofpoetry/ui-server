...
type load_error =
  | Add_error of Dynamic_grid.Dynamic_grid_abstract.add_error
...
class ['a] t ?(edit_caps = Absolute)
   ...
   * let add = Fab.make ~icon:add_icon () in *)
  (* let fab =
   ...
   *     ~items:[add]
   *     () in *)
  let fab = Widget.create_div () in
  let e, push = React.E.create () in
  let widgets = [grid#widget; fab#widget] in
  let elt = Box.make_element ~dir:`Column widgets in
  object(self)
