...
let properties_changed interface =
  ...
  OBus_member.Signal.make ~interface ~member:"PropertiesChanged"
...
let monitor proxy interface switch =
  ...
  >>= fun event ->
  OBus_property.get_all_no_cache proxy interface >>= fun (context, dict) ->
  Lwt.return
    (S.fold_s ~eq:(String_map.equal ( == ))
       (fun (map : (OBus_context.t * OBus_value.V.single) String_map.t)
            (context, updates) ->
...
module Nm = struct
  module Config = struct
    ...
    let unwrap_int = function
      | OBus_value.V.Basic (OBus_value.V.Int32 i) -> Some (Int32.to_int i)
      | _ -> None

    let unwrap_bool = function
      | OBus_value.V.Basic (OBus_value.V.Boolean b) -> Some b
      | _ -> None

    let unwrap_address_list v =
      let unwrap' = function
        | OBus_value.V.Array (OBus_value.T.Basic OBus_value.T.Uint32, ilst) -> (
            match ilst with
            | OBus_value.V.Basic (OBus_value.V.Uint32 adds)
              :: OBus_value.V.Basic (OBus_value.V.Uint32 mask) :: _ ->
      ...
      match v with
      | OBus_value.V.Array
          (OBus_value.T.Array (OBus_value.T.Basic OBus_value.T.Uint32), ars) ->
    ...
    let unwrap_ip_list = function
      | OBus_value.V.Array (OBus_value.T.Basic OBus_value.T.Uint32, lst) ->
          Some
            (List.filter_map
               (function
                 | OBus_value.V.Basic (OBus_value.V.Uint32 i) ->
                     Some (Ipaddr.V4.of_int32 @@ reverse_int32 i)
                 | _ -> None)
               lst)
      | _ -> None
    ...
    let unwrap_autoconnect (flag, prior) =
      match (flag --> unwrap_bool, prior --> unwrap_int) with
    ...
    let of_dbus d =
      ...
      let ( <+> ) else_ a = match a with None -> else_ | Some _ -> a in

      let result_to_opt = function Ok x -> Some x | Error _ -> None in

      let of_eth opts =
        opts.%{"mac-address"} --> unwrap_bytes >>= fun x ->
        result_to_opt @@ Macaddr.of_octets @@ Bytes.to_string x
        >>= fun mac_address -> Some { mac_address }
      in
      let of_conn opts =
        opts.%{"id"} --> unwrap_string >>= fun id ->
        opts.%{"uuid"} --> unwrap_string >>= fun uuid ->
        (opts.%{"autoconnect"}, opts.%{"autoconnect-priority"})
        |> unwrap_autoconnect
        <+> Some (True 0)
        >>= fun autoconnect -> Some { autoconnect; id; uuid }
      in

      let of_ipv4 opts =
        let gateway =
          opts.%{"gateway"} --> unwrap_string >>= fun x ->
          result_to_opt @@ Ipaddr.V4.of_string x
        in
        let static =
          opts.%{"routes"} --> unwrap_address_list |> function
          | Some l -> l
          | None -> []
        in
        let routes = { gateway; static } in
        opts.%{"addresses"} --> unwrap_address_list >>= function
        | [] -> None
        | hd :: _ ->
            Some hd >>= fun address ->
            opts.%{"dns"} --> unwrap_ip_list >>= fun dns ->
            opts.%{"method"} --> unwrap_string >>= meth_of_string
            >>= fun meth -> Some { address; routes; dns; meth }
      in

      List.fold_left
        (fun (eth, conn, ipv4) (name, opts) ->
          match name with
        ...
        (None, None, None) d
      |> function
      | Some ethernet, Some connection, Some ipv4 ->
          Some { ethernet; connection; ipv4; ipv6 = (); proxy = () }
      | _ -> None
    ...
    let wrap_dns_list x =
      List.map
        (fun dns ->
          OBus_value.V.basic_uint32 @@ reverse_int32 @@ Ipaddr.V4.to_int32 dns)
        x
      ...
      List.map
        (fun (addr, mask) ->
          V.dict T.String T.Variant
            [
              ( V.string "address",
                V.variant @@ V.basic_string @@ Ipaddr.V4.to_string addr );
    ...
    let to_dbus c =
      let eth =
        [
          ( "mac-address",
            wrap_bytes
            @@ Bytes.of_string
            @@ Macaddr.to_octets c.ethernet.mac_address );
        ]
      in
      let conn =
        [
          ("id", wrap_string c.connection.id);
      ...
      let ipv4 =
        ...
        let routes =
          match c.ipv4.routes.gateway with
        ...
        [ ("address-data", address_data); ("dns", dns); ("method", meth) ]
        @
        match routes with
  ...
  module Device = struct
    ...
    let make bus path =
      OBus_proxy.make
        ~peer:
          (OBus_peer.make ~connection:bus
             ~name:"org.freedesktop.NetworkManager")
        ~path

    let type_ethernet = 1l

    let type_prop proxy = OBus_property.make ~monitor p_DeviceType proxy

    let interface_prop proxy = OBus_property.make ~monitor p_Interface proxy

    let connection_prop proxy =
      OBus_property.make ~monitor p_ActiveConnection proxy

    let get_applied_connection proxy () =
      OBus_method.call m_GetAppliedConnection proxy 0l >>= fun (set, _) ->
      Lwt.return set

    let reapply proxy s v = OBus_method.call m_Reapply proxy (s, v, 0l)
  end

  module Connection = struct
    include OBus_proxy.Private

    open Nm_connection_interfaces
    ...
    let settings_prop proxy = OBus_property.make ~monitor p_Connection proxy
  end

  module Settings = struct
    include OBus_proxy.Private

    open Nm_settings_interfaces
    ...
    let get_settings proxy = OBus_method.call m_GetSettings proxy

    let update proxy s = OBus_method.call m_Update proxy s

    let save proxy () = OBus_method.call m_Save proxy ()
  end
  ...
  let devices_prop proxy = OBus_property.make ~monitor p_AllDevices proxy
  ...
  let activate_connection proxy (dev : Device.t) (settings : Settings.t) =
    OBus_method.call m_ActivateConnection proxy
      (OBus_proxy.path settings, OBus_proxy.path dev, OBus_path.empty)
    >>= fun _ -> Lwt.return_unit

  let add_and_activate proxy = OBus_method.call m_AddAndActivateConnection proxy
end
       ...
       (fun acc (n, v) ->
         Printf.sprintf "%s\n%s: { %s }" acc n
           (List.fold_left
              (fun acc (n, v) ->
                Printf.sprintf "%s\n\t%s: %s" acc n
                  (OBus_value.V.string_of_single v))
              "" v))
       "" conf)
  |> ignore
...
type eth_conn_error = [ `Nm_no_connection of string | `Nm_no_settings of string ]
...
class virtual eth_connection =
  object
    method virtual get_config
        : 'a. unit ->
          (Network_config.t, ([> get_conf_error ] as 'a)) Lwt_result.t
...
let eth_connection ?defaults bus nm name device :
  ...
  OBus_property.get @@ Nm.Device.connection_prop device >>= fun conn_path ->
  ( if OBus_path.compare OBus_path.empty conn_path <> 0 then Lwt.return conn_path
  else
    let defaults =
      match defaults with
    ...
    in
    Nm.Device.reapply device defaults 1L >>= fun () ->
  ...
  Lwt.return_ok
    (object
       ...
       method get_config () =
         Nm.Device.get_applied_connection device () >>= fun conf ->
       ...
       method apply new_sets =
         Lwt.catch
           (fun () ->
             ...
             >>= fun () ->
             Nm.activate_connection nm device settings >>= Lwt.return_ok)
...
let create (kv : Kv.RW.t) =
  ...
  and internal_options = [ "pc"; "network"; "internal" ] in
  Kv.RW.parse Network_settings.of_string kv settings_path >>=? fun config ->
  OBus_bus.system () >>= fun bus ->
  let nm_proxy = Nm.make bus in

  OBus_property.get @@ Nm.devices_prop nm_proxy >>= fun device_paths ->
       ...
       try
         List.map
           (fun proxy ->
             ...
             >>= fun iface -> Lwt.return (typ, iface, proxy))
           x
       with _ -> [])
  ...
  ( match
      List.find_opt
        ...
        devices
    with
  | None -> Lwt.return_error (`No_network_device "internal")
  ...
  | Ok v ->
      v#get_config () >>=? fun applied ->
      let default =
        match config.extern with
        | None -> applied
        | Some c -> (
            match Network_settings.apply applied c with
            | None -> applied
            | Some x -> x )
      in
      Net_options.create ~default kv internal_options >>=? fun options ->
      Lwt.return_ok (Some (v, options)) )
  >>=? fun exterior ->
  (* push settings *)
  ( interior#get_config () >>=? fun conf ->
    ...
    | Some new_conf ->
        ...
        in
        interior#apply new_conf )
  >>=? fun () ->
  (* external iface settings *)
  ( match exterior with
  | None -> Lwt.return_ok ()
  | Some (exterior, external_opts) ->
      exterior#get_config () >>=? fun conf ->
      external_opts#get >>= fun old_conf ->
      if Network_config.equal conf old_conf then Lwt.return_ok ()
      else exterior#apply old_conf )
  >>=? fun () -> Lwt.return_ok { intern = interior; extern = exterior }

let get_ext_settings net =
  match net.extern with
  | None -> Lwt.return_error (`No_network_device "external")
  | Some (_ext, ext_opts) -> ext_opts#get >>= Lwt.return_ok
...
let apply_ext_settings (net : t) sets =
  ...
  | None -> Lwt.return_error (`No_network_device "external")
  | Some (ext, ext_opts) ->
      ext_opts#get >>= fun opts ->
      if Network_config.equal sets opts then Lwt.return_ok ()
