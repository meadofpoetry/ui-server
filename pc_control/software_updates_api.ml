...
let is_upgraded () =
  match S.value state with `Need_reboot -> true | _ -> false
...
let status_signal f trans =
  ...
  S.l2 ~eq:Unit.equal f status perc |> Lwt.return

let package_processor trans =
  let ( let* ) = Lwt.bind in
  let* package = trans#package in
  E.map (fun (_, pkg, _) -> Stack.push pkg !packages) package |> Lwt.return
...
let ui_server_version list =
  ...
  let starts_with pref s =
    try
      String.iteri (fun i c -> if c != s.[i] then raise Exit) pref;
      true
    with _ -> false
  in
...
let error_msg event =
  ...
  fun ~no_msg ~msg ->
    Fun.protect
      ...
      (fun () ->
        match Lwt.state error_thread with
        | Return (_, v) -> msg v
        | _ -> no_msg ())
...
let check_for_upgrades (su : Software_updates.t) _user _body _env _state =
  ...
  Lwt_mutex.with_lock upg_lock (fun () ->
      ...
      | _ -> (
          ...
          let* status =
            status_signal
              (fun stat perc -> push_state (`Checking (stat, perc)))
              trans
          in
          ...
          | 1l, _ ->
              ...
              if Stack.is_empty !packages then push_state `Updates_not_avail
              else push_state `Updates_avail;
              add_update_info_timeout su;
              Lwt.return
                (`Value (Util_json.Int.to_yojson (Stack.length !packages)))
          | _ ->
              let msg =
                error_msg ~msg:(fun s -> s) ~no_msg:(fun () -> "Unknown error")
              in
              Logs.err ~src (fun m ->
                  m "check_for_upgrades: error `%s` during obtaining info" msg);
              push_state `Unchecked;
...
let do_upgrade (su : Software_updates.t) reboot _user _body _env _state =
  ...
  Lwt_mutex.with_lock upg_lock (fun () ->
      ...
      | _ -> (
          ...
          let* status =
            status_signal
              (fun stat perc -> push_state (`Upgrading (stat, perc)))
              trans
          in
          let error_msg = error_msg error in
          let package_list = List.of_seq @@ Stack.to_seq !packages in
          let new_version =
            match ui_server_version package_list with
            | Some v -> v
            | None -> su.current
          in
          Logs.info ~src (fun m ->
              m "upgrading packages: %s" @@ String.concat ", " package_list);
          ...
          | 1l, _ ->
              ...
              let* () =
                if Option.value reboot ~default:true then Power.reboot ()
