...
class t (elt : Dom_html.element Js.t) =
  ...
  object (self)
    ...
    val tz_selector : string Select.t =
      ...
      Select.attach tz_elt ~on_change:(fun _ -> push ())

    method! init () : unit = super#init ()

    method! initial_sync_with_dom () : unit =
      _handlers <- [ React.E.map (fun () -> _set_by_user <- true) changed ];
    ...
    method value = tz_selector#value_as_string
...
let make (zones : string list) (init : Pc_control_types.Timedate_config.t) : t =
