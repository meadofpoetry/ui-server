...
class t (elt : Dom_html.element Js.t)
        (mtimer : Mtime.span React.signal)
  ...
  object (self)
    ...
    method! initial_sync_with_dom () : unit =
      ...
      _handlers <-
        [
          React.E.map (fun t -> self#update_time t)
          @@ React.S.changes mtimer;

          React.E.map (fun flag ->
              self#set_disabled flag)
    ...
    method update_time span =
      ...
      let secs =
        (to_s span) -. (to_s _mtime_base)
      in
      let time = Ptime.add_span
                   self#value
    ...
    method private handle_time_change =
      try begin
          let (y, m, d) =
            Scanf.sscanf
              (Option.get date#value)
              "%04d-%02d-%02d"
              (fun y m d -> (y, m, d))
          in
          let (hours, minutes) =
            Scanf.sscanf
              (Option.get time#value)
              "%02d:%02d"
              (fun h m -> (h, m))
          in
          let tz_off =
      ...
      with _ ->
        print_endline "Date or time is null"
...
let make
      (init : Pc_control_types.Timedate_config.t)
      (mtimer : Mtime.span React.signal)
      (disabled : bool React.signal) : t =
