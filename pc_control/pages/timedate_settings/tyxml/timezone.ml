  ...
  let create ?classes ?(a = []) (options : string list)
    ...
    let tz_selector =
      ...
      let options =
        List.map
          (fun opt ->
            if opt = v.timezone then
              Native.option ?classes ~text:opt ~selected:true ()
            else Native.option ?classes ~text:opt ())
          options
      in
      Native.select ~a:[ a_id tz_id ] ?classes ~outlined:true
        ~input_id:tz_input_id ~options ()
    in
    let header =
      create_section_header ~title:(`Text "Часовой пояс") ()
    in
    create_section ?classes ~a:(a_id id :: a) ~header ~children:[ tz_selector ]
      ()
end
