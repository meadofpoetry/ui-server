...
let make_warning_dialog () =
  ...
  let content =
    ...
    @@ Dialog.Markup.create_content_simple
      "После завершения обновления прибор будет автоматически перезагружен.\n\
       Во время перезагрузки доступ к прибору будет недоступен.\n"
      () in
  let actions =
    List.map Js_of_ocaml_tyxml.Tyxml_js.To_dom.of_element
      [ Dialog.Markup.create_action ~action:Close ~label:"Отмена" ()
      ; Dialog.Markup.create_action ~action:Accept ~label:"Продолжить обновление" ()
      ] in
...
class t (elt : Dom_html.element Js.t) = object(self)
  ...
  method private handle_state_change (state : state) : unit Lwt.t =
    ...
    if not @@ equal_state _state state
    then placeholder#set_text ~loading hint;
    _state <- state;
    (* Update progress bar *)
    match state with
          ...
          (warning_dialog#open_await ()
           ...
           | Error err ->
             let msg = Printf.sprintf
                 ...
                 (Api_js.Http.error_to_string err) in
  ...
  method private set_placeholder_icon path =
    ...
    | Some x ->
      ...
      | None -> ()
      | Some x ->
        Element.set_attribute x "d" path
