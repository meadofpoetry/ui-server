...
module D =
  Page_network_settings_tyxml.Make (Tyxml_js.Xml) (Tyxml_js.Svg) (Tyxml_js.Html)
...
let make_dialog set =
  ...
  let content =
    dialog_content
      ~children:
        [
          Tyxml_js.Html.txt
            "Применение настроек может привести \
             к разрыву соединения. Хотите \
             продолжить?";
        ]
      ()
  in
  let actions =
    [
      ...
      dialog_action ~appearance:Raised ~action:Accept
        ~label:"Продолжить" ();
    ]
  in
  ...
  Button.make ~appearance:Raised
    ~on_click:(fun btn _ _ ->
      ...
      let value =
        ...
        | _ -> None
      in
      match value with None -> Lwt.return_unit | Some x -> open_dialog btn x)
    ~label:"Применить" ()
...
let () =
  ...
  let thread =
    Pc_control_http_js.Network.get_config () >>=? fun config ->
    ...
    let set value =
      ...
      let thread =
        Pc_control_http_js.Network.set_config value >>=? fun () ->
    ...
    let submit =
      make_submit_button ethernet ipv4 dns routes config open_dialog
    in
    ...
    page#set_on_destroy (fun () ->
        Element.remove_child_safe Dom_html.document##.body
          confirmation_dialog#root;
        snackbar#destroy ();
        confirmation_dialog#destroy ();
        React.E.stop ~strong:true event';
        React.E.stop ~strong:true event;
        Lwt.async (fun () ->
            Api_js.Websocket.JSON.unsubscribe socket event_id >>= function
