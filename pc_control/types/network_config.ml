...
let ( >>= ) v f = match v with None -> None | Some x -> f x

let result_to_opt = function Ok v -> Some v | _ -> None

type meth = Auto | Manual [@@deriving yojson, eq]

let meth_to_string = function Auto -> "auto" | Manual -> "manual"
...
let address_of_string s =
  ...
  | [ a; m ] ->
      Ipaddr.V4.of_string a |> result_to_opt >>= fun a ->
      Int32.of_string_opt m >>= fun m -> Some (a, m)
  | _ -> None

type routes = { static : address list; gateway : Ipaddr.V4.t option }
[@@deriving yojson, eq]
...
(* TODO make optional *)
and conn_conf = { autoconnect : autoconnect; id : string; uuid : string }
