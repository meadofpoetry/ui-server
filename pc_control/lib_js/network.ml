...
let make_routes (routes : Network_config.address list) =
  ...
  let add_box  = new Box.t ~widgets:[address#widget; mask#widget; add_but#widget] () in

  let full_box = new Box.t ~widgets:[list#widget; add_box#widget] () in
...
let make_ipv4 (ipv4 : Network_config.ipv4_conf) =
  ...
  let meth       = new Switch.t ~input_id:"Автоматическая настройка" () in
  
  let address    = new Textfield.t ~input_id:"Адрес" ~input_type:Widget.IPV4 () in
  let mask       = new Textfield.t ~input_id:"Маска подсети" ~input_type:(Widget.Integer (Some 0, Some 32)) () in
  let gateway    = new Textfield.t ~input_id:"Шлюз" ~input_type:Widget.IPV4 () in
  let routes, routes_s, routes_set = make_routes ipv4.routes.static in
  ...
  Lwt_react.S.keep @@
    Lwt_react.S.map (fun disabled -> address#set_disabled disabled;
                                     ...
                                     gateway#set_disabled) meth#s_state;
  
  let ipv4_sets  = new Card.t
                     ~widgets:[ ipv4_head#widget
                              ; (new Card.Media.t ~widgets:[new Box.t ~vertical:true ~widgets:[ address#widget;
                                                                                                mask#widget;
                                                                                                gateway#widget;
                                                                                                routes#widget ] ()
                                   ] ())#widget
