...
let pack_boards (blist : Boards.Board.t list) =
  List.fold_left
    (fun acc (b : Boards.Board.t) -> Boards.Board.Map.add b.control b acc)
    Boards.Board.Map.empty blist

let gen_stream_ts ?(desc = None) src id : Common.Stream.t =
  { source = src; id = `Ts id; description = desc }

let gen_stream_ip ?(desc = None) src ip : Common.Stream.t =
  { source = src; id = `Ip ip; description = desc }
...
let stream_on_port1 () =
  ...
  let parent =
    gen_stream_ts
      (Input { input = RF; id = 0 })
      Single ~desc:(Some "this stream")
  in
  p2 [ parent ];
  p1 [ gen_raw_stream_ts (Port 1) Single ~desc:None ];
  let reslt =
    try
      React.S.value streams |> List.find (fun s -> s.id = `Ts Single)
      |> fun s -> Some s
    with _ -> None
  in
  let expected = Some parent in
  Alcotest.(check (option strmtst)) "stream_on_port1" expected reslt
...
let no_parent_streams () =
  ...
  let plpstrm =
    gen_stream_ts
      (Input { input = ASI; id = 1 })
      (T2mi_plp 1) ~desc:(Some "this stream")
  in
  p4 [ plpstrm ];
  p3
    [
      gen_stream_ts (Parent plpstrm) Single ~desc:(Some "this stream"); plpstrm;
    ];
  p1 [ gen_raw_stream_ts (Port 1) Single ~desc:None ];
  let reslt =
    try
  ...
  Alcotest.(check (option strmtst)) "no_parent_streams" expected reslt
...
let hierarchy () =
  ...
  in
  let child =
    gen_stream_ts (Parent plpstrm) (Unknown 42l) ~desc:(Some "this stream")
  in
  p4 [ plpstrm ];
  p3 [ child ];
  ...
  let reslt =
    try
      React.S.value streams |> List.find (fun s -> s.id = `Ts (Unknown 42l))
      |> fun s -> Some s
    with _ -> None
  in
  let expected = Some child in
  Alcotest.(check (option strmtst)) "hierarchy" expected reslt
...
let bad_id () =
  ...
  let reslt =
    try
      React.S.value streams |> List.find (fun s -> s.id = `Ts (Unknown 43l))
      |> fun s -> Some s
    with _ -> None
  in
  let expected = None in
  Alcotest.(check (option strmtst)) "bad_id" expected reslt
...
let unpacked_plp () =
  ...
  let parent =
    gen_stream_ts
      (Input { input = RF; id = 0 })
      (T2mi_plp 42) ~desc:(Some "plp parent")
  in
  p2 [ parent ];
  p1
    [
      gen_raw_stream_ts (Stream (T2mi_plp 42)) (Unknown 43l)
        ~desc:(Some "43rd stream");
      gen_raw_stream_ts (Stream (T2mi_plp 42)) (Unknown 44l)
        ~desc:(Some "44th stream");
  ...
  in
  let expected_a =
    gen_stream_ts (Parent parent) (Unknown 43l) ~desc:(Some "43rd stream")
  in
  let expected_b =
    gen_stream_ts (Parent parent) (Unknown 44l) ~desc:(Some "44th stream")
  in
  let expected = [ expected_a; expected_b; parent ] in
  Alcotest.(check (list strmtst)) "unpacked_plp" expected reslt
...
let unpacked_plp_plus_single () =
  ...
  Alcotest.(check (list strmtst)) "unpacked_plp_plus_single" expected reslt
...
let lost_stream () =
  ...
  let reslt =
    try
      let sms = React.S.value streams in
      List.find (fun s -> s.id = `Ts Single) sms |> ignore;
      (* exists *)
      p2 [];
      (* remove parent stream *)
      List.find (fun s -> s.id = `Ts Single) sms |> fun s -> Some s
    with _ -> None
  in
  let expected = Some parent in
  Alcotest.(check (option strmtst)) "lost_stream" expected reslt
...
let removed_stream () =
  ...
  let reslt =
    try
      let sms = React.S.value streams in
      List.find (fun s -> s.id = `Ts (Unknown 43l)) sms |> ignore;
      (* exists *)
      List.find (fun s -> s.id = `Ts (Unknown 44l)) sms |> ignore;
      (* exists *)
      List.find (fun s -> s.id = `Ts (T2mi_plp 42)) sms |> ignore;
      (* exists *)
  ...
  Alcotest.(check (option (list strmtst))) "removed_stream" expected reslt
...
let removed_stream_v2 () =
  ...
  let addit =
    gen_stream_ts
      (Input { input = RF; id = 0 })
      (Unknown 13l) ~desc:(Some "additional")
  in
  let parent =
    gen_stream_ts
      (Input { input = RF; id = 0 })
  ...
  Alcotest.(check (option (list strmtst))) "removed_stream_v2" expected reslt
  ...
  let reslt =
    try
      ...
      try
        p1 [];
        let sms = React.S.value streams in
        List.find (fun s -> s.id = `Ts Single) sms |> fun s -> Some (Some s)
      with _ -> Some None
    with _ -> None
  in
  let expected = Some None in
  Alcotest.(check (option (option strmtst)))
    "removed_lost_stream" expected reslt
...
let local_stream () =
  ...
  let expected =
    Some
      (gen_stream_ts
         (Input { input = TSOIP; id = 2 })
         (Unknown 13l) ~desc:(Some "Lucky 13th"))
  in
  Alcotest.(check (option strmtst)) "local_stream" expected reslt
...
let appeared_stream () =
  ...
  in
  let expected = Some parent in
  Alcotest.(check (option strmtst)) "appeared_stream" expected reslt
...
let () = Alcotest.run "Meta_board test" [ ("test", test_set) ]
