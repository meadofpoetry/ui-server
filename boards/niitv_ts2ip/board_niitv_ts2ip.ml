  ...
  List.fold_left
    (fun acc { Topology.port; _ } ->
      let s =
        ...
        @@ React.E.map
             (fun ({ sync; _ } : device_status) ->
               List.exists (( = ) port % socket_to_enum) sync)
             status
      in
...
let make_streams ~incoming_streams ~outgoing_streams config ports =
  React.S.l3
    (fun incoming outgoing config ->
      let outgoing =
        List.filter_map
          (fun (mode : udp_mode) ->
            let url =
              ( ( Uri.with_host_v4 Uri.empty mode.dst_ip |> fun uri ->
                  Uri.with_scheme uri @@ Some "udp" )
            ...
            let present, stream =
              ...
              | Full s ->
                  let present =
                    match Stream.find_by_id s.id outgoing with
                    | None -> false
                    | Some _ -> true
                  in
                  (present, Some s)
      ...
      let incoming =
        List.filter_map
          (fun (stream : Stream.t) ->
            match
              List.find_opt
                (fun ({ stream = s; _ } : Stream.Table.stream) ->
                  Stream.ID.equal stream.id s.id)
                outgoing
            with
  ...
  | `Fine -> (
      let rec check_loop acc = function
        ...
        | { Stream.Table.url; stream } :: tl -> (
            match
              ( Netlib.Uri.host_v4 url,
                Uri.port url,
                stream_to_socket ports stream )
            with
            ...
            | _, _, None -> Error (`Internal_error "Invalid stream") )
      in
      ...
      | Ok x -> (
          let mode =
            List.map
              ...
              x
          in
          Api_transmitter.set_mode_ api mode >>= function
          | `Error e -> Lwt.return_error @@ `Internal_error e
          | `Unit -> Lwt.return_ok () ) )

let board_id = Board_niitv_ts2ip_types.board_id

let create (b : Topology.topo_board) (streams : Stream.t list React.signal)
    (convert_streams :
      Topology.topo_board ->
      Stream.Raw.t list React.signal ->
      Stream.t list React.signal) (send : Cstruct.t -> unit Lwt.t) (_ : Db.t)
  ...
  let source_state =
    React.S.l2 ~eq:Stream.Table.equal_source_state
      (fun s d ->
        ...
        | _ -> `Forbidden)
      api.notifs.state api.notifs.devinfo
  in
  let range =
  ...
  let board =
    {
      Boards.Board.http = Board_niitv_ts2ip_http.handlers b.control api;
      ...
      stream_handler =
        Some
          (object
             method streams =
               make_streams ~outgoing_streams:api.notifs.outgoing_streams
                 ~incoming_streams:api.notifs.incoming_streams cfg#s b.ports
