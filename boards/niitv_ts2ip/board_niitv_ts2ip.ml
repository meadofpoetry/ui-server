...
let get_ports_active ports =
  List.fold_left
    (fun acc {Topology.port; _} -> Boards.Board.Ports.add port React.S.Bool.one acc)
  ...
  | `Fine -> (
      let rec check_loop acc = function
        ...
        | {Stream.Table.url; stream} :: tl -> (
          match Netlib.Uri.host_v4 url, Uri.port url, stream_to_socket ports stream with
...
let create
  ...
  let source_state =
    React.S.l2
      ~eq:Stream.Table.equal_source_state
      (fun s d ->
        match s, d with
