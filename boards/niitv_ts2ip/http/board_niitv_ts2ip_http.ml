...
let handlers (control : int) (api : Protocol.api) =
  ...
  [ merge
      ...
      [ make
          ~prefix:"device"
          [ node
              ~doc:"Returns the state of the device"
              ~meth:`GET
          ...
          ; node
              ~doc:"Sets IP address of the device"
              ~meth:`POST
              ~restrict:[`Guest]
              ~path:(Path.Format.of_string "ip-address")
              ~query:Query.empty
          ...
          ; node
              ~doc:"Sets subnet mask of the device"
              ~meth:`POST
              ~restrict:[`Guest]
              ~path:(Path.Format.of_string "subnet-mask")
              ~query:Query.empty
          ...
          ; node
              ~doc:"Sets gateway address of the device"
              ~meth:`POST
              ~restrict:[`Guest]
              ~path:(Path.Format.of_string "gateway")
              ~query:Query.empty
      ...
      ; make
          ~prefix:"transmitter"
          [ node
              ~doc:"UDP transmitter status"
              ~meth:`GET
              ~path:Path.Format.("status" @/ empty)
              ~query:Query.empty
              (Api_transmitter.get_status api)
          ; node
              ~doc:"Sets streams to transmit via UDP"
              ~meth:`POST
              ~restrict:[`Guest]
...
let ws (control : int) (api : Protocol.api) =
  ...
  [ merge
      ...
      [ make
          ...
          ; event_node
              ~doc:"Device config socket"
              ~path:(Path.Format.of_string "config")
              ~query:Query.empty
              (Api_device.Event.get_config api)
          ; event_node
              ~doc:"Device status socket"
              ~path:(Path.Format.of_string "status")
              ~query:Query.empty
      ...
      ; make
          ~prefix:"transmitter"
          [ event_node
              ~doc:"Transmitter status socket"
              ~path:(Path.Format.of_string "status")
              ~query:Query.empty
              (Api_transmitter.Event.get_status api)
          ; event_node
              ~doc:"Transmitters mode socket"
              ~path:(Path.Format.of_string "mode")
              ~query:Query.empty
