...
let send (type a) (src : Logs.src) (state : Topology.state React.signal)
    (push : _ Lwt_stream.bounded_push) (sender : Cstruct.t -> unit Lwt.t)
    (req : a Request.t) =
  ...
  | `Fine ->
      Lwt.catch
        (fun () ->
          let t, w = Lwt.task () in
          let send stream =
            Fsm.request src stream sender req >>= fun x ->
...
let find_stream (ports : Topology.topo_port list) (stream : Stream.t)
    (socket : socket) (streams : Stream.t list) =
  List.find_opt
    (fun (t : Stream.t) ->
...
let to_out_streams_s (ports : Topology.topo_port list)
    ...
    (streams : Stream.t list React.signal) (config : config React.signal) =
  React.S.sample
    (fun (status : transmitter_status) (streams, config) ->
      let config_udp = config.mode.udp in
      let status_udp =
        Boards.Util.List.take (List.length config_udp) status.udp
      in
      List.map2
        (fun { enabled; sync; stream; _ } { socket; dst_ip; dst_port; _ } ->
                ...
                | Some port ->
                    ...
                    then
                      let (orig_id : Stream.container_id) =
                        TSoIP { scheme = "udp"; addr = dst_ip; port = dst_port }
                      in
                      Some { s with orig_id }
                    else None)
              streams
          else None)
        status_udp config_udp
...
let port_to_stream (port : Topology.topo_port) =
  ...
  | Some socket ->
      let stream =
        {
          Stream.Raw.source = { info = Plain; node = Port port.port };
          id = TS_raw;
          typ = TS;
        }
      in
...
let update_incoming_streams (streams : Stream.t list React.signal)
    (conv : Stream.Raw.t list React.signal -> Stream.t list React.signal)
    (status : device_status React.event) (ports : Topology.topo_port list) =
  ...
  let input_streams =
    ...
    @@ React.E.map (fun (status : device_status) -> status.sync) status
  in
  React.S.merge
...
let change_udp_mode (streams : Stream.t list React.signal)
    (status : transmitter_status React.event) (kv : config Kv_v.rw) =
               ...
               | Full s ->
                   let change', mode =
                     ...
                     | Some { orig_id = TS_multi _ | TS_raw; _ } ->
                         if not enabled then (true, { mode with enabled = true })
                         else (false, mode)
                     | None | Some { orig_id = TSoIP _; _ } ->
                         if enabled then (true, { mode with enabled = false })
                         else (false, mode)
                   in
...
let create (src : Logs.src) (sender : Cstruct.t -> unit Lwt.t)
    ...
    (kv : config Kv_v.rw) (ports : Topology.topo_port list) =
  let state, set_state = React.S.create ~eq:Topology.equal_state `No_response in
  let devinfo, set_devinfo =
    React.S.create ~eq:(Util_equal.Option.equal equal_devinfo) None
  in
  ...
  let incoming_streams =
    update_incoming_streams incoming_streams streams_conv device_status ports
  in
  let outgoing_streams =
    React.S.hold ~eq:(Util_equal.List.equal Stream.equal) []
    @@ to_out_streams_s ports transmitter_status incoming_streams kv#s
  in
  let (notifs : notifs) =
    {
      state;
    ...
    | _ ->
        List.iter
          (function
            ...
            | x -> push_rsp_queue @@ Some x)
          parsed
  in
  let channel req = send src state push_req_queue sender req in
  let loop =
    Fsm.start src sender req_queue rsp_queue evt_queue mode_changer kv set_state
      (fun ?step x -> set_devinfo ?step @@ Some x)
      set_device_status set_transmitter_status
  in
  let api = { notifs; loop; ports; push_data; channel; kv } in
  Lwt.return_ok api
