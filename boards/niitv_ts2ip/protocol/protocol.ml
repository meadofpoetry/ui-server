...
let port_to_stream (port : Topology.topo_port) =
  ...
  | Some socket ->
      let stream =
        {Stream.Raw.source = {info = Plain; node = Port port.port}; id = TS_raw; typ = TS}
      in
      Some (socket, stream)
             ...
             | Full s ->
                 let change', mode =
                   ...
                   | None | Some {orig_id = TSoIP _; _} ->
                       if enabled then true, {mode with enabled = false} else false, mode
                 in
