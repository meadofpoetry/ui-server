...
let request (type a) (src : Logs.src) (stream : Request.msg Lwt_stream.t)
    (sender : Cstruct.t -> unit Lwt.t) (req : a Request.t) :
    (a, Request.error) result Lwt.t =
...
let start (src : Logs.src) (sender : Cstruct.t -> unit Lwt.t)
    ...
    (evt_queue : Cstruct.t Lwt_stream.t) (mode_changer : mode Lwt_stream.t)
    (kv : config Kv_v.rw) (set_state : Application_types.Topology.state set)
    (set_devinfo : devinfo set) (set_device_status : device_status set)
    ...
    | l ->
        ...
        List.iteri
          (fun i (x : udp_mode) ->
            Logs.debug (fun m ->
                m
                  "Setting UDP transmitter %d: stream = %s, IP = %a, port = \
                   %d, self port = %d, enabled = %b, socket = %s"
                  (start + i) (show_stream x.stream) Ipaddr.V4.pp x.dst_ip
    ...
    let rec loop () =
      Lwt.pick
        [
          ...
          (Lwt_stream.next rsp_queue >>= function
  ...
  and initialize (info : devinfo) =
    ...
    sender @@ Serializer.serialize Request.(Set_mac mac) >>= fun () ->
    let mode, aux_1, aux_2 = Request.split_mode mode in
    Logs.debug (fun m ->
        m "Setting network: IP = %a, mask = %a, gateway = %a" Ipaddr.V4.pp
          mode.network.ip Ipaddr.V4.pp mode.network.mask Ipaddr.V4.pp
          mode.network.gateway);
    log_udp_transmitters 0 mode.udp;
    sender @@ Serializer.serialize Request.(Set_mode_main mode) >>= fun () ->
    log_udp_transmitters Message.n_udp_main aux_1;
    sender @@ Serializer.serialize Request.(Set_mode_aux_1 aux_1) >>= fun () ->
    log_udp_transmitters Message.(n_udp_main + n_udp_aux) aux_2;
    sender @@ Serializer.serialize Request.(Set_mode_aux_2 aux_2) >>= fun () ->
    Lwt_stream.junk_old rsp_queue >>= fun () ->
    Lwt_stream.junk_old evt_queue >>= idle
  and idle () =
    set_state `Fine;
    Logs.info (fun m -> m "Initialization done!");
    Lwt.pick [ status_loop (); mode_loop (); client_loop () ] >>= restart
  and mode_loop () =
    Lwt_stream.last_new mode_changer >>= fun mode ->
    let mode, aux_1, aux_2 = Request.split_mode mode in
    sender @@ Serializer.serialize Request.(Set_mode_main mode) >>= fun () ->
    sender @@ Serializer.serialize Request.(Set_mode_aux_1 aux_1) >>= fun () ->
  ...
  and client_loop () =
    ...
    | `E ->
        Logs.err (fun m ->
            m
              "Got devinfo during operation, seems the device is not \
               responding...");
        Lwt.return ()
  and status_loop () =
    Lwt.pick
      [
        ( Lwt_stream.next evt_queue >>= fun data ->
