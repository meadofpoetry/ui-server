...
let parse_udp_status (buf : Cstruct.t) =
  ...
  let stream =
    match Int32.logand strm 0x3F_FF_FFl with
    | 0l -> Stream.TS_raw
    | x -> TS_multi (Stream.Multi_TS_ID.of_int32_pure x)
  in
...
let parse_status_data (buf : Cstruct.t) =
  let iter =
    Cstruct.iter (fun _ -> Some Message.sizeof_udp_status) parse_udp_status buf
  in
...
let parse_status (buf : Cstruct.t) =
  try
    ...
    let sync =
      ...
      |> cons_if (input land 0x10 > 0) ASI_2
    in
...
let check_prefix (buf : Cstruct.t) =
  try
    ...
    if Message.tag_start = tag then Ok buf else Error (Invalid_start_tag tag)
