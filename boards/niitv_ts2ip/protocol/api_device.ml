...
module Event = struct
  ...
  let get_state (api : Protocol.api) _user =
    let event =
      S.changes api.notifs.state |> E.map Application_types.Topology.state_to_yojson
    in
    Lwt.return event
...
let get_network (api : Protocol.api) _user _body _env _state =
  (api.kv)#get >>= fun cfg -> return_value (network_mode_to_yojson cfg.mode.network)

let get_ip_address (api : Protocol.api) _user _body _env _state =
  (api.kv)#get >>= fun cfg -> return_value (Ipaddr.V4.to_yojson cfg.mode.network.ip)

let get_mask (api : Protocol.api) _user _body _env _state =
  (api.kv)#get >>= fun cfg -> return_value (Ipaddr.V4.to_yojson cfg.mode.network.mask)

let get_gateway (api : Protocol.api) _user _body _env _state =
  (api.kv)#get >>= fun cfg -> return_value (Ipaddr.V4.to_yojson cfg.mode.network.gateway)
...
let set_mac (api : Protocol.api) force _user body _env _state =
  ...
  | Ok mac ->
      ...
      >>=? fun () -> (api.kv)#set {config with mac} >>= fun () -> Lwt.return `Unit
...
let set_nw (api : Protocol.api) body _of f =
  ...
  | Ok v ->
      ...
      Lwt_result.Infix.(
        api.channel (Request.Set_mode_main main) >>= fun () -> Lwt.return_ok mode)
...
let set_mode (api : Protocol.api) _user body _env _state =
  ...
  | Ok mode -> (
      ...
      | Ok udp ->
          ...
          (api.kv)#get
          >>= fun config ->
          (api.kv)#set {config with mode} >>= fun () -> Lwt.return `Unit)

let set_network (api : Protocol.api) _user body _env _state =
  set_nw api body network_mode_of_yojson (fun cfg network -> {cfg.mode with network})
