...
let base_class = "dvb-niit-measure"
...
module type R = sig
  type t
  val make : int -> t option React.signal -> unit Item_list.Item.t
end
...
module Make(M:M) = struct
  ...
  let make (event:(int * measures) React.event)
    ...
    let (module R)   =
      (module struct
         include M let typ = conf.typ end
              : Row.M with type t = M.t) in
    let (module ROW) =
      (module struct include Row.Make(R) end
              : R with type t = M.t) in
    let ids = match conf.ids with
      | Some x -> x
      | None   -> List.map fst @@ React.S.value config in
    let items =
      List.map (fun id -> ROW.make id @@ React.S.hold None @@ M.get id event)
      @@ List.sort compare ids in
    let list = new Item_list.t
                 ~non_interactive:true
                 ~items:(List.map (fun x -> `Item x) items) () in
...
module Bitrate =
  Make(struct
      ...
      let get x e =
        React.(E.filter (fun (id,_) -> id = x) e
               |> E.map (fun (_,m)  ->
                      Option.map (fun x -> float_of_int x /. 1_000_000.)
                        m.bitrate))
    end)
