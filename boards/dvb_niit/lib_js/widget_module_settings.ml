...
let make_mode_box ~(standard : standard)
  ...
  let b =
    new Vbox.t
      ~widgets:(match standard with
                | T | C -> [freq#widget; bw#widget]
                | T2 -> [freq#widget; bw#widget; plp#widget]) () in
  let s =
    React.S.l3 (fun freq bw plp ->
        match freq, bw, plp with
        | Some freq, Some bw, Some plp -> Some { freq; bw; plp }
        | _ -> None)
      freq#s_selected_value bw#s_selected_value plp#s_input in
  let _ =
    React.E.map (fun (s : channel) ->
        set_freq s.freq; set_bw s.bw; set_plp s.plp)
      event in
  let _ =
    React.S.map (fun x ->
...
let make_module_settings ~(id : int)
  ...
  let t2_box, s_t2 =
    make_mode_box
      ~state
      ~event:(React.E.map (fun x -> x.channel) event)
      ~standard:T2
      ~init:init.channel
      () in
  let t_box, s_t =
  ...
  let s : (int * mode) option React.signal =
    React.S.l5 (fun standard t2 t c state ->
        ...
        | Some T2, Some t2, _, _, `Fine ->
           Some (id, { standard = T2; channel = t2 })
        | Some T, _, Some t, _, `Fine ->
           Some (id, { standard = T ; channel = t  })
        | Some C, _, _, Some c, `Fine ->
           Some (id,{ standard = C ; channel = c  })
        | _ -> None)
      standard#s_selected_value s_t2 s_t s_c state in
  let box =
    new Vbox.t
      ~widgets:[ standard#widget
               ; t2_box#widget
               ; t_box#widget
               ; c_box#widget ]
      () in
  let update_visibility = function
    ...
    | Some T ->
       (try Dom.removeChild box#root t2_box#root with _ -> ());
       (try Dom.removeChild box#root c_box#root with _ -> ());
  ...
  let submit = fun (id, m) -> Lwt_result.return () in
    (* Requests.Receivers.HTTP.post_mode ~id m control >|= (fun _ -> ()) in *)
  box#widget,s,submit
...
let make ~(state : Common.Topology.state React.signal)
  ...
  let w, s, set =
    make_module_settings ~id:conf.id
