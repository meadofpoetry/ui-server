...
let make (control:int) =
  ...
  let mer_p   = factory#create (Module_measure { id = 0; typ = `Mer }) in
  let ber_p   = factory#create (Module_measure { id = 0; typ = `Ber }) in
  let frq_p   = factory#create (Module_measure { id = 0; typ = `Freq }) in
  let pow_p   = factory#create (Module_measure { id = 0; typ = `Power }) in
  let br_p    = factory#create (Module_measure { id = 0; typ = `Bitrate}) in
  let param1  = factory#create (Module_measures { id = 0 }) in
  let param2  = factory#create (Module_measures { id = 1 }) in
  let param3  = factory#create (Module_measures { id = 2 }) in
  let param4  = factory#create (Module_measures { id = 3 }) in
  let ms      = factory#create (Measures { ids = None }) in
  let m       = factory#create (Measure { ids = None; typ = `Mer }) in
  ...
  let wrap x  = x in
  let items   = [ Widget_grid.Item.to_item ~pos:{x=0;y=0;w=2;h=3}  @@ wrap s
                ; Widget_grid.Item.to_item ~pos:{x=2;y=0;w=2;h=3}  @@ wrap ms
                ...
                ; Widget_grid.Item.to_item ~pos:{x=0;y=3;w=2;h=2}  @@ wrap m
