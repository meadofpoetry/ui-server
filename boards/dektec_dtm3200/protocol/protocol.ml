...
let send (type a) (src : Logs.src) (state : Topology.state React.signal)
    ...
    (kv : config Kv_v.rw) (req : a Request.t) =
  ...
  | `Fine ->
      Lwt.catch
        (fun () ->
          let t, w = Lwt.task () in
          let send stream =
            Fsm_common.request src sender stream kv req >>= fun x ->
...
let to_streams_s (config : config signal) (status : status event) =
  ...
  @@ S.sample
       (fun ({ asi_bitrate; protocol; _ } : status)
            ({ ip_receive; nw; _ } : config) ->
         if asi_bitrate <= 0 then []
         else
           let scheme = match protocol with RTP -> "rtp" | UDP -> "udp" in
           let addr =
             match ip_receive.addressing_method with
             | Multicast -> ip_receive.multicast
             | Unicast -> nw.ip_address
           in
           let (info : Stream.Source.ipv4) =
             { addr; port = ip_receive.udp_port; scheme }
           in
...
let create (src : Logs.src) (sender : Cstruct.t -> unit Lwt.t) streams_conv
    (kv : config Kv_v.rw) (_ : Db.t) =
  ...
  let devinfo, set_devinfo =
    S.create ~eq:(Util_equal.Option.equal equal_devinfo) None
  in
  let notifs =
    {
  ...
  in
  let channel req = send src state push_req_queue sender kv req in
  let loop =
    Fsm.start src sender req_queue rsp_queue kv set_state
      (fun ?step x -> set_devinfo ?step @@ Some x)
      set_status
  in
  let (api : api) = { notifs; loop; push_data; kv; channel } in
  Lwt.return_ok api
