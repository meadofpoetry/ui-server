...
let to_streams_s (config : config signal) (status : status event) =
  ...
  @@ S.sample
       (fun ({asi_bitrate; protocol; _} : status) ({ip_receive; nw; _} : config) ->
         if asi_bitrate <= 0
         then []
         else
           let scheme =
           ...
           let (stream : Stream.Raw.t) =
             {source = {info = IPV4 info; node = Port 0}; id = TS_raw; typ = TS}
           in
           [stream])
       status
       config
...
let create
  ...
  let devinfo, set_devinfo = S.create ~eq:(Util_equal.Option.equal equal_devinfo) None in
  let notifs =
