...
let step ~(return : Request.error -> unit Lwt.t)
    ~(continue : devinfo -> unit Lwt.t) (src : Logs.src)
    (sender : Cstruct.t -> unit Lwt.t) (rsp_queue : Cstruct.t cmd Lwt_stream.t)
  ...
  and typ acc =
    request src sender rsp_queue config (Device Type) >>=? fun (x : int) ->
    mac_address GList.(x :: acc)
  and mac_address acc =
    ...
    let devinfo =
      ...
      | [ typ; serial; fw_ver; hw_ver; fpga_ver ] ->
          { fpga_ver; hw_ver; fw_ver; serial; typ; mac }
    in
    continue devinfo
  in
  fpga_version
