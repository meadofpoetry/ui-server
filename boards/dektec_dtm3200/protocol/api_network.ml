...
module Event = struct
  ...
  let get_config (api : Protocol.api) _user =
    let event = S.changes api.notifs.config |> E.map (fun x -> nw_to_yojson x.nw) in
    Lwt.return event
end
...
let get_ip_address (api : Protocol.api) _user _body _env _state =
  api.channel Request.(Network (IP_address `R)) >>=? return_value % Ipaddr.V4.to_yojson
...
let set_ip_address (api : Protocol.api) _user body _env _state =
  ...
  | Ok ip_address ->
      ...
      (api.kv)#get
      >>= fun cfg ->
      (api.kv)#set {cfg with nw = {cfg.nw with ip_address}}
      >>= fun () -> return_value @@ Ipaddr.V4.to_yojson ip'

let get_subnet_mask (api : Protocol.api) _user _body _env _state =
  api.channel Request.(Network (Subnet_mask `R)) >>=? return_value % Ipaddr.V4.to_yojson
...
let set_subnet_mask (api : Protocol.api) _user body _env _state =
  ...
  | Ok mask ->
      ...
      (api.kv)#get
      >>= fun cfg ->
      (api.kv)#set {cfg with nw = {cfg.nw with mask}}
      >>= fun () -> return_value @@ Ipaddr.V4.to_yojson mask'

let get_gateway (api : Protocol.api) _user _body _env _state =
  api.channel Request.(Network (Gateway `R)) >>=? return_value % Ipaddr.V4.to_yojson
...
let set_gateway (api : Protocol.api) _user body _env _state =
  ...
  | Ok gateway ->
      ...
      (api.kv)#get
      >>= fun cfg ->
      (api.kv)#set {cfg with nw = {cfg.nw with gateway}}
      >>= fun () -> return_value @@ Ipaddr.V4.to_yojson gateway'

let get_dhcp (api : Protocol.api) _user _body _env _state =
  api.channel Request.(Network (DHCP `R)) >>=? return_value % Util_json.Bool.to_yojson
...
let set_dhcp (api : Protocol.api) _user body _env _state =
  ...
  | Ok dhcp ->
      ...
      (api.kv)#get
      >>= fun cfg ->
      (api.kv)#set {cfg with nw = {cfg.nw with dhcp}}
      >>= fun () -> return_value @@ Util_json.Bool.to_yojson dhcp'

let get_mac_address (api : Protocol.api) _user _body _env _state =
  api.channel Request.(Network MAC_address) >>=? return_value % Macaddr.to_yojson
