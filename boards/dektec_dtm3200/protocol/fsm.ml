...
let start (src : Logs.src) (sender : Cstruct.t -> unit Lwt.t)
    ...
    (rsp_queue : Cstruct.t Request.cmd Lwt_stream.t) (kv : config Kv_v.rw)
    (set_state : Application_types.Topology.state set)
    (set_devinfo : devinfo set) (set_status : status set) =
  ...
  and detect () =
    ...
    Fsm_detect.step
      ~return:(fun e ->
        ...
        restart ())
      ~continue:initialize src sender rsp_queue kv ()
  and initialize (devinfo : devinfo) =
    ...
    Fsm_init.step
      ~return:(fun e ->
        Logs.err (fun m ->
            m "Error during init step: %s" @@ Request.error_to_string e);
        restart ())
      ~continue:(fun () ->
        set_state `Fine;
  ...
  and pull_status () =
    Fsm_probes.step
      ~return:(fun e ->
        Logs.err (fun m ->
            m "Error during pulling status: %s" @@ Request.error_to_string e);
        restart ())
      ~continue:(fun status ->
        set_status status;
        idle ())
  ...
  and idle ?wait_msg ?timer () =
    let timer =
      match timer with
      | Some x -> x
      | None -> Lwt_unix.sleep status_interval >>= fun () -> Lwt.return `S
    in
    let wait_msg =
      match wait_msg with
      | Some x -> x
      | None -> Lwt_stream.next req_queue >>= fun x -> Lwt.return @@ `M x
    in
