  ...
  [
    merge ~prefix:(string_of_int control)
      [
        make ~prefix:"device"
          [
            node ~doc:"State of the device" ~meth:`GET
              ~path:(Path.Format.of_string "state")
              ~query:Query.empty (Api_device.get_state api);
            node ~doc:"Integrated device description" ~meth:`GET
              ~path:(Path.Format.of_string "info")
              ~query:Query.empty (Api_device.get_info api);
            node ~doc:"Device configuration" ~meth:`GET
            ...
            node ~doc:"Hardware version number" ~meth:`GET
              ~path:(Path.Format.of_string "hardware-version")
              ~query:Query.empty
              (Api_device.get_hardware_version api);
            node
              ~doc:
                "Firmware version: the major version is encoded in the tens, \
            ...
            node ~doc:"Device type number, e.g. 3200" ~meth:`GET
              ~path:(Path.Format.of_string "type")
              ~query:Query.empty (Api_device.get_type api);
          ];
        make ~prefix:"network"
          [
            node ~doc:"Resets the board" ~restrict:[ `Guest ] ~meth:`POST
              ~path:(Path.Format.of_string "reboot")
              ~query:Query.empty (Api_network.reboot api);
            node ~doc:"Integrated network configuration" ~meth:`GET
              ~path:(Path.Format.of_string "config")
              ~query:Query.empty
              (Api_network.get_config api);
            node ~doc:"IP address of the device" ~meth:`GET
              ~path:(Path.Format.of_string "ip-address")
              ~query:Query.empty
              (Api_network.get_ip_address api);
            node ~doc:"IP address of the device" ~restrict:[ `Guest ]
              ~meth:`POST
              ~path:(Path.Format.of_string "ip-address")
              ~query:Query.empty
              (Api_network.set_ip_address api);
            node ~doc:"Subnet mask" ~meth:`GET
              ~path:(Path.Format.of_string "subnet-mask")
              ~query:Query.empty
              (Api_network.get_subnet_mask api);
            node ~doc:"Subnet mask" ~restrict:[ `Guest ] ~meth:`POST
              ~path:(Path.Format.of_string "subnet-mask")
              ~query:Query.empty
              (Api_network.set_subnet_mask api);
            node ~doc:"Gateway" ~meth:`GET
              ~path:(Path.Format.of_string "gateway")
              ~query:Query.empty
              (Api_network.get_gateway api);
            node ~doc:"Gateway" ~restrict:[ `Guest ] ~meth:`POST
              ~path:(Path.Format.of_string "gateway")
              ~query:Query.empty
              (Api_network.set_gateway api);
            node ~doc:"DHCP" ~meth:`GET
              ~path:(Path.Format.of_string "dhcp")
              ~query:Query.empty (Api_network.get_dhcp api);
            node ~doc:"DHCP" ~restrict:[ `Guest ] ~meth:`POST
              ~path:(Path.Format.of_string "dhcp")
              ~query:Query.empty (Api_network.set_dhcp api);
            node ~doc:"MAC address" ~meth:`GET
        ...
        make ~prefix:"receiver"
          [
            node ~doc:"Integrated IP receiver configuration" ~meth:`GET
              ~path:(Path.Format.of_string "config")
              ~query:Query.empty
            ...
            node ~doc:"FEC delay in milliseconds" ~meth:`GET
            ...
            node ~doc:"Enables or disables FEC encoding" ~restrict:[ `Guest ]
              ~meth:`POST
              ~path:(Path.Format.of_string "fec/enabled")
              ~query:Query.empty
              (Api_receiver.set_fec_enable api);
            node ~doc:"#FEC columns" ~meth:`GET
              ~path:(Path.Format.of_string "fec/columns")
              ~query:Query.empty
              (Api_receiver.get_fec_columns api);
            node ~doc:"#FEC rows" ~meth:`GET
              ~path:(Path.Format.of_string "fec/rows")
              ~query:Query.empty
              (Api_receiver.get_fec_rows api);
            node
              ~doc:
                "The time (in ms) that the device waits to receive 'late' IP \
                 packets (1..120ms)"
              ~meth:`GET
            ...
            node
              ~doc:
                "UDP port number at which the device listens for the incoming \
                 TS"
              ~meth:`GET
            ...
            node
              ~doc:
                "Multicast address at which the device listens for the \
                 incoming TS"
              ~meth:`GET
            ...
            node
              ~doc:
                "Counter that keeps the number of bitrate changes detected on \
                 this channel. During normal operation this counter should \
                 remain constant"
              ~meth:`GET
              ~path:(Path.Format.of_string "rate-change-counter")
              ~query:Query.empty
              (Api_receiver.get_rate_change_counter api);
            node ~doc:"Returns rate estimation mode" ~meth:`GET
              ~path:(Path.Format.of_string "rate-estimation-mode")
              ~query:Query.empty
              (Api_receiver.get_rate_estimation_mode api);
            node ~doc:"Sets rate estimation mode" ~restrict:[ `Guest ]
              ~meth:`POST
              ~path:(Path.Format.of_string "rate-estimation-mode")
              ~query:Query.empty
              (Api_receiver.get_rate_estimation_mode api);
            node
              ~doc:
                "Counter that keeps the number of IP jitter errors for this \
                 channel. A jitter error occurs when an incoming IP packet has \
                 a jitter that exceeds the Jitter Tolerance. During normal \
            ...
            node
              ~doc:
                "Counter that keeps the number of channel restarts. A channel \
                 is restarted only under adverse circumstances, like large \
                 jitter of IP packets in combination with packet loss. A \
                 channel restart causes a signal interruption for this channel \
                 of about 1 second. During normal operation this counter \
            ...
            node
              ~doc:
                "The channel's delay factor in us. Delay factor is a measure \
        ...
        make ~prefix:"asi-output"
          [
            node ~doc:"Size of TS packets" ~restrict:[ `Guest ] ~meth:`POST
              ~path:(Path.Format.of_string "packet-size")
              ~query:Query.empty
              (Api_asi_output.set_packet_size api);
            node ~doc:"Size of TS packets" ~meth:`GET
              ~path:(Path.Format.of_string "packet-size")
              ~query:Query.empty
              (Api_asi_output.get_packet_size api);
            node
              ~doc:
                "Physical port number of ASI output; always 1 for the DTM-3200"
              ~meth:`GET
        ...
        make ~prefix:"status"
          [
            node ~doc:"Overall status of a device" ~meth:`GET
