...
module Event = struct
  ...
  let get_config sock control =
    Api_js.Websocket.JSON.subscribe
      ...
      ~query:Query.empty control ip_receive_of_yojson sock
end
...
let get_addressing_method control =
  Api_http.perform ~meth:`GET
    ~path:
      Path.Format.("api/board" @/ Int ^/ "receiver/addressing-method" @/ empty)
    ~query:Query.empty control
    (ignore_env_bind (Lwt.return % map_err % meth_of_yojson))

let set_addressing_method meth control =
  Api_http.perform ~meth:`POST ~body:(meth_to_yojson meth)
    ~path:
      Path.Format.("api/board" @/ Int ^/ "receiver/addressing-method" @/ empty)
    ~query:Query.empty control
    (ignore_env_bind (Lwt.return % map_err % meth_of_yojson))
...
let get_ip_jitter_tolerance control =
  Api_http.perform ~meth:`GET
    ~path:
      Path.Format.(
        "api/board" @/ Int ^/ "receiver/ip-jitter-tolerance" @/ empty)
    ~query:Query.empty control
    (ignore_env_bind (Lwt.return % map_err % Util_json.Int.of_yojson))
...
let get_ip_lost_after_fec control =
  Api_http.perform ~meth:`GET
    ~path:
      Path.Format.("api/board" @/ Int ^/ "receiver/ip-lost-after-fec" @/ empty)
    ~query:Query.empty control
...
let get_ip_lost_before_fec control =
  Api_http.perform ~meth:`GET
    ~path:
      Path.Format.("api/board" @/ Int ^/ "receiver/ip-lost-before-fec" @/ empty)
    ~query:Query.empty control
    (ignore_env_bind (Lwt.return % map_err % Util_json.Int64.of_yojson))
...
let get_ip_to_output_delay control =
  Api_http.perform ~meth:`GET
    ~path:
      Path.Format.("api/board" @/ Int ^/ "receiver/ip-to-output-delay" @/ empty)
    ~query:Query.empty control
    (ignore_env_bind (Lwt.return % map_err % Util_json.Int.of_yojson))
...
let get_multicast_address control =
  Api_http.perform ~meth:`GET
    ~path:
      Path.Format.("api/board" @/ Int ^/ "receiver/multicast-address" @/ empty)
    ~query:Query.empty control
...
let get_rate_change_counter control =
  Api_http.perform ~meth:`GET
    ~path:
      Path.Format.(
        "api/board" @/ Int ^/ "receiver/rate-change-counter" @/ empty)
    ~query:Query.empty control
...
let get_rate_estimation_mode control =
  Api_http.perform ~meth:`GET
    ~path:
      Path.Format.(
        "api/board" @/ Int ^/ "receiver/rate-estimation-mode" @/ empty)
    ~query:Query.empty control
    (ignore_env_bind (Lwt.return % map_err % rate_mode_of_yojson))

let set_rate_estimation_mode mode control =
  Api_http.perform ~meth:`POST ~body:(rate_mode_to_yojson mode)
    ~path:
      Path.Format.(
        "api/board" @/ Int ^/ "receiver/rate-estimation-mode" @/ empty)
    ~query:Query.empty control
...
let get_jitter_error_counter control =
  Api_http.perform ~meth:`GET
    ~path:
      Path.Format.(
        "api/board" @/ Int ^/ "receiver/jitter-error-counter" @/ empty)
    ~query:Query.empty control
    (ignore_env_bind (Lwt.return % map_err % Util_json.Int32.of_yojson))
...
let get_lock_error_counter control =
  Api_http.perform ~meth:`GET
    ~path:
      Path.Format.("api/board" @/ Int ^/ "receiver/lock-error-counter" @/ empty)
    ~query:Query.empty control
    (ignore_env_bind (Lwt.return % map_err % Util_json.Int32.of_yojson))
