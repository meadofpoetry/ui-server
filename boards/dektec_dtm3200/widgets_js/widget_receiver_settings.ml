...
type event = [ `Mode of ip_receive | `State of Topology.state ]
    ...
    Custom
      {
        ...
        of_string =
          (fun x ->
            ...
            | Error (`Msg s) -> Error s
            | Ok x ->
                if Ipaddr.V4.is_multicast x then Ok x
...
let make_checkbox ~label checked =
  ...
  let en =
    Switch.make ~checked ~on_change:(fun _ -> Lwt.return @@ push ()) ()
  in
...
let make_method multicast value =
  ...
  (match value with Multicast -> show multicast | Unicast -> hide multicast);
  let event, push = React.E.create () in
  let validation =
    Select.(
      Custom
  ...
  let options =
    Select.native_options_of_values
      ~label:(function Unicast -> "Unicast" | Multicast -> "Multicast")
      validation [ Unicast; Multicast ]
  in
  let select =
    Select.make_native ~value ~label:"Метод"
      ~on_change:(fun x ->
        ( match x#value with
        | None -> ()
...
let make_multicast value =
  ...
  let mcast =
    Textfield.make
      ~on_input:(fun _ _ -> Lwt.return @@ push ())
      ~label:"Multicast адрес" ~value ~validation:multicast ()
  in
  (mcast, event)

class t (state : Topology.state) (mode : ip_receive) (control : int) =
  let en, e_en =
    make_checkbox ~label:"Включить приём TSoIP" mode.enable
  in
  let fec, e_fec =
    make_checkbox ~label:"Включить FEC" mode.fec_enable
  in
    ...
    method submit () : (unit, [ `Msg of string ]) Lwt_result.t =
      ...
      | Some mode ->
          let req =
            Http_receiver.(
              let ( >>= ) = Lwt_result.( >>= ) in
              set_enable mode.enable control >>= fun _ ->
              set_fec_enable mode.fec_enable control >>= fun _ ->
              set_multicast_address mode.multicast control >>= fun _ ->
              set_ip_to_output_delay mode.ip_to_output_delay control
              >>= fun _ ->
              set_addressing_method mode.addressing_method control >>= fun _ ->
              set_udp_port mode.udp_port control >>= fun _ ->
              set_rate_estimation_mode mode.rate_mode control >>= fun _ ->
              Lwt.return_ok ())
          in
