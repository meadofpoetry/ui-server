...
let make_method multicast value =
  ...
  let validation = Select.(
      Custom { to_string = (string_of_int % meth_to_enum)
             ; of_string = (fun s -> match int_of_string_opt s with
                   | None -> Error "Bad method value"
                   | Some x -> match meth_of_enum x with
                     | Some x -> Ok x
                     | None -> Error "Bad method value" )
             }) in
  let select = Select.make_native
      ~value
      ~label:"Метод"
      ~on_change:(fun x ->
          (match x#value with
          ...
          push ())
      ~items:(`Data [ Unicast, Some "Unicast"
    ...
    method submit () : (unit, string) Lwt_result.t =
      ...
      | Some mode ->
        let req = Http_receiver.(
            ...
            >>= fun _ -> set_fec_enable mode.fec_enable control
            >>= fun _ -> set_multicast_address mode.multicast control
            >>= fun _ -> set_ip_to_output_delay mode.ip_to_output_delay control
            >>= fun _ -> set_addressing_method mode.addressing_method control
            >>= fun _ -> set_udp_port mode.udp_port control
            >>= fun _ -> set_rate_estimation_mode mode.rate_mode control
