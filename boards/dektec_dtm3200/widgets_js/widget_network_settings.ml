...
type event = [ `Mode of nw | `State of Topology.state ]
...
let make_dhcp checked (inputs : 'a #Textfield.t list) =
  ...
  in
  let form =
    Form_field.make_of_widget ~label:"DHCP" ~align_end:true ~input:dhcp ()
  in
  (form, event)
...
class t (state : Topology.state) (mode : nw) (control : int) =
  ...
  let mask, e_mask =
    make_ip ~id:"mask" ~label:"Маска подсети" mode.mask
  in
      ...
      | Some mode ->
          let req =
            Http_network.(
              ...
              set_ip_address mode.ip_address control >>= fun ip_address ->
              set_subnet_mask mode.mask control >>= fun mask ->
              set_gateway mode.gateway control >>= fun gateway ->
              set_dhcp mode.dhcp control >>= fun dhcp ->
              if
                equal_nw mode { ip_address; mask; gateway; dhcp }
                && _dhcp = dhcp
              then Lwt.return_ok ()
              else reboot control)
          in
