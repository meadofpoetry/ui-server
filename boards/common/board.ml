    ...
    | (h : Topology.topo_port) :: tl ->
       ...
       | Topology.Board b ->
          try
            ...
            let eq = List.equal Stream.equal in
...
let merge_streams (boards : t Map.t)
  ...
  let find_cor_stream (s : Raw.t) (lst : Stream.t list React.signal) = (* use S.fmap `None*)
    ...
    let eq = Option.equal Stream.equal in
  ...
  let eq_lst = List.equal Stream.equal in
  let eq_opt = Option.equal Stream.equal in
  let eq =
    React.S.equal ~eq:eq_opt
    |> Pair.equal Raw.equal
    |> List.equal in
  React.(
    raw_streams
    |> S.map ~eq (lookup [] [])
    |> S.map ~eq:(List.equal @@ S.equal ~eq:eq_opt) (List.map snd)
