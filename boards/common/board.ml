...
type stream_handler =
  ...
  ; set :
      Stream.Table.setting list -> (unit, Stream.Table.set_error) Lwt_result.t
  ; constraints : constraints >

type tab = Netlib.Uri.Path.t * Api_template.template_props

type tab_tag = [ `Input of Topology.topo_input | `Stream ]
...
let merge_streams (boards : t Map.t) (topo : Topology.topo_board)
    (raw_streams : Stream.Raw.t list React.signal) : Stream.t list React.signal
    =
  ...
  let find_cor_stream (s : Raw.t) (lst : Stream.t list React.signal) =
    (* use S.fmap `None*)
    let map prev =
      ...
      | id, sid ->
          if equal_container_id id sid then Some { prev with typ = s.typ }
          else None
    in
    ...
    let rec find_map = function
      | [] -> None
      | x :: l' -> (
          match map x with Some _ as res -> res | None -> find_map l' )
    in
  ...
  let compose_hier (s : Raw.t) (parent : container_id) sms =
    List.find_opt
      (fun ((stream : Raw.t), _) -> equal_container_id stream.id parent)
      sms
    |> function
    | None -> `Await s
    | Some (_, v) ->
