...
let mer_to_string = function None -> "-" | Some x -> Printf.sprintf "%g dBm" x

let ber_to_string = function None -> "-" | Some x -> Printf.sprintf "%0.3e" x
...
let to_string (x : t) =
  Printf.sprintf
    "lock: %b, power: %s, MER: %s, BER: %s, frequency: %s, bitrate: %s" x.lock
    (power_to_string x.power) (mer_to_string x.mer) (ber_to_string x.ber)
