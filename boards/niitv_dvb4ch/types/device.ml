...
type standard = T2 [@value 1] | T | C [@@deriving yojson, eq, enum]

type bw = Bw8 [@value 1] | Bw7 | Bw6 [@@deriving yojson, eq, enum]

type channel = { bw : bw; freq : int; plp : int } [@@deriving yojson, eq]

type mode = { standard : standard; channel : channel } [@@deriving yojson, eq]

type mode_rsp = { mode : mode; hw_present : bool; lock : bool }
[@@deriving yojson]

type config = { mode : (int * mode) list; source : int } [@@deriving yojson, eq]

let standard_to_string ?(full = false) x =
  let suffix = match x with T2 -> "T2" | T -> "T" | C -> "C" in
...
let bw_to_string = function Bw8 -> "8 MHz" | Bw7 -> "7 MHz" | Bw6 -> "6 MHz"
...
let mode_rsp_to_string (x : mode_rsp) =
  Printf.sprintf "hw present: %b, lock: %b, mode: %s" x.hw_present x.lock
    (mode_to_string x.mode)

let info_to_string (x : info) =
  Printf.sprintf
    "serial: %d, hw version: %d, fpga version: %d, fw version: %d, asi: %b, \
     receivers: [%s]"
