...
let handlers (control : int) (api : Protocol.api) =
  ...
  [ merge
      ...
      ; make
          ...
          ; node
              ~doc:"Returns the receiving mode of the requested tuner"
              ~meth:`GET
              ~path:Path.Format.(Int ^/ "mode" @/ empty)
              ~query:Query.empty
              (Api_receiver.get_mode api)
          ; node
              ~doc:"Returns corresponding stream description, if any"
              ~meth:`GET
              ~path:Path.Format.(Int ^/ "stream" @/ empty)
              ~query:Query.empty
              (Api_receiver.get_stream api)
          ; node
              ~doc:"Returns measurements for the specified receiver"
              ~meth:`GET
              ~path:Path.Format.(Int ^/ "measurements" @/ empty)
              ~query:Query.empty
              (Api_receiver.get_measurements api)
          ; node
              ~doc:"Returns parameters for the specified receiver"
              ~meth:`GET
              ~path:Path.Format.(Int ^/ "parameters" @/ empty)
              ~query:Query.empty
              (Api_receiver.get_parameters api)
          ; node
              ~doc:"Returns PLP list for the specified receiver"
              ~meth:`GET
...
let ws (control : int) (api : Protocol.api) =
  ...
  [ merge
      ...
      [ make
          ...
          ; event_node
              ~doc:"Tuner receiving mode socket"
              ~path:(Path.Format.of_string "mode")
              ~query:Query.["id", (module List (Int))]
      ...
      ; make
          ...
          ; event_node
              ~doc:"Stream measurements socket"
              ~path:(Path.Format.of_string "measurements")
              ~query:Query.["id", (module List (Stream.ID))]
              (Api_stream.Event.get_measurements api)
          ; event_node
              ~doc:"Stream parameters socket"
              ~path:(Path.Format.of_string "parameters")
              ~query:Query.["id", (module List (Stream.ID))]
