...
let make_frequency ?(value : int option) (standard : standard option React.signal) =
  ...
  in
  let keydown =
    Lwt_js_events.keydowns input#input_element (fun e _ ->
    ...
    Lwt_js_events.seq_loop
      ...
      (fun _ _ ->
        ...
        | `Selected e ->
            ...
            apply_selected (Widget.event_detail e)##.item)
  in
...
let make_plp ?value (plps : int list React.signal) =
  ...
  let set_menu_toggle_disabled disabled =
    ...
    | None -> ()
    | Some x -> x#set_disabled disabled
  in
...
class t config state mode plps control =
  ...
  object
    ...
    method notify (event : event) = mode_box#notify event
  end

let make config state mode plps control = new t config state mode plps control
