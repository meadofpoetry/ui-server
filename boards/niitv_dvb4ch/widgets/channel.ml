type t = { chan : int; freq : int; spec : bool; name : string }

let mhz = 1_000_000

let to_string chan freq special =
  if special then Printf.sprintf "ТВК S%02d (%d МГц)" chan (freq / mhz)
  ...
  let lst : t list =
    let f = function
      ...
      | x ->
          ...
          { chan; freq; spec = false; name = to_string chan freq false }
    in
    let rec aux acc = function 0 -> acc | n -> aux (f n :: acc) (pred n) in
    aux [] 56
end
