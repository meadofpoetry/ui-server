...
module Event = struct
  ...
  let get_mode (api : Protocol.api) (ids : int list) _user =
    let to_yojson =
      Util_json.(List.to_yojson (Pair.to_yojson Int.to_yojson mode_to_yojson))
    in
    let event =
      match ids with
      | [] ->
...
let reset (api : Protocol.api) _user _body _env _state =
  api.channel Reset >>=? return_value % info_to_yojson
...
let get_mode (api : Protocol.api) (ids : int list) _user _body _env _state =
  ...
  let value =
    match ids with
    | [] -> config.mode
    | ids -> List.filter (fun (id, _) -> List.mem id ids) @@ config.mode
  in
  let to_yojson =
    Util_json.(List.to_yojson (Pair.to_yojson Int.to_yojson mode_to_yojson))
  in
