...
module Event = struct
  ...
  let to_json f (v : (int * 'a ts) list) =
    Util_json.(List.to_yojson (Pair.to_yojson Int.to_yojson (ts_to_yojson f)) v)

  let get_measurements (api : Protocol.api) (ids : int list) _user =
    let event =
      api.notifs.measures |> filter_if_needed ids |> E.map (to_json Measure.to_yojson)
    in
    Lwt.return event

  let get_parameters (api : Protocol.api) (ids : int list) _user =
    let event =
      api.notifs.params |> filter_if_needed ids |> E.map (to_json Params.to_yojson)
    in
    Lwt.return event

  let get_plp_list (api : Protocol.api) (ids : int list) _user =
    let event =
      api.notifs.plps |> filter_if_needed ids |> E.map (to_json Plp_list.to_yojson)
    in
    Lwt.return event
end

let to_json f x = Util_json.(Pair.to_yojson Int.to_yojson (ts_to_yojson f)) x
...
let set_mode (api : Protocol.api) id _user body _env _state =
  ...
  | Ok mode ->
      ...
      let set id value list =
        (id, value) :: List.filter (fun (id', _) -> id <> id') list
      in
...
let get_mode (api : Protocol.api) id _user _body _env _state =
  let value =
    List.find_opt (fun (id', _) -> id = id') @@ (React.S.value api.notifs.config).mode
  in
  let to_yojson =
    Util_json.(Option.to_yojson @@ Pair.to_yojson Int.to_yojson Device.mode_to_yojson)
  in
...
let get_plp_list (api : Protocol.api) (id : int) _user _body _env _state =
  api.channel (Request.Get_plp_list id) >>=? return_value % to_json Plp_list.to_yojson
