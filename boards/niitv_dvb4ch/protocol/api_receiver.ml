...
module Event = struct
  ...
  let filter_if_needed ids event =
    ...
    | ids ->
        E.fmap
          (list_to_option % List.filter (fun (id, _) -> List.mem id ids))
          event
  ...
  let get_plp_list (api : Protocol.api) (ids : int list) _user =
    let event =
      api.notifs.plps
      |> filter_if_needed ids
      |> E.map (to_json Plp_list.to_yojson)
    in
    Lwt.return event
end
...
let set_mode (api : Protocol.api) id _user body _env _state =
  ...
  | Ok mode ->
      api.channel (Set_mode (id, mode)) >>=? fun x ->
      api.kv#get >>= fun config ->
      let set id value list =
        (id, value) :: List.filter (fun (id', _) -> id <> id') list
      in
      let mode = set id mode config.mode in
      api.kv#set { config with mode } >>= fun () ->
      return_value @@ Device.mode_rsp_to_yojson @@ snd x

let get_mode (api : Protocol.api) id _user _body _env _state =
  let value =
    List.find_opt (fun (id', _) -> id = id')
    @@ (React.S.value api.notifs.config).mode
  in
  let to_yojson =
    Util_json.(
      Option.to_yojson @@ Pair.to_yojson Int.to_yojson Device.mode_to_yojson)
  in
...
let get_stream (api : Protocol.api) (id : int) _user _body _env _state =
  ...
  let stream =
    Api_stream.find_stream_by_receiver_id ~source_id:source id
