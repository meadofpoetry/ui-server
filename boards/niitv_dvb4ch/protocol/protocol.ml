...
let mode_to_stream (source_id : int) (id, ({ standard; channel } : Device.mode))
  ...
  let (bw : float) =
    match channel.bw with Bw8 -> 8. | Bw7 -> 7. | Bw6 -> 6.
  in
  let (info : Source.t) =
    match standard with
    | C -> DVB_C { freq; bw }
...
(** Converts absolute measured channel frequency value to frequency offset in
    measurements event. *)
...
let create (src : Logs.src) (sender : Cstruct.t -> unit Lwt.t) streams_conv
    (kv : Device.config Kv_v.rw) (control : int) (db : Db.t) =
  ...
  in
  Lwt_result.Infix.(
    Model.create src control measures db >>= fun model ->
    ...
    let loop =
      Fsm.start src sender req_queue rsp_queue kv set_state
        ...
        set_measures set_params set_plps
    in
