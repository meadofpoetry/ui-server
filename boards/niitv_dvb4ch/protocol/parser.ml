...
let parse_measures ?(timestamp = Ptime_clock.now ()) (buf : Cstruct.t) =
  ...
  try
    ...
    | Some lock ->
        let power =
          match (int_to_opt % get_rsp_measure_power) buf with
          | None -> None
          | Some x -> Some (Float.neg @@ (float_of_int x /. 10.))
        in
        let mer =
          match (int_to_opt % get_rsp_measure_mer) buf with
          | None -> None
          | Some x -> Some (float_of_int x /. 10.)
        in
        let ber =
          match (int32_to_opt % get_rsp_measure_ber) buf with
          | None -> None
          | Some x -> Some (Int32.to_float x /. (2. ** 24.))
        in
        let freq =
          match (int32_to_opt % get_rsp_measure_freq) buf with
          | None -> None
          | Some x -> Some (Int32.to_int x)
        in
        let bitrate =
          match (int32_to_opt % get_rsp_measure_bitrate) buf with
          | None -> None
          | Some x -> Some (Int32.to_int x)
        in
...
let parse_plp_list (buf : Cstruct.t) =
  try
    ...
    | Some lock ->
        let plps =
          match get_rsp_plp_list_plp_qty opts with
          | 0xFF -> []
          | n ->
              let iter =
                Cstruct.iter
...
let check_tag_start buf =
  try
    ...
    if tag = tag_start then Ok buf else Error (Bad_tag_start tag)
