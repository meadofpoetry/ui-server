...
let log_ok (type a) src (req : a Request.t) v =
  Logs.debug ~src (fun m ->
      m
        "Request \"%s\" succeeded. Response = %s"
...
let start
  ...
  let wait_probes =
    ...
    let rec f () =
      ...
      if !cnt_params = params_interval && !cnt_meas = meas_interval
      then (
        cnt_params := 0;
        cnt_meas := 0;
        Lwt.return `EBOTH)
  ...
  and detect () =
    ...
    >>= function
    | Error e ->
        Logs.err (fun m -> m "Error during detect step: %s" @@ Request.error_to_string e);
        restart ()
    | Ok devinfo ->
  ...
  and set_source_id tuners =
    ...
    | Ok id ->
        ...
        then initialize tuners config
          ...
          Logs.err (fun m ->
              m
                "Failure setting up source ID = %d. Device returned ID = %d"
                config.source
                id);
          restart ())
  and initialize tuners (config : Device.config) =
    let rec aux acc = function
      ...
      | ((id, _) as hd) :: tl -> (
          ...
          then (
            Logs.warn (fun m ->
                m
                  "Found configuration to be set for tuner %d, but the correponding \
    ...
    | Error _ -> restart ()
    | Ok x ->
        let mode = List.map (fun (id, ({mode; _} : Device.mode_rsp)) -> id, mode) x in
