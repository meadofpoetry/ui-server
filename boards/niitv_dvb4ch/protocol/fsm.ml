...
let log_ok (type a) src (req : a Request.t) v =
  Logs.debug ~src (fun m ->
      m "Request \"%s\" succeeded. Response = %s" (Request.to_string req)
        (Request.value_to_string req v))
...
let rec wait_ack stream =
  Lwt.pick [ Lwt_stream.next stream >>= Lwt.return_ok; sleep ack_timeout ]
  >>= function
...
let request (type a) ?timestamp (src : Logs.src)
    (stream : Request.msg Lwt_stream.t) (sender : Cstruct.t -> unit Lwt.t)
    (req : a Request.t) : (a, Request.error) result Lwt.t =
  let rec loop = function
    | 0 -> Lwt.return_error Request.Not_responding
    | n -> (
        sender @@ Serializer.serialize req >>= fun () ->
        Lwt_result.Infix.(wait_ack stream >>= wait_rsp ?timestamp stream req)
        >>= function
        | Ok x ->
            log_ok src req x;
            Lwt.return_ok x
        | Error e ->
            ...
            loop (pred n) )
  in
  Lwt_stream.junk_old stream >>= fun () -> loop 3

let start (src : Logs.src) (sender : Cstruct.t -> unit Lwt.t)
    (req_queue : api_msg Lwt_stream.t) (rsp_queue : Request.msg Lwt_stream.t)
  ...
  let wait_probes =
    ...
    let rec f () =
      Lwt_unix.sleep @@ float_of_int gcd >>= fun () ->
      cnt_meas := !cnt_meas + gcd;
      cnt_params := !cnt_params + gcd;
      if !cnt_params = params_interval && !cnt_meas = meas_interval then (
        cnt_params := 0;
        cnt_meas := 0;
        Lwt.return `EBOTH )
      else if !cnt_meas mod meas_interval = 0 then (
        cnt_meas := 0;
        Lwt.return `EMEAS )
      else if !cnt_params mod params_interval = 0 then (
        cnt_params := 0;
        Lwt.return `EPARAMS )
      else f ()
    in
  ...
  let rec restart () =
    ...
    Lwt_stream.junk_old rsp_queue >>= fun () ->
    Lwt_stream.junk_old req_queue >>= fun () ->
    Lwt_unix.sleep cooldown_timeout >>= detect
  and detect () =
    Logs.info (fun m -> m "Start of connection establishment...");
    set_state `Detect;
    let req = Request.Get_devinfo in
    request src rsp_queue sender req >>= function
    | Error e ->
        Logs.err (fun m ->
            m "Error during detect step: %s" @@ Request.error_to_string e);
        restart ()
    | Ok devinfo ->
  ...
  and set_source_id tuners =
    kv#get >>= fun config ->
    request src rsp_queue sender (Request.Set_src_id config.source) >>= function
    | Error _ -> restart ()
    | Ok id ->
        if id = config.source then initialize tuners config
        else (
          Logs.err (fun m ->
              m "Failure setting up source ID = %d. Device returned ID = %d"
                config.source id);
          restart () )
  and initialize tuners (config : Device.config) =
    let rec aux acc = function
      | [] -> Lwt.return_ok acc
      | ((id, _) as hd) :: tl -> (
          if not @@ List.mem id tuners then (
            Logs.warn (fun m ->
                m
                  "Found configuration to be set for tuner %d, but the \
                   correponding hardware is not present"
                  id);
            aux acc tl )
          else
            ...
            | Ok x -> aux (x :: acc) tl )
    in
    aux [] config.mode >>= function
    | Error _ -> restart ()
    | Ok x ->
        let mode =
          List.map (fun (id, ({ mode; _ } : Device.mode_rsp)) -> (id, mode)) x
        in
        kv#set { config with mode } >>= fun () ->
        Logs.info (fun m -> m "Initialization done!");
        set_state `Fine;
        idle tuners ()
  and idle ?timer tuners () =
    let timer = match timer with Some x -> x | None -> wait_probes () in
    let wait_msg = Lwt_stream.next req_queue >>= fun x -> Lwt.return @@ `M x in
    Lwt.choose [ wait_msg; timer ] >>= function
    ...
    | (`EMEAS | `EPARAMS | `EBOTH) as probes -> (
        ...
        let t =
          Lwt_result.Infix.(
            match probes with
            | `EMEAS ->
                pull_measurements tuners >>= fun x ->
                set_measures x;
                Lwt.return_ok ()
            | `EPARAMS ->
                pull_parameters tuners >>= fun prms ->
                pull_plps tuners >>= fun plps ->
                let step = React.Step.create () in
                ...
                Lwt.return_ok ()
            | `EBOTH ->
                pull_measurements tuners >>= fun meas ->
                pull_parameters tuners >>= fun prms ->
                pull_plps tuners >>= fun plps ->
                let step = React.Step.create () in
                set_params ~step prms;
