    ...
    try
      ...
      let rec aux n size =
        ...
        | -1 -> (match !@(foreign_value "errno" int) with
                 ...
                 | x -> Error (`Other x))
        | _  -> Ok (Cstruct.to_string @@ fst @@ Cstruct.split buf str_size)
      in
...
let get_encoding (text:Cstruct.t) =
  ...
  try
    let firstbyte = Cstruct.get_uint8 text 0 in
...
let fold_multibyte (text:Cstruct.t) =
  let iter = Cstruct.iter (fun b -> if Cstruct.len b >= 2 && (not @@ Char.equal (Cstruct.get_char b 0) '\000')
...
let fold_singlebyte (text:Cstruct.t) =
  let iter = Cstruct.iter (fun b -> if Cstruct.len b >= 1 && (not @@ Char.equal (Cstruct.get_char b 0) '\000')
