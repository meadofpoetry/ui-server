...
let typ : (ID.t * T2mi_info.t Time.timespan) Caqti_type.t =
  ...
  Types.custom
    ...
    ~encode:(fun (id, ({ from
      ...
      let l1_pre, l1_post_conf = match data.l1 with
        | None -> None, None
        | Some x ->
           let to_string = Yojson.Safe.to_string in
           Some (to_string @@ l1_pre_to_yojson x.l1_pre),
           Some (to_string @@ l1_post_conf_to_yojson x.l1_post_conf) in
      Ok (ID.to_db id,
          (data.t2mi_pid,
           (sid,
            (packets,
    ...
    ~decode:(fun (id,
      ...
      let packets = Json.(List.of_yojson Int.of_yojson)
                    @@ Yojson.Safe.from_string packets in
