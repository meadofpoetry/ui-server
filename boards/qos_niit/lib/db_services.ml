    ...
    ~encode:(fun (id, ({ from; till; data = (sid, data) } : Service.t timespan)) ->
      ...
      Ok (((ID.to_db id, sid),
          ...
          ((data.dscr, data.dscr_list, data.eit_schedule, data.eit_pf),
           (data.free_ca_mode, data.running_status,
            data.service_type, data.service_type_list),
           (elements, from, till))))
    ~decode:(fun (((id, sid),
                   (name, provider_name),
                  ...
                  ((dscr, dscr_list, eit_schedule, eit_pf),
                   ...
                   (elements, from, till))) ->
      match Common.Json.(List.of_yojson Int.of_yojson)
            @@ Yojson.Safe.from_string elements with
      | Error e -> Error e
      | Ok elements ->
         Ok (let (data : Service.info) =
               { name
               ; provider_name
               ; pmt_pid
               ; pcr_pid
