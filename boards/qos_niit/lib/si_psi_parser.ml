...
module Descriptor = struct
        ...
        if Bitstring.length x = 0 then acc
              ...
              | {| language_code : 24
                 ...
                 let nodes =
                   ...
                   ; to_node ~parsed:typ ~offset:(off_1 +offset) 8 "audio_type" (Hex (Int audio))
                   ]
                 in
  ...
  module Copyright = struct
    ...
    let decode bs off =
      ...
      | {| copyright_id : 32
         ...
         in
         parse_bytes ~offset:(off + off_1) node rest "additional_copyright_info"
 ...
 (* FIXME *)
  ...
  module Transport_profile = struct
    ...
    let decode bs off =
      ...
      | {| transport_profile : 8
         ...
         in
         parse_bytes ~offset:(off + off_1) node rest "private_data"
  ...
  module Network_name = struct
    ...
    let decode bs off =
      match Text_decoder.decode @@ Bitstring.to_cstruct bs with
        | Ok parsed -> [to_node ~parsed ~offset:off (Bitstring.length bs) "network name" (Bits (Int 1))]
        | Error _   -> parse_bytes ~offset:off [] bs "char"
  ...
  module Bouquet_name = struct
    ...
    let decode bs off =
      match Text_decoder.decode @@ Bitstring.to_cstruct bs with
        | Ok parsed -> [to_node ~parsed ~offset:off (Bitstring.length bs) "bouquet name" (Bits (Int 1))]
        | Error _   -> parse_bytes ~offset:off [] bs "char"
      ...
      | {| service_type     : 8
         ...
         let serv = match Text_decoder.decode @@ Bitstring.to_cstruct service with
           | Ok parsed -> [to_node ~parsed ~offset:(off + off_3) (length_2 * 8)
                             "service" (Bits (Int 1))]
           | Error _ -> parse_bytes ~offset:(off + off_4) [] service "char"
         in
  ...
  module Country_availability = struct
    ...
    let decode bs off =
      ...
      | {| country_avail_flag : 1
         ...
         in
         parse_bytes ~bytes:3 ~offset:(off + off_2) nodes rest "country_code"
  ...
  module CA_identifier = struct
    ...
    let decode bs off =
      parse_bytes ~bytes:2 ~offset:off [] bs "CA_system_id"
  ...
  module Frequency_list = struct
    ...
    let decode bs off =
      ...
      | {| rfu         : 6
         ...
         in
         parse_bytes ~bytes:4 ~offset:(off + off_2) nodes rest "centre_frequency"
    ...
    let decode bs off =
      ...
      | {| data_broadcast_id : 16
         ...
         let nodes = (parse_bytes ~offset:(off + off_3) nodes_1 selector_bytes "selector_byte")
                     @ [ to_node ~parsed ~offset:(off + off_4) 24
  ...
  module Service_availability = struct
    ...
    let decode bs off =
      ...
      | {| availability_flag : 1
         ...
         in
         parse_bytes ~bytes:2 ~offset:(off + off_2) nodes rest "cell_id"
