  ...
  | 0x01 ->
    begin match%bitstring x with
      | {| bouquet_id : 16
         ...
         |} ->
        [ Node.make ~offset 16 "bouquet_id" (Hex (Int bouquet_id))], rest, (offset + off_1)
    end
  | 0x02 | 0x03 ->
    begin match%bitstring x with
      | {| on_id : 16
        ...
        ; Node.make ~offset:(offset + off_1) 16 "transport_stream_id" (Hex (Int ts_id))
  ...
  | 0x04 ->
    begin match%bitstring x with
      | {| on_id : 16
        ...
        ; Node.make ~offset:(offset + off_1) 16 "transport_stream_id" (Hex (Int ts_id))
        ; Node.make ~offset:(offset + off_2) 16 "service_id" (Hex (Int sv_id))
        ; Node.make ~offset:(offset + off_3) 16 "event_id" (Hex (Int ev_id))], rest, (offset + off_4)
    end
  | _ -> assert false

let rec f_2 off x =
  if Bitstring.bitstring_length x = 0 then [] else
    ...
    | {| rfu                : 2
      ...
      let nodes =
        ...
        ; Node.make ~offset:(off + off_1) 6 "elementary_cell_id" (Dec (Int elementary_cell_id)) ]
      in
  ...
  if Bitstring.bitstring_length x = 0 then [] else
    ...
    | {| logical_cell_id : 6
      ...
      let nodes =
        ...
        ; Node.make ~offset:(off + off_3) 8 "elementary_cell_field_length" (Dec (Int elem_len)) ]
      in
      let nodes = nodes @ f_2 (off + off_4) elem_cell_field in
      let cli_parsed = parse_cli cli in
      let cli_node = [Node.make ~parsed:cli_parsed ~offset:(off + off_5) 8
...
let parse bs off = match%bitstring bs with
