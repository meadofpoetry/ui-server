...
let rec parse bs off =
  if Bitstring.bitstring_length bs = 0 then []
    ...
    | {| cont_lvl1 : 4
      ...
      let nodes =
        [ Node.make ~parsed:lvl_1 ~offset:off 4 "content_nibble_level_1" (Hex (Int cont_lvl1))
        ; Node.make ~parsed:lvl_2 ~offset:(off + off_1) 4 "content_nibble_level_2" (Hex (Int cont_lvl2))
