...
let decode_handover_linkage bs off =
  ...
  | {| 0x00            : 4
    ...
    ; Node.make ~offset:(off + off_1) 3  "reserved_future_use" (Bits (Int rfu))
    ; Node.make ~offset:(off + off_2) 1  "origin_type" (Bits (Bool origin_type))
    ; Node.make ~offset:(off + off_3) 16 "initial_service_id" (Dec (Int init_service_id)) ]
...
let decode_extended_event_linkage ~linkage bs off =
    ...
    if Bitstring.bitstring_length x = 0 then []
      ...
      | {| target_event_id : 16
        ...
        let nodes =
          ...
          ; Node.make ~offset:(off + off_1) 1 "target_listed" (Bits (Bool target_listed))
          ; Node.make ~offset:(off + off_2) 1 "event_simulcast" (Bits (Bool event_simulcast))
          ; Node.make ~parsed:(parse_link_type ~linkage ~link:link_type)
              ~offset:(off + off_3) 2 "link_type" (Hex (Int link_type))
          ; Node.make ~parsed:(parse_target_id target_id_type) ~offset:(off + off_4)
              2 "target_id_type" (Hex (Int target_id_type))
          ; Node.make ~offset:(off + off_5) 1 "original_network_id_flag" (Bits (Bool on_id_flag))
          ; Node.make ~offset:(off + off_6) 1 "service_id_flag" (Bits (Bool service_id_flag))]
        in
  ...
  | {| loop_length    : 8
    ...
    ; Node.make ~offset:(off + off_1) (loop_length * 8) "extended_event_linkage"
        (List extended_linkage)
    ; Node.make ~offset:(off + off_1)
        (Bitstring.bitstring_length rest) "private_data_bytes" (List private_data)
    ]
...
let parse bs off =
  ...
  | {| ts_id                : 16
    ...
    ; Node.make ~offset:(off + off_5)
        (Bitstring.bitstring_length rest) "private_data_bytes" (List bytes)
    ]
  | {| ts_id              : 16
     ; on_id              : 16 : save_offset_to (off_1)
     ; service_id         : 16 : save_offset_to (off_2)
  ...
  | {| ts_id        : 16
    ...
    ; Node.make ~offset:(off + off_1) 16 "original_network_id" (Hex (Int on_id))
    ; Node.make ~offset:(off + off_2) 16 "service_id" (Hex (Int service_id))
    ; Node.make ~parsed:(parse_linkage_type linkage_type)~offset:(off + off_3) 8
