  ...
  | {| time_slicing      : 1
    ...
    ->
      ...
      [
        ...
        Node.make ~offset:(off + off_2) 2 "reserved_for_future_use"
          (Bits (Int rfu));
        Node.make ~offset:(off + off_3) 3 "frame_size" (Dec (Int frame_size));
        Node.make ~offset:(off + off_4) 8 "max_burst_duration"
          (Dec (Int max_burst_dur));
        Node.make ~offset:(off + off_5) 4 "max_average_rate"
          (Dec (Int max_aver_rate));
        Node.make ~offset:(off + off_6) 4 "time_slice_fec_id"
          (Dec (Int time_slice_fec_id));
        Node.make ~offset:(off + off_7)
          (Bitstring.bitstring_length rest)
          "id_selector" (String id_selector);
      ]
