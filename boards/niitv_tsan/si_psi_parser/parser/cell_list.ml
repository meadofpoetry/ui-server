  ...
  if Bitstring.bitstring_length x = 0 then []
    ...
    | {| cell_id_ext : 8
      ...
      let nodes =
        ...
        ; Node.make ~parsed:lat ~offset:(off + off_1) 16
            "subcell_latitude" (Dec (Int subcell_lat))
        ; Node.make ~parsed:lon ~offset:(off + off_2) 16
  ...
  if Bitstring.bitstring_length bs = 0 then []
    ...
    | {| cell_id      : 16
      ...
      let nodes =
        ...
        ; Node.make ~offset:(off + off_5) 8 "subcell_info_loop_length" (Dec (Int loop_length))
        ; Node.make ~offset:(off + off_6) (loop_length * 8) "Subcells" (List snd_loop) ]
      in
