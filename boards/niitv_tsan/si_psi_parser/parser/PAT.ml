...
let rec parse_programs off x =
  let to_name = function 0 -> "network_PID" | _ -> "program_map_PID" in
  if Bitstring.bitstring_length x = 0 then []
    ...
    | {| id       : 16
      ...
      ->
        ...
        in
        let name =
          match id with 0 -> "network" | x -> Printf.sprintf "program %d" x
        in
        let node =
          Node.make ~offset:off (parsed_length nodes) name (List nodes)
        in
