...
let parse bs off =
  ...
  | {| lang_code  : 24 : bitstring
    ...
    ->
      ...
      [
        Node.make ~parsed:parsed_code ~offset:off 24 "ISO_639_language_code"
          (Bits (Int lang_code));
        Node.make ~offset:(off + off_1) 8 "event_name_length"
          (Dec (Int length_1));
        Node.make ~offset:(off + off_2) (length_1 * 8) "event_name"
          (String name);
