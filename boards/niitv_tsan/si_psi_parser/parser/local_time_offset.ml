...
let rec parse bs off =
  if Bitstring.bitstring_length bs = 0 then []
    ...
    | {| country_code     : 24 : bitstring
      ...
      let time_ch = match Date_time.parse_timestamp time_of_change with
        | Some x -> Node.Time x
      ...
      let nt_offset =
        ...
        | None -> match%bitstring time_of_change with
          | {| i : 40 |} -> Node.Dec (Uint64  i) in
      let nodes =
        [ Node.make ~parsed:p_code ~offset:off 24 "country_code" (Bits (Int country_code))
        ...
        ; Node.make ~offset:(off + off_3) 1  "local_time_offset_polarity" (Bits (Bool offset_pol))
