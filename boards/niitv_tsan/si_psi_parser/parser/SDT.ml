    ...
    (match%bitstring x with
     | {| service_id     : 16
       ...
       let nodes =
         ...
         ; Node.make ~offset:(off_1 + off)  6  "reserved_fuure_use" (Bits (Int rfu))
         ; Node.make ~offset:(off_2 + off)  1  "EIT_schedule_flag" (Bits (Bool schedule_flag))
         ; Node.make ~offset:(off_3 + off)  1  "EIT_present_following_flag" (Bits (Bool present_flag))
         ; Node.make ~offset:(off_4 + off)  3  "runnning_status" (Hex (Int running_status))
         ; Node.make ~offset:(off_5 + off)  1  "free_CA_mode" (Bits (Bool free_ca_mode))
         ; Node.make ~offset:(off_6 + off)  12 "descriptors_loop_length" (Hex (Int length))
         ; Node.make ~offset:(off_7 + off) (length * 8) "descriptors" (List dscrs)
         ]
       in
       let parsed = parse_status running_status in
       let service_name = Printf.sprintf "service %d" service_id in
       let node = Node.make ~parsed ~offset:off (parsed_length nodes) service_name (List nodes) in
...
let parse bs =
  ...
  | {| header           : 24 : bitstring
    ...
    let nodes =
      ...
      ; Node.make ~offset:off_9 (services_length off_8) "services" (List services)
