  ...
  | {| profile_space        : 2
    ...
    ->
      [
        ...
        Node.make ~offset:(off + off_3) 32 "profile_compatibility_indication"
          (Hex (Int32 pr_com_ind));
        Node.make ~offset:(off + off_4) 1 "progressive_source_flag"
          (Bits (Bool prog_src_flag));
        Node.make ~offset:(off + off_5) 1 "interlaced_source_flag"
          (Bits (Bool interlaced_src_flag));
        Node.make ~offset:(off + off_6) 1 "non_packed_constraint_flag"
          (Bits (Bool np_const_flag));
        Node.make ~offset:(off + off_7) 1 "frame_only_constraint_flag"
          (Bits (Bool fo_const_flag));
        Node.make ~offset:(off + off_8) 44 "reserved_zero_44bits"
          (Bits (Int64 res_zero_44bits));
        Node.make ~offset:(off + off_9) 8 "level_idc" (Hex (Int level_idc));
        Node.make ~offset:(off + off_10) 1 "temporal_layer_subset_flag"
          (Bits (Bool true));
        Node.make ~offset:(off + off_11) 1 "HEVC_still_present_flag"
          (Bits (Bool hevc_still_flag));
        Node.make ~offset:(off + off_12) 1 "HEVC_24hr_picture_present_flag"
          (Bits (Bool hevc_24_flag));
        Node.make ~offset:(off + off_13) 1 "sub_pic_hrd_params_not_present_flag"
          (Bits (Bool sph_flag));
        Node.make ~offset:(off + off_14) 4 "reserved" (Bits (Int reserved_1));
        Node.make ~offset:(off + off_15) 3 "temporal_id_min"
          (Dec (Int temp_id_min));
        Node.make ~offset:(off + off_16) 5 "reserved" (Bits (Int reserved_2));
        Node.make ~offset:(off + off_17) 3 "temporal_id_max"
          (Dec (Int temp_id_max));
