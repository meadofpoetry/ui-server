      ...
      | {| reserved : 6
        ...
        ->
          [
            ...
            Node.make ~offset:(off + off_1) 18 "scrambling_sequence_index"
              (Dec (Int ssi));
            Node.make ~offset:(off + off_2) 8 "input_stream_identifier"
              (Dec (Int isi));
          ] )
  | true, false -> (
      match%bitstring x with
...
let parse bs off =
  ...
  | {| sss       : 1
    ...
    ->
      let nodes =
        [
          Node.make ~offset:off 1 "scrambling_sequence_selector"
            (Bits (Bool sss));
          Node.make ~offset:(off + off_1) 1 "multiple_input_stream_flag"
            (Bits (Bool misf));
          Node.make ~offset:(off + off_2) 1 "backwards_compatibility_indicator"
            (Bits (Bool backw_ind));
          Node.make ~offset:(off + off_3) 5 "reserved_future_use"
            (Bits (Int rfu));
        ]
      in
