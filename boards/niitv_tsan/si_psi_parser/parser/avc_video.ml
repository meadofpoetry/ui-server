  ...
  | {| profile_idc          : 8
    ...
    ->
      [
        ...
        Node.make ~offset:(off + off_9) 1 "AVC_still_present"
          (Bits (Bool avc_still_present));
        Node.make ~offset:(off + off_10) 1 "AVC_24_hour_picture_flag"
          (Bits (Bool avc_24_hr_pict_flag));
        Node.make ~offset:(off + off_11) 1 "Frame_Packing_SEI_not_present_flag"
          (Bits (Bool fr_flag));
