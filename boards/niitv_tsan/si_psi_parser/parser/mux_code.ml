  ...
  if Bitstring.bitstring_length x = 0 then []
    ...
    | {| channel : 8
      ...
      ->
        ...
        in
        let channel_name =
          Printf.sprintf "Channel %s" (string_of_int channel)
        in
      ...
      ->
        let nodes =
          [
            ...
            Node.make ~offset:(offset + off_1) 3 "repetitionCount"
              (Dec (Int rep_count));
          ]
        in
        ...
        let node =
          Node.make ~offset (8 + (slot_count * 16)) slot_name (List channels)
        in
    ...
    ->
      let nodes =
        [
          ...
          Node.make ~offset:(off + off_3) 8 "substructureCount"
            (Dec (Int sub_count));
        ]
      in
