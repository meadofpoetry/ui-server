  ...
  if Bitstring.bitstring_length bs = 0 then []
    ...
    | {| item_descr_length : 8
      ...
      let item =
        ...
        | Error _ -> "Failed to decode" in
      let nodes =
        [ Node.make ~offset:off 8 "item_description_length" (Dec (Int item_descr_length))
        ; Node.make ~offset:(off + off_1) (item_descr_length * 8) "item_description" (String item_descr)
...
let parse bs off =
  ...
  | {| desc_num      : 4
    ...
    ; Node.make ~parsed:parsed_code ~offset:(off + off_2) 24 "ISO_639_language_code"
        (Bits (Int lang_code))
