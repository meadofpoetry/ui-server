...
let parse_ref_type rf =
  ...
  | 1 ->
      "Announcement is broadcast in a separate audio stream that is part of \
       the service"
  | 2 ->
      "Announcement is broadcast by means of a different service within the \
       same transport system"
  | 3 ->
      "Announcement is broadcast by means of a different service within a \
  ...
  if Bitstring.bitstring_length x = 0 then []
    ...
    | {| ann_type             : 4
      ...
      ->
        ...
        let ref_typ = parse_ref_type 1 in
        (* FIXME parsing reference type*)
        let nodes =
          [
            Node.make ~parsed:ann_typ ~offset:off 4 "announcement_type"
              (Bits (Int ann_type));
            Node.make ~offset:(off + off_1) 1 "reserved_future_use"
              (Bits (Bool rfu));
            Node.make ~parsed:ref_typ ~offset:(off + off_2) 3 "reference_type"
              (Hex (Int 0x01))
            (* FIXME parsing reference type*);
            Node.make ~offset:(off + off_3) 16 "original_network_id"
              (Hex (Int original_network_id));
            Node.make ~offset:(off + off_4) 16 "transport_stream_id"
              (Hex (Int transport_stream_id));
            Node.make ~offset:(off + off_5) 16 "service_id"
              (Hex (Int service_id));
            Node.make ~offset:(off + off_6) 8 "component_tag"
        ...
        let nodes =
          [
            ...
            Node.make ~offset:(off + off_1) 1 "reserved_future_use"
              (Bits (Bool rfu));
            Node.make ~parsed:ref_typ ~offset:(off + off_2) 3 "reference_type"
              (Hex (Int reference_type));
          ]
        in
        let type_name =
          Printf.sprintf "Type %s" (string_of_int ann_type) ^ ann_typ
        in
