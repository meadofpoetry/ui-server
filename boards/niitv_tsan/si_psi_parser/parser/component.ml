...
let parse bs off =
  ...
  | {| stream_content_ext : 4
    ...
    let text = match Text_decoder.decode @@ Util.Bitstring.to_cstruct rest with
      | Ok s -> s
    ...
    ; Node.make ~parsed ~offset:(off + off_2) 8 "component_type" (Hex (Int component_type))
    ...
    ; Node.make ~offset:(off + off_5)
        (Bitstring.bitstring_length bs - 48) "text" (String text)
    ]
