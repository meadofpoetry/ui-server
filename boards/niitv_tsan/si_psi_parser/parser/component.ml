...
let parse ~stream_content ~stream_content_ext ~component_type =
  ...
  | 0x1 -> (
      ...
      | 0x0A ->
          "MPEG-2 high definition video, 16:9 aspect ratio with pan vectors, \
           25 Hz "
      | 0x0B ->
          "MPEG-2 high definition video, 16:9 aspect ratio without pan \
           vectors, 25 Hz"
  ...
  | 0x2 -> (
      ...
      | 0x42 ->
          "receiver-mix supplementary audio as per annex E of ETSI TS 101 154"
  ...
  | 0x3 -> (
      ...
      | 0x15 ->
          "DVB subtitles (normal) with plano-stereoscopic disparity for \
           display on a high definition monitor"
      | 0x20 ->
          "DVB subtitles (for the hard of hearing) with no monitor aspect \
           ratio criticality"
      | 0x21 ->
          "DVB subtitles (for the hard of hearing) for  display on 4:3 aspect \
           ratio monitor"
      | 0x22 ->
          "DVB subtitles (for the hard of hearing) for display on 16:9 aspect \
           ratio monitor"
      | 0x23 ->
          "DVB subtitles (for the hard of hearing) for display on 2.21:1 \
           aspect ratio monitor"
      | 0x24 ->
          "DVB subtitles (for the hard of hearing) for display on a high \
           definition monitor"
      | 0x25 ->
          "DVB subtitles (for the hard of hearing) with plano-stereoscopic \
  ...
  | 0x4 -> (
      ...
      | x when x >= 0x80 && x <= 0xFF ->
          "reserved for enhanced AC-3 audio modules"
      | _ -> assert false )
  | 0x5 -> (
      match component_type with
      ...
      | 0x04 ->
          "H.264/AVC standard definition video, > 16:9 aspect ratio, 25 Hz"
      ...
      | 0x08 ->
          "H.264/AVC standard definition video, > 16:9 aspect ratio, 30 Hz"
      ...
      | 0x80 ->
          "H.264/AVC plano-stereoscopic frame compatible high definition \
      ...
      | 0x82 ->
          "H.264/AVC plano-stereoscopic frame compatible high definition \
           video, 16:9 aspect ratio, 30 Hz, Side-by-Side"
      | 0x83 ->
          "H.264/AVC stereoscopic frame compatible high definition video, 16:9 \
           aspect ratio, 30 Hz, Top-and-Bottom"
      | 0x84 ->
          "H.264/MVC dependent view, planostereoscopic service compatible video"
      | x
        when x = 0x00
             || x = 0x02
             || x = 0x06
  ...
  | 0x6 -> (
      ...
      | 0x42 ->
          "HE AAC receiver-mix supplementary audio as per annex E of ETSI TS \
           101 154 [9]"
      ...
      | 0x46 ->
          "HE AAC v2 receiver-mix supplementary audio as per annex E of ETSI \
           TS 101 154 [9]"
      | 0x47 ->
  ...
  | 0x9 -> (
      ...
      | 0x1 -> (
          ...
          | 0x05 ->
              "AC-4 main audio, multichannel, dialogue enhancement enabled"
          | 0x06 ->
              "AC-4 broadcast-mix audio description, mono, for the visually \
               impaired"
          | 0x07 ->
              "AC-4 broadcast-mix audio description, mono, for the visually \
               impaired, dialogue enhancement enabled"
          | 0x08 ->
              "AC-4 broadcast-mix audio description, stereo, for the visually \
               impaired"
          | 0x09 ->
              "AC-4 broadcast-mix audio description, stereo, for the visually \
               impaired, dialogue enhancement enabled"
          | 0x0A ->
              "AC-4 broadcast-mix audio description, multichannel, for the \
               visually impaired"
          | 0x0B ->
              "AC-4 broadcast-mix audio description, multichannel, for the \
               visually impaired, dialogue enhancement enabled"
          | 0x0C ->
              "AC-4 receiver-mix audio description, mono, for the visually \
               impaired"
          | 0x0D ->
              "AC-4 receiver-mix audio description, stereo, for the visually \
               impaired"
...
let parse bs off =
  ...
  | {| stream_content_ext : 4
    ...
    ->
      ...
      in
      [
        Node.make ~offset:off 4 "stream_content_ext"
          (Hex (Int stream_content_ext));
        Node.make ~offset:(off + off_1) 4 "stream_content"
          (Hex (Int stream_content));
        Node.make ~parsed ~offset:(off + off_2) 8 "component_type"
          (Hex (Int component_type));
        Node.make ~offset:(off + off_3) 8 "component_tag"
          (Hex (Int component_tag));
        Node.make ~parsed:parsed_lang ~offset:(off + off_4) 24
          "ISO_639_language_code" (Hex (Int lang_code));
        Node.make ~offset:(off + off_5)
