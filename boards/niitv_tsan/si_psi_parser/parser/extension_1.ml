...
let parse bs off =
  ...
  | {| 0x02 :  8
     ...
     |} ->
      ...
      let nodes =
        [
          Node.make ~parsed:(parse_tag 2) ~offset:off 8
            "extension_descriptor_tag" (Hex (Int 0x02));
  ...
  | {| 0x03 : 8
     ...
     |} ->
      let node =
        [
          Node.make ~parsed:(parse_tag 3) ~offset:off 8
            "extension_descriptor_tag" (Hex (Int 0x03));
        ]
      in
  ...
  | {| ext_desc_tag : 8
    ...
    ->
      let node =
        Node.make ~parsed:(parse_tag ext_desc_tag) ~offset:off 8
          "extension_descriptor_tag" (Hex (Int ext_desc_tag))
      in
