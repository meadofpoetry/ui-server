...
let parse bs off =
  ...
  | {| profile_and_level   : 16
    ...
    let nodes =
      ...
      ; Node.make ~offset:(off + off_1)  32 "horizontal_size" (Dec (Int32 horizontal_size))
      ...
      ; Node.make ~offset:(off + off_4)  32 "max_buffer_size" (Dec (Int32 max_buffer_size))
      ...
      ; Node.make ~offset:(off + off_7)  8  "color_specification" (Hex (Int color_specification))
      ; Node.make ~offset:(off + off_8)  1  "still_mode" (Bits (Bool still_mode))
      ; Node.make ~offset:(off + off_9)  1  "interlaced_video" (Bits (Bool interlaced_video))
