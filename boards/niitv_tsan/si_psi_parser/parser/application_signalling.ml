  ...
  if Bitstring.bitstring_length bs = 0 then []
    ...
    | {| rfu_1       : 1
      ...
      let node_list =
        ...
        ; Node.make ~offset:(off + off_1) 15 "application_type" (Hex (Int app_type))
        ; Node.make ~offset:(off + off_2) 3  "reserved_future_use" (Bits (Int rfu_2))
        ; Node.make ~offset:(off + off_3) 5  "AIT_version_number" (Dec (Int ait_ver_num)) ]
      in
      let node = [Node.make ~offset:off 24 "application signalling" (List node_list)] in
