        ...
        let nodes =
          [
            ...
            Node.make ~offset:(off_1 + off) 16 "original_network_id"
              (Hex (Int on_id));
            Node.make ~offset:(off_2 + off) 16 "service_id"
              (Hex (Int service_id));
            Node.make ~offset:(off_3 + off) 16 "event_id" (Hex (Int event_id));
            Node.make ~offset:(off_4 + off) 5 "reserved_future_use"
              (Bits (Int rfu));
            Node.make ~offset:(off_5 + off) 3 "running_status"
              (Hex (Int status));
          ]
        in
        let event_name = Printf.sprintf "event %d" event_id in
        let node =
          Node.make ~offset:off (parsed_length nodes) event_name (List nodes)
        in
