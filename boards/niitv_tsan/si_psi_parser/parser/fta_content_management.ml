...
let parse_access access =
  ...
  | 3 ->
      "Redistribution over the Internet is not allowed with the following \
       exception: Redistribution over the Internet within a managed domain is \
       enabled after a specified long (possibly indefinite) period of time."
  | _ -> assert false
  ...
  | {| user_defined    : 1
    ...
    ->
      ...
      [
        ...
        Node.make ~offset:(off + off_2) 1 "do_not_scramble"
          (Bits (Bool do_not_scramble));
        Node.make ~parsed ~offset:(off + off_3) 2
          "control_remote_access_over_internet" (Bits (Int cont_remote_acc));
        Node.make ~offset:(off + off_4) 1 "do_not_apply_revocation"
          (Bits (Bool do_not_apply));
      ]
