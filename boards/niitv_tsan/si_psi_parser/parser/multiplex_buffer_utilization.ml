...
let parse bs off =
  ...
  | {| bound_valid_flag : 1
    ...
    ->
      [
        Node.make ~offset:off 1 "bound_valid_flag"
          (Bits (Bool bound_valid_flag));
        Node.make ~offset:(off_1 + off) 15 "LTW_offset_lower_bound"
          (Dec (Int lower_bound));
        Node.make ~offset:(off_2 + off) 1 "reserved" (Bits (Bool reserved));
        Node.make ~offset:(off_3 + off) 15 "LTW_offset_upper_bound"
          (Dec (Int upper_bound));
      ]
