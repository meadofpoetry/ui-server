  ...
  | {| reserved_1           : 1
    ...
    ->
      ...
      [
        ...
        Node.make ~offset:(off_1 + off) 1 "temporal_scalability"
          (Bits (Bool temporal_scalability));
        Node.make ~offset:(off_2 + off) 1 "quality_scalability"
          (Bits (Bool quality_scalability));
        Node.make ~offset:(off_3 + off) 1 "spatial_scalability"
          (Bits (Bool spatial_scalability));
        Node.make ~parsed ~offset:(off_4 + off) 4 "hierarchy_type"
          (Hex (Int hier_typ));
        Node.make ~offset:(off_5 + off) 2 "reserved_2" (Bits (Int reserved_2));
        Node.make ~offset:(off_6 + off) 6 "hierarchy_layer_index"
          (Dec (Int hierarchy_index));
        Node.make ~offset:(off_7 + off) 1 "tref_present_flag"
          (Bits (Bool tref_present_flag));
        Node.make ~offset:(off_8 + off) 1 "reserved_3" (Bits (Bool reserved_3));
        Node.make ~offset:(off_9 + off) 6 "hierarchy_embedded_layer_index"
          (Dec (Int emb_index));
        Node.make ~offset:(off_10 + off) 2 "reserved_4" (Bits (Int reserved_4));
        Node.make ~offset:(off_11 + off) 6 "hierarchy_channel"
          (Dec (Int hierarchy_channel));
      ]
