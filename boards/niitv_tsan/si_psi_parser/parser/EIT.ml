  ...
  if Bitstring.bitstring_length x = 0 then []
    ...
    | {| event_id       : 16
      ...
      ->
        ...
        let time =
          match Date_time.parse_timestamp start_time with
          | Some x -> Node.Time x
          | None -> (
              match%bitstring start_time with
              | {| i : 40 |} -> Node.Dec (Uint64 i) )
        in
        let dur =
          match Date_time.parse_duration duration with
          | Some x -> Node.Duration x
          | None -> (
              match%bitstring duration with {| i : 24 |} -> Node.Dec (Uint i) )
        in
        ...
        let nodes =
          [
            ...
            Node.make ~offset:(off_3 + off) 3 "running_status"
              (Dec (Int running_status));
            Node.make ~offset:(off_4 + off) 1 "free_CA_mode"
              (Bits (Bool free_ca_mode));
            Node.make ~offset:(off_5 + off) 12 "decriptors_loop_length"
              (Dec (Int length));
            Node.make ~offset:(off_6 + off) (length * 8) "descriptors"
              (List dscrs);
          ]
        in
        let event_name = Printf.sprintf "event %d" event_id in
        let node =
          Node.make ~parsed ~offset:off (parsed_length nodes) event_name
            (List nodes)
        in
    ...
    ->
      ...
      let nodes =
        [
          ...
          Node.make ~offset:off_9 8 "segment_last_section_number"
            (Dec (Int seg_last_sec_num));
