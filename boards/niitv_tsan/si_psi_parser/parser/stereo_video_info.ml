...
let parse bs off =
  ...
  | {| reserved_1    : 7
    ...
    ; Node.make ~offset:(off + off_3) 1 "leftview_flag" (Bits (Bool leftview_flag)) ]
  | {| reserved_1   : 7
    ...
    ; Node.make ~offset:(off + off_2) 7 "reserved" (Bits (Int reserved_2))
    ; Node.make ~offset:(off + off_3) 1 "usable_as_2D" (Bits (Bool usable_as_2d))
    ; Node.make ~parsed:hu ~offset:(off + off_4) 4 "horizontal_upsampling_factor" (Bits (Int hu_f))
    ; Node.make ~parsed:vu ~offset:(off + off_5) 4 "vertical_upsampling_factor" (Bits (Int vu_f))
    ]
