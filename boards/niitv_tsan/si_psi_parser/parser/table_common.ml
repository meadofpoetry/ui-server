      ...
      ->
        ...
        let nodes =
          [
            ...
            Node.make ~offset:(off_1 + off) 16 "original_network_id"
              (Dec (Int on_id));
            Node.make ~offset:(off_2 + off) 4 "reserved_future_use"
              (Bits (Int rfu));
            Node.make ~offset:(off_3 + off) 12 "transport_descriptors_length"
              (Dec (Int length));
        ...
        let node =
          Node.make ~offset:off (parsed_length nodes) ts_name (List nodes)
        in
