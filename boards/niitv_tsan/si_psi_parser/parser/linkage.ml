  ...
  | {| 0x01        : 4
    ...
    ->
      [
        ...
        Node.make ~offset:(off + off_2) 1 "origin_type"
          (Bits (Bool origin_type));
  ...
  | {| 0x00            : 4
    ...
    ->
      [
        ...
        Node.make ~offset:(off + off_2) 1 "origin_type"
          (Bits (Bool origin_type));
        Node.make ~offset:(off + off_3) 16 "initial_service_id"
  ...
  | {| target_event_id : 16
    ...
    ->
      [
        ...
        Node.make ~offset:(off + off_1) 1 "target_listed"
          (Bits (Bool target_listed));
        Node.make ~offset:(off + off_2) 1 "event_simulcast"
...
let decode_extended_event_linkage ~linkage bs off =
    ...
    if Bitstring.bitstring_length x = 0 then []
      ...
      | {| target_event_id : 16
        ...
        ->
          let nodes =
            [
              Node.make ~offset:off 16 "target_event_id"
                (Dec (Int target_event_id));
              Node.make ~offset:(off + off_1) 1 "target_listed"
                (Bits (Bool target_listed));
              Node.make ~offset:(off + off_2) 1 "event_simulcast"
                (Bits (Bool event_simulcast));
              ...
              Node.make
                ...
                ~offset:(off + off_4) 2 "target_id_type"
                (Hex (Int target_id_type));
              Node.make ~offset:(off + off_5) 1 "original_network_id_flag"
                (Bits (Bool on_id_flag));
              Node.make ~offset:(off + off_6) 1 "service_id_flag"
  ...
  | {| loop_length    : 8
    ...
    ->
      ...
      let private_data =
        Bytes.parse ~offset:(off + off_2) rest "private_data_byte"
      in
      [
        Node.make ~offset:off 8 "loop_length" (Dec (Int loop_length));
        Node.make ~offset:(off + off_1) (loop_length * 8)
          "extended_event_linkage" (List extended_linkage);
        Node.make ~offset:(off + off_1)
          (Bitstring.bitstring_length rest)
          "private_data_bytes" (List private_data);
      ]
...
let parse bs off =
  ...
  | {| ts_id                : 16
    ...
    ->
      let handover =
        decode_handover_linkage mobile_handover_info (off + off_4)
      in
      ...
      [
        ...
        Node.make ~offset:(off + off_4) 24 "mobile_handover_info"
          (List handover);
        Node.make ~offset:(off + off_5)
          (Bitstring.bitstring_length rest)
          "private_data_bytes" (List bytes);
      ]
  | {| ts_id              : 16
     ; on_id              : 16 : save_offset_to (off_1)
     ; service_id         : 16 : save_offset_to (off_2)
    ...
    ->
      let bytes = Bytes.parse ~offset:(off + off_5) rest "private_data_bytes" in
      let event_linkage =
        decode_event_linkage event_linkage_info (off + off_4)
      in
      [
        Node.make ~offset:off 16 "transport_stream_id" (Hex (Int ts_id));
        Node.make ~offset:(off + off_1) 16 "original_network_id"
        ...
        Node.make ~offset:(off + off_4) 24 "event_linkage_info"
          (List event_linkage);
        Node.make ~offset:(off + off_5)
          (Bitstring.bitstring_length rest)
          "private_data_bytes" (List bytes);
    ...
    ->
      ...
      [
        ...
        Node.make
          ...
          ~offset:(off + off_3) 8 "linkage_type" (Hex (Int linkage_type));
        Node.make ~offset:(off + off_4)
          (Bitstring.bitstring_length rest)
          "private_data_bytes" (List bytes);
      ]
