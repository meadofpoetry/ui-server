...
let rec parse bs off =
  if Bitstring.bitstring_length bs
     = 0 then []
    ...
    | {| lang_code : 24 : bitstring
      ...
      let ser =
        ...
        | Error _ -> "Unable to decode" in
      let nodes =
        [ Node.make ~parsed ~offset:off 24 "ISO_639_language_code" (Bits (Int lang_code))
        ; Node.make ~offset:(off + off_1) 8 "service_provider_name_length" (Dec (Int length_1))
        ; Node.make ~offset:(off + off_2) (length_1 * 8) "service_provider_name" (String ser_p)
