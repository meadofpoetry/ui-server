...
let parse bs off =
  ...
  | {| frequency         : 32
    ...
    ; Node.make ~offset:(off + off_1) 16 "orbital_position" (Hex (Int orbital_position))
    ; Node.make ~offset:(off + off_2) 1  "west_east_flag" (Bits (Bool west_east_flag))
    ; Node.make ~parsed:pol ~offset:(off + off_3) 2  "polarization" (Bits (Int polarization))
    ; Node.make ~parsed:rf ~offset:(off + off_4) 2  "roll_off" (Bits (Int roll_off))
    ; Node.make ~parsed:mod_system ~offset:(off + off_5) 1
        "modulation_system" (Bits (Bool modulation_system))
    ; Node.make ~parsed:mod_type ~offset:(off + off_6) 2
