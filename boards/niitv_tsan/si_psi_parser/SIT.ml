    ...
    (match%bitstring x with
     | {| service_id     : 16
       ...
       let nodes =
         ...
         ; Node.make ~offset:(off_1 + off)  1 "reserved_future_use" (Bits (Bool dvb_rfu))
         ; Node.make ~offset:(off_2 + off)  3 "running_status" (Dec (Int running_status))
         ; Node.make ~offset:(off_3 + off) 12 "service_loop_length" (Dec (Int length))
         ; Node.make ~offset:(off_4 + off) (length * 8) "descriptors" (List dscrs) ]
       in
       let service_name = Printf.sprintf "event %d" service_id in
       let node = Node.make ~offset:off (parsed_length nodes) service_name (List nodes) in
       node :: (parse_services (off_5 + off) rest))

let parse bs =
  let services_length off till =
    Bitstring.bitstring_length bs - off - (till * 8) - 32 in
  ...
  | {| header           : 24 : bitstring
    ...
    let nodes =
      ...
      ; Node.make ~offset:off_8 12 "transmission_info_loop_length" (Dec (Int len))
      ; Node.make ~offset:off_9 (len * 8) "descriptors" (List dscrs)
      ; Node.make ~offset:off_10 (services_length off_9 len) "services" (List services)
