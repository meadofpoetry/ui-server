  ...
  if Bitstring.bitstring_length x = 0 then []
    ...
    | {| cell_id_ext : 8
      ...
      let nodes =
        ...
        ; Node.make ~offset:(off + off_1) 32 "transposer_frequency" (Dec (Uint32 trans_freq)) ]
      in
  ...
  if Bitstring.bitstring_length bs = 0 then []
    ...
    | {| cell_id     : 16
      ...
      let nodes =
        ...
        ; Node.make ~offset:(off + off_1) 32 "frequency" (Dec (Uint32 frequency))
        ; Node.make ~offset:(off + off_2) 8 "subcell_info_loop_length" (Dec (Int loop_length))
        ; Node.make ~offset:(off + off_3) (loop_length * 8) "Subcells" (List snd_loop) ]
      in
