...
let ports_sync (src : Logs.src) (board : Topology.topo_board)
    (input : input signal) (streams : Stream.t list signal) :
...
let ports_active (src : Logs.src) (board : Topology.topo_board)
...
let make_services_tab_template (b : Topology.topo_board) =
  ( Netlib.Uri.Path.(
    ...
    Api_template.make_template_props ~title:"Сервисы"
      ~stylesheets:
        [
...
let make_pids_tab_template (b : Topology.topo_board) =
  ( Netlib.Uri.Path.(
      concat (Topology.make_board_path b.control) (of_string "pids")),
    Api_template.make_template_props ~title:"PIDs"
      ~stylesheets:
        [
          "/css/board-niitv-tsan.min.css";
          "/css/board-niitv-tsan-page-input.min.css";
...
let make_si_psi_tab_template (b : Topology.topo_board) =
  ( Netlib.Uri.Path.(
      concat (Topology.make_board_path b.control) (of_string "si-psi")),
    Api_template.make_template_props ~title:"SI/PSI"
      ~stylesheets:
        [
          "/css/Chart.min.css";
          "/css/board-niitv-tsan.min.css";
...
let create (b : Topology.topo_board) (_streams : Stream.t list React.signal)
    (convert_streams :
      Topology.topo_board ->
      Stream.Raw.t list React.signal ->
      Stream.t list React.signal) (send : Cstruct.t -> unit Lwt.t) (_db : Db.t)
  ...
  ( if get_is_demo_from_env src b then
    Demo_protocol.create src send (convert_streams b) kv
