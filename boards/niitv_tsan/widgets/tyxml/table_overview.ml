...
struct
  ...
  let create_title ?(a = []) ?title ?(children = nil ()) () =
    let children =
      match title with None -> children | Some x -> txt x @:: children
    in
  ...
  let create_menu_selection_icon ?a () =
    Icon_markup.SVG.icon
      ~classes:
        (return [ Item_list.CSS.item_graphic; Menu.CSS.selection_group_icon ])
      ?a ~d:(return Svg_icons.check) ()

  let create_menu_mode_item ?(a = []) ?(selected = return false) ~mode () =
    let classes =
      fmap (fun x -> if x then [ Menu.CSS.item_selected ] else []) selected
    in
    Menu_markup.Item_list.list_item ~classes
      ~a:
        ( a_user_data "id-mode"
            (match mode with `Hex -> return "hex" | `Dec -> return "dec")
        :: a_aria "selected" (fmap (fun x -> [ string_of_bool x ]) selected)
        :: a )
      ~graphic:(return @@ create_menu_selection_icon ())
      ~primary_text:
        (`Text
          (match mode with `Hex -> return "Hex ID" | `Dec -> return "Dec ID"))
      ()

  let create_menu ?classes ?a ?(hex = return false) () =
    Menu_markup.menu ?classes ?a
      ~list_children:
        ( li
            (singleton
               (return
                  (ul
                     ...
                     @:: create_menu_mode_item ~selected:(fmap not hex)
                           ~mode:`Dec ()
                     @:: nil () ))))
          ...
          ^:: div
                ...
                ( Icon_button_markup.icon_button
                    ...
                    ~icon:
                      ( return
                      @@ Icon_markup.SVG.icon
                           ~d:(return Svg_icons.dots_vertical)
                           () )
                    ()
                @:: create_menu ?hex ()
                @:: nil () )
  ...
  let create ?(classes = return []) ?(a = []) ?(dense = true)
      ?(hex = return false) ?details ?data ?rows ?title ?row_a ?row_classes
      ~format ~control () =
    let ( @:: ) x l = cons x l in
    let classes =
      fmap
        (fun x ->
          CSS.root :: x
          |> Utils.cons_if (Option.is_some details) CSS.with_details)
        classes
    in
    let placeholder =
      match rows with
      ...
      | Some rows ->
          Xml.Wutils.totlist
            ( fmap (function [] -> [ create_empty_placeholder () ] | _ -> [])
            @@ Xml.Wutils.tot rows )
    in
    let back =
      match details with None -> None | Some _ -> Some (create_back_action ())
    in
    let header = create_header ~hex ?back ?title () in
    let table =
      Data_table_markup.data_table_of_fmt ?row_a ?row_classes ?rows ~dense
    ...
    let body =
      match details with
      | None -> return table
      | Some details -> fmap (function None -> table | Some x -> x) details
    in
    div
      ~a:
        ( a_class classes
        :: a_user_data "id-mode"
             (fmap (fun x -> if x then "hex" else "dec") hex)
