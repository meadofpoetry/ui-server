...
struct
  ...
  let data_of_service_info ?(bitrate = return None) (id, (info : Service.t)) :
      _ Fmt.data =
    Fmt.
      [
        return id;
        ...
        fmap
          ...
          bitrate;
        fmap
          (function None -> None | Some (_, { Bitrate.min; _ }) -> Some min)
          bitrate;
        fmap
          (function None -> None | Some (_, { Bitrate.max; _ }) -> Some max)
          bitrate;
      ]
  ...
  let create ?a ?dense ?(hex = return false) ?(bitrate = return None)
      ?(init = nil ()) ?pids ?(selected = return None) ~control =
    ...
    in
    let details =
      fmap (function None -> None | Some x -> Some service_info) selected
    in
    let rows =
      Xml.W.map
        (fun ((id, _) as x) ->
    ...
    create
      ...
      ?a ?dense ~details ~hex ~title ~format ~rows ~control
end
