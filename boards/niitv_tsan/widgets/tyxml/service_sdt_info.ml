...
struct
  ...
  let create_item_icon_meta ?(classes = []) ?(a = []) ?(active = false) ?children () =
    let classes = Item_list.CSS.item_meta :: classes in
    let a = if active then Svg.Unsafe.string_attrib "data-active" "" :: a else a in
    let children =
      match children with
      | Some x -> x
      | None ->
          Icon_markup.SVG.
            [ icon_path ~d:Svg_icons.check_circle ()
            ; icon_path ~d:Svg_icons.close_circle () ]
    in
    Icon_markup.SVG.icon ~classes ~a ~children ()
  ...
  let create_item ?(classes = []) ?a ?meta ~primary_text () =
    ...
    in
    Item_list_markup.list_item ~classes ?a ~primary_text ?meta ()

  let create ?(classes = []) ?a ?(info : (int * Service.t) option) ?children () =
