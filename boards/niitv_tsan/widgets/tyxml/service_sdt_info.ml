...
struct
  ...
  let create_item_icon_meta ?a ?(active = return false) () =
    let classes =
      fmap
        (fun x -> Utils.cons_if x CSS.meta_active [ Item_list.CSS.item_meta ])
        active
    in
    let children =
      SVG.icon_path
        ~d:
          (fmap
             (fun x ->
               if x then Svg_icons.check_circle else Svg_icons.close_circle)
             active)
        ()
      @:: nil ()
    in
  ...
  let create ?a ?(info : (int * Service.t) option wrap = return None) ?children
    ...
    let children =
      ...
      | None ->
          ...
          let meta_icon f =
            let active =
              fmap (function None -> false | Some (_, x) -> f x) info
            in
          ...
          @:: create_item
                ...
                ~meta:
                  (meta (fun x -> MPEG_TS.service_type_to_string x.service_type))
                ()
          @:: create_item
          ...
          @:: create_item
                ...
                ~meta:
                  (meta (fun x ->
                       MPEG_TS.running_status_to_string x.running_status))
                ()
          @:: nil ()
    in
