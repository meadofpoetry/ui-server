...
let sum_bitrate : (int * int) list -> int = List.fold_left (fun acc (_, x) -> acc + x) 0
...
let make_sdt_info () =
  ...
  let typ, set_typ =
    ...
    item, fun x -> meta#set_text @@ Mpeg_ts.service_type_to_string x
  in
  let eit_s, set_eit_s =
    ...
    , function
      ...
      | false ->
          ...
          (meta#path)#set Icon.SVG.Path.close_circle )
  in
  let scr, set_scr =
    let meta = Icon.SVG.(create_simple Path.check_circle) in
  ...
  let running_status, set_running_status =
    ...
    item, fun x -> meta#set_text @@ Mpeg_ts.running_status_to_string x
  in
  ()
...
let make_description () =
  ...
  let main_box =
    new Card.t ~outlined:true ~widgets:[main_primary#widget; main_media#widget] ()
  in
  let sdt_box =
    new Card.t ~outlined:true ~widgets:[sdt_primary#widget; sdt_media#widget] ()
  in
  let box = new Hbox.t ~wrap:`Wrap ~widgets:[main_box; sdt_box] () in
  box#add_class _class;
  let set ?hex (x : Service.t) =
    set_main ?hex x;
    set_sdt x
  in
...
module Pids = struct
  ...
  let get_service_pids ((_, info) : Service.t) =
    ...
    List.cons_maybe pmt info.elements |> List.sort_uniq ~cmp:compare

  let filter_pids service (pids : Pid.t list) =
    let service_pids = get_service_pids service in
    List.filter (fun ((pid, _) : Pid.t) -> List.mem ~eq:( = ) pid service_pids) pids
  ...
  class t
    ...
    let init =
      match init with
      | None -> None
      | Some x -> Some {x with data = filter_pids service x.data}
    in
...
class t
  ...
  let table = pids in
  let tabs =
    let info_icon = Icon.SVG.(create_simple Path.file_document_box_outline) in
    let list_icon = Icon.SVG.(create_simple Path.view_list) in
    [ new Tab.t ~content:(Both ("Описание", info_icon)) ~value:info ()
    ; new Tab.t ~content:(Both ("PIDs", list_icon)) ~value:table#widget () ]
  in
  let div = Widget.create_div () in
  let bar, _ = Ui_templates.Tabs.create_simple ~body:div tabs in
  object (self)
    inherit Vbox.t ~widgets:[bar#widget; (new Divider.t ())#widget; div] () as super
    ...
    method info : Service.t = _info

    method service_id : int = fst _info

    method not_available : bool = self#has_class not_available_class
    ...
    method update_pids (x : Pid.t list Time.timestamped) : unit = pids#update x
    ...
    (* Private methods *)
    method private set_hex (x : bool) : unit = set_info ~hex:x _info
