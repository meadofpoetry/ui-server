...
module Table_info = struct
  ...
  let compare (a : t) (b : t) : int = compare_id (fst a) (fst b)

  let equal (a : t) (b : t) : bool = 0 = compare a b
end

module Set = Set.Make (Table_info)

(** Returns string representing human-readable section name *)
let to_table_name ?(is_hex = false) table_id table_id_ext id_ext_1 id_ext_2
    service =
  let open Printf in
  let divider = ", " in
  let name = Mpeg_ts.(table_to_string @@ table_of_int table_id) in
  let id ?service s x =
    let s = if is_hex then sprintf "%s=0x%02X" s x else sprintf "%s=%02d" s x in
    match service with None -> s | Some x -> sprintf "%s (%s)" s x
  in
  ...
  let specific =
    ...
    | `SDT _ -> [ id "tsid" table_id_ext; id "onid" id_ext_1 ]
    | `BAT -> [ id "bid" table_id_ext ]
    | `EIT _ ->
        [
          id ?service "sid" table_id_ext; id "tsid" id_ext_1; id "onid" id_ext_2;
        ]
    | _ -> []
  in
...
let to_table_extra ?(hex = false) ((id, _) : SI_PSI_table.t) =
  ...
  let specific =
    ...
    | `SDT _ -> Some [ ("tsid", id.table_id_ext); ("onid", id.id_ext_1) ]
  ...
  Tyxml_js.Html.(
    let wrap x =
      List.mapi
        (fun i (s, v) ->
          ...
          span
            [
              span
                ~a:[ a_class [ Typography.Markup.subtitle2_class ] ]
                [ txt (s ^ ": ") ];
              txt v;
            ])
        x
      |> span
    in
    toelt @@ match specific with None -> span [] | Some l -> wrap l)
    ...
    Custom_elt
      {
        is_numeric = false;
        compare = SI_PSI_table.compare;
        to_elt = to_table_extra ~hex:true;
      }
  in
...
module Heading = struct
  class t ?meta ?title ?subtitle () =
    ...
    object (self)
      ...
      method set_title (s : string) : unit = title'#set_text_content s

      method set_subtitle (s : string) : unit = subtitle'#set_text_content s
    end
end

let add_row (table : 'a Table.t) (is_hex : bool) (stream : Stream.ID.t)
    (parent : #Widget.t) (control : int)
  ...
  let listener =
    row#listen_click_lwt (fun _ _ ->
        ...
        let cell =
          let open Table in
          match row#cells with _ :: _ :: _ :: _ :: _ :: _ :: x :: _ -> x
        in
        ...
        let dump =
          new Widget_tables_dump.t
            ~stream ~sections:cell#value ~table_id:id.table_id
            ~table_id_ext:id.table_id_ext ~id_ext_1:id.id_ext_1
            ~id_ext_2:id.id_ext_2 control ()
        in
        let set_hex (x : bool) =
        ...
        let box =
          new Vbox.t
            ~widgets:[ heading#widget; (new Divider.t ())#widget; dump#widget ]
            ()
        in
        dump#set_hex is_hex;
        ...
        >|= (fun _ ->
              ...
              @@ List.length
              @@ List.filter (fun (_, d) -> Option.is_some d) sections;
...
class t ?(settings : Settings.t option)
  (init : SI_PSI_table.t list Time.timestamped option) (stream : Stream.ID.t)
  (control : int) () =
  let init, timestamp =
    match init with
    | None -> ([], None)
    | Some { data; timestamp } -> (data, Some timestamp)
  in
  let s_time, set_time =
    React.S.create ~eq:(Equal.option Time.equal) timestamp
  in
  let table, on_change = make_table () in
  let dump, set_dump =
    let eq (_, w1) (_, w2) = Widget.equal w1 w2 in
    React.S.create ~eq:(Equal.option eq) None
  in
  let empty =
  ...
  object (self)
    ...
    method! init () : unit =
      ...
      React.S.map ~eq:Equal.unit
        (function
          | [] -> self#append_child empty | _ -> self#remove_child empty)
        table#s_rows
      |> self#_keep_s;
      Set.iter Fun.(ignore % self#add_row) _data;
      self#add_class base_class

    method s_timestamp : Time.t option React.signal = s_time
    ...
    method set_hex (x : bool) : unit =
      _hex <- x;
      (match React.S.value dump with None -> () | Some (set, _) -> set x);
      on_change x
    (** Sets identifiers to hex or decimal view *)

    method set_settings (x : Settings.t) : unit = self#set_hex x.hex
      ...
      let rec aux = function
        ...
        | (rate : Bitrate.table) :: tl, rows -> (
            let find (row : 'a Table.Row.t) =
              ...
              && id.id_ext_2 = rate.id_ext_2
            in
            match List.find_opt find rows with
            | None -> aux (tl, rows)
            | Some row ->
                let open Table in
                let rate', pct', min', max' =
                  match row#cells with
                  | [ _; _; _; _; _; _; _; _; rate; pct; min; max ] ->
                      (rate, pct, min, max)
                in
                let br = Float.(of_int rate.bitrate / 1_000_000.) in
                let pct = Float.(100. * of_int rate.bitrate / of_int x.total) in
    ...
    (* Private methods *)
    method private _update_row (row : 'a Table.Row.t)
        ((_, info) : SI_PSI_table.t) =
      let open Table in
      let open SI_PSI_table in
      match row#cells with
    ...
    method private _row_to_table_info (row : 'a Table.Row.t) =
      let open Table in
      match row#cells with _ :: _ :: _ :: x :: _ -> x#value
  end
...
let make_dashboard_item ?settings init stream control : 'a Dashboard.Item.item =
  ...
  let (settings : Dashboard.Item.settings) =
    Dashboard.Item.make_settings ~widget:settings
      ~set:(fun () -> Lwt_result.return @@ w#set_settings settings#value)
      ()
  in
  let tz_offset_s = Ptime_clock.current_tz_offset_s () in
  let timestamp =
    Dashboard.Item.make_timestamp ~time:w#s_timestamp
      ~to_string:(Time.to_human_string ?tz_offset_s)
      ()
  in
  Dashboard.Item.make_item ~name:"Список таблиц SI/PSI"
    ~subtitle:(Timestamp timestamp) ~settings w
