...
let req_of_table table_id table_id_ext id_ext_1 id_ext_2 section =
  let r = Requests.Streams.HTTP.get_si_psi_section ~table_id ~section in
...
module Section = struct
  ...
  let equal_model (a : model) (b : model) = equal_section_info (fst a) (fst b)

  let widget w = w#widget

  let id_of_model ((x : section_info), _) = x.section
  ...
  let make (init : model) =
    let bytes, update_bytes =
      let to_string x =
        let s =
          ...
          | _ -> "байт"
        in
        Printf.sprintf "%d %s" x s
      in
      let w = new Typography.Text.t ~text:"" () in
      let v =
        {
          get = (fun (x : model) -> (fst x).length);
          eq = Int.equal;
          upd = Fun.(w#set_text % to_string);
        }
      in
      (w, v)
    in
    ...
    let leaf =
      new Item_list.Item.t ~text:"" ~meta:bytes#widget ~value:init ()
    in
    ...
    in
    let update ?(previous : model option) (model : model) =
...
module Sections = Make_array (struct
  open SI_PSI_table
  module Node = Section
  ...
  let append_child (w : widget) (i : Node.widget) =
    i#listen Widget.Event.click (fun _ _ ->
        w#set_active i;
        true)
    |> ignore;
    w#append_item i
  ...
  let remove_child (w : widget) (i : Node.widget) = w#remove_item i

  let make (_ : Node.model list) =
    let list = new Item_list.t ~dense:true ~selection:`Single ~items:[] () in
    (list, fun _ -> ())
end)

let make_list
    (sections :
      (SI_PSI_table.section_info * Dump.t Time.timestamped option) list) =
...
let make_hexdump_options hexdump =
  ...
  let base =
    new Select.t
      ...
      ~items:
        [
          `Item (new Select.Item.t ~value:`Hex ~text:"16" ());
  ...
  let width =
    new Select.t
      ...
      ~items:
        [
          `Item (new Select.Item.t ~value:4 ~text:"4" ());
  ...
  in
  let options =
    new Hbox.t ~widgets:[ base#widget; width#widget; line_numbers#widget ] ()
  in
  let s_line_num =
    React.S.map ~eq:Equal.unit hexdump#set_line_numbers
      line_numbers#input_widget#s_state
  in
  let s_width =
    React.S.map ~eq:Equal.unit
      (function None -> () | Some x -> hexdump#set_width x)
      width#s_selected_value
  in
  let s_base =
    React.S.map ~eq:Equal.unit
      (function None -> () | Some x -> hexdump#set_base x)
      base#s_selected_value
  in
...
let make_dump_header base_class () =
  ...
  in
  let subtitle =
    new Typography.Text.t ~adjust_margin:false ~split:true ~text:"" ()
  in
  let button =
    new Ui_templates.Buttons.Get.t ~style:`Raised ~label:"Загрузить" ()
  in
  let title_box = new Vbox.t ~widgets:[ title#widget; subtitle#widget ] () in
  let header =
    new Hbox.t
      ~halign:`Space_between
      ~widgets:[ title_box#widget; button#widget ]
      ()
  in
  (* CSS classes setup *)
...
let make_tree (x : Dump.parsed) =
  ...
  let value_to_string : value -> string = function
    | Bytes s -> String.concat " " @@ List.map Fun.(String.of_char % Char.chr) s
  ...
  let rec aux acc = function
    ...
    | hd :: tl ->
        ...
        let text =
          match name with None -> vs | Some n -> Printf.sprintf "%s (%s)" n vs
        in
        let meta, nested =
          match value with
          | List [] ->
          ...
          | List l ->
              ...
              (None, Some tree)
          | _ -> (Some (new Typography.Text.t ~text ())#widget, None)
        in
        let item = new Tree.Item.t ?meta ?nested ~value:hd ~text:hd.name () in
        let info =
          Printf.sprintf "Длина: %d, смещение: %d" hd.length
            hd.offset
        in
...
let make_dump ~(table_id : int) ~(table_id_ext : int) ~(id_ext_1 : int)
    ~(id_ext_2 : int) (stream : Stream.ID.t)
    (list :
      (SI_PSI_table.section_info * Dump.t Time.timestamped option) Item_list.t)
    (control : int) =
  ...
  object (self)
    inherit
      Vbox.t
        ~widgets:
          [ header; vsplit#widget; (new Divider.t ())#widget; options#widget ]
        () as super
          ...
          let upd : Dump.t Time.timestamped option -> unit = function
            | Some { timestamp; data = { section; content = Some x; _ } } ->
                ...
                let rec get_items acc = function
                  ...
                  | hd :: tl ->
                      let acc =
                        match hd#nested_tree with
                        | None -> (
                            match hd#value.value with
                            | List _, _ -> acc
                            | _ -> hd :: acc )
                        | Some tree -> get_items acc tree#items
                      in
                      get_items acc tl
                in
                let items = get_items [] tree#items in
                List.iter
                  (fun (i : (node, _) Tree.Item.t) ->
            ...
            | None ->
                ...
                subtitle#set_text "-";
                parsed#append_child
                  (ph "Нет захваченных данных");
                set_hexdump ""
          in
          let get () =
            Lwt.catch
              (fun () ->
                (req_of_table table_id table_id_ext id_ext_1 id_ext_2
                   section.section)
                  ~id:stream control
                |> Lwt_result.map_err
...
class t ~(stream : Stream.ID.t) ~(table_id : int) ~(table_id_ext : int)
  ~(id_ext_1 : int) ~(id_ext_2 : int)
  ~(sections :
     (SI_PSI_table.section_info * Dump.t Time.timestamped option) list)
  (control : int) () =
  ...
  let dump =
    make_dump ~table_id ~table_id_ext ~id_ext_1 ~id_ext_2 stream list control
  in
  let list_name =
  ...
  in
  let list_box =
    Widget.create_div ~widgets:[ list_name#widget; list#widget ] ()
  in
