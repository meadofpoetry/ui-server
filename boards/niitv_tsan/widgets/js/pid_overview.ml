...
let pid_type_fmt : MPEG_TS.PID.Type.t Data_table.custom =
  MPEG_TS.PID.Type.{ to_string
                   ; compare
                   ; is_numeric = false
                   }

let pid_flags_fmt : pid_flags Data_table.custom_elt =
  { to_elt = make_pid_flags_widget
  ; compare = compare_pid_flags
  ; is_numeric = false
  }

let dec_pid_fmt = Data_table.(Int None)

let hex_pid_fmt = Data_table.(Int (Some (Util.pid_to_hex_string)))
...
let make_table_fmt ?(is_hex = false) () =
  ...
  :: (make_column ~sortable:true ~title:"Сервис" (), Option (String None, ""))
