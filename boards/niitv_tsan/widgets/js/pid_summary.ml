...
module Attr = struct
  ...
  let set_pid_type typ elt =
    let typ =
      ...
      @@ Application_types.MPEG_TS.PID.Type.to_yojson typ in
  ...
  let get_pid_type elt =
    ...
    | Some x ->
      let result =
        Application_types.MPEG_TS.PID.Type.of_yojson
        @@ Yojson.Safe.from_string x in
      match result with
      | Error e -> failwith e
      | Ok x -> x
  ...
  let set_pid_info (info : PID_info.t) elt =
    let set_bool elt attr x =
      if x then Element.set_attribute elt attr ""
      else Element.remove_attribute elt attr in
    let set_option elt attr f = function
      | None -> Element.remove_attribute elt attr
      | Some x -> Element.set_attribute elt attr (f x) in
...
class t (elt : Dom_html.element Js.t) = object(self)
  ...
  method pids : (int * PID_info.t) list =
    List.map (fun x ->
        let text = Js.Opt.get x##.textContent
            (fun () ->
               let msg = name ^ ": text content is not set for PID element" in
               failwith msg) in
        int_of_string @@ Js.to_string text,
        Attr.pid_info_of_element x)
  ...
  method private add_pid pid =
    let elt = Tyxml_js.To_dom.of_element @@ Markup.make_pid pid in
