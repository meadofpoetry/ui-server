...
module Selector = struct
  let total = Printf.sprintf ".%s.%s .%s"
      CSS.value
      CSS.value_total
      Item_list.CSS.item_meta

  let effective = Printf.sprintf ".%s.%s .%s"
      CSS.value
      CSS.value_effective
      Item_list.CSS.item_meta
end
...
class t (elt : Dom_html.element Js.t) = object(self)
  ...
  method private set_rate = function
    | None -> self#set_total None; self#set_effective None
    | Some (x : Bitrate.t) ->
...
let make ?classes ?attrs ?total ?effective () =
  let elt =
    ...
    @@ Markup.make ?classes ?attrs ?total ?effective () in
  new t elt
