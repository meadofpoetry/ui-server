...
let make ?a ?info ?children () =
  D.create ?a ?info ?children () |> Tyxml_js.To_dom.of_ul |> attach

let make_r ?a ?info ?children () =
  R.create ?a ?info ?children () |> Tyxml_js.To_dom.of_ul |> attach
