...
let on_visible (page : #page) state_ref control elt =
  ...
  let thread =
    ...
    >>=? fun (_, pids_ev) ->
    let notif = E.merge (fun _ -> page#notify) ()
    ...
    Lwt.return_ok state in
  let _loader = Components_lab.Loader.make_loader ~elt thread in
  ()

let on_hidden state_ref = match !state_ref with
  | None -> ()
  | Some state -> state#finalize (); state_ref := None
...
let initialize id control =
  ...
  let pie_cell = Layout_grid.Cell.make
      ~span:4
      ~span_tablet:8
      [pie] in
  let rate_cell = Layout_grid.Cell.make
      ~span:8
      [ rate#widget
      ; Widget.coerce @@ Divider.make ()
      ; pids#widget ] in
