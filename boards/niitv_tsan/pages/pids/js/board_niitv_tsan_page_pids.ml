...
module Selector = struct
  ...
  let pid_bitrate_pie_chart =
    Printf.sprintf ".%s" Pid_bitrate_pie_chart.CSS.root
...
class t ~set_stream ~set_hex elt () =
  object
    ...
    method notify : event -> unit =
      function `Bitrate x -> pid_bitrate_pie_chart#notify (`Bitrate x)
  end
...
let on_visible ~input (elt : Dom_html.element Js.t) (state : state) control =
  ...
  let thread =
    ...
    let pid_overview =
      Pid_overview.R.create ~hex ~init:signal ~bitrate ~control ()
    in
    let page =
