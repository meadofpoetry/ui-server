...
module CSS = struct
  let root =
    Printf.sprintf "%s-page-pids" (Topology.make_dom_node_class board_id)
end
...
struct
  ...
  module Pid_bitrate_pie_chart_markup =
    Pid_bitrate_pie_chart.Make (Xml) (Svg) (Html)
  module Divider_markup = Divider.Make (Xml) (Svg) (Html)

  let create ?(classes = return []) ?(a = []) ~stream_select ~bitrate_summary
      ~pid_summary ~pid_overview () =
    let classes =
    ...
    let children =
      Layout_grid_markup.
        [
          ...
          layout_grid_cell ~span:4 ~span_tablet:8
            ~children:(const [ Pid_bitrate_pie_chart_markup.create () ])
            ();
          layout_grid_cell ~span:8
            ~children:
              (const
                 [ bitrate_summary; Divider_markup.divider_hr (); pid_summary ])
            ();
