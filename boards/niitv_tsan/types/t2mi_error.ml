...
let name_of_code = function
  ...
  | code -> (
    ...
    | Some e ->
        Printf.sprintf "%s %s" (T2MI.ETR290_error.number e) (T2MI.ETR290_error.name e))
...
module Info = struct
  let of_error (e : 'a Error.e) =
    ...
    | 0x09 ->
        ...
        Printf.sprintf
          "Длительность T2-кадра равна %ld мс (больше 250 \
           мс)"
          sec
    | 0xF0 -> (
    ...
    | 0xF1 -> (
      ...
      | 1 ->
          "В L1 обнаружен код FFT, не разрешённый для \
           режима T2-Lite"
      | 2 ->
      ...
      | 4 ->
          "Количество T2-кадров в суперкадре меньше \
           двух"
      ...
      | 12 ->
          "Получен пакет \"L1-Future\", не предусмотренный \
           режимом"
      | 13 -> "Длина пакета \"DVB-T2 Timestamp\" не равна 11 байт"
      | 15 ->
          "Пакет \"L1-Current\" отсутствует в потоке более \
           3 секунд"
      | _ -> "")
    | _ -> ""
end
