...
module Info = struct
  ...
  let of_error (e : 'a Error.e) =
    ...
    | 0x09 ->
      ...
      Printf.sprintf "Длительность T2-кадра равна %ld мс (больше 250 мс)" sec
    | 0xF0 ->
      (match e.err_ext with
       | 0 -> "Нет места для pointer_field и начала T2-MI пакета"
       | 1 -> "Длина поля адаптации больше 182 байт"
       | 2 -> "pointer_field указывает за пределы пакета"
       | 3 -> "Наложение T2-MI пакетов"
       | _ -> "")
    | 0xF1 ->
      (match e.err_ext with
       | 0 -> "Поток предназначен не для DVB-T2"
       | 1 -> "В L1 обнаружен код FFT, не разрешённый для режима T2-Lite"
       | 2 -> "Некорректное значение защитного интервала"
       | 3 -> "Значение защитного интервала не совместимо с режимом FFT"
       | 4 -> "Количество T2-кадров в суперкадре меньше двух"
       | 5 -> "Длина поля L1_Post_Configurable слишком велика"
       | 6 -> "Длина поля L1_Dynamic слишком велика"
       | 7 | 8 -> "Длина пакета \"L1-Current\" слишком мала"
       | 9 -> "В пакете \"L1-Current\" отсутствует поле \"Dynamic\""
       | 10 -> "Некорректная длина поля \"Dynamic\""
       | 11 -> "Длина пакета \"L1-Future\" слишком мала"
       | 12 -> "Получен пакет \"L1-Future\", не предусмотренный режимом"
       | 13 -> "Длина пакета \"DVB-T2 Timestamp\" не равна 11 байт"
       | 15 -> "Пакет \"L1-Current\" отсутствует в потоке более 3 секунд"
       | _ -> "")
    | _ -> ""
