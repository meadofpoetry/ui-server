...
type table_info = { section : int; id : int; id_ext : int }
...
module Info = struct
  ...
  type interval =
    [ `S of float | `Ms of float | `Us of float | `Ns of float | `Unk ]
  [@@deriving show]
    ...
    | `Ns _ ->
        ...
        if s >. 1. then `S s
          ...
          if ms >. 1. then `Ms ms
            ...
            let us = x /. 1e3 in
            if us >. 1. then `Us us else `Ns x
    | `Unk -> `Unk

  let period_to_float = function `S x | `Ms x | `Us x | `Ns x -> x | _ -> 0.0
  ...
  let crc_err ?(hex = false) ?table (e : 'a Error.e) =
    ...
    Printf.sprintf "%s, CRC = %s, должно быть %s" prefix computed
      actual

  let interval_err ?(coef = 0.1) ~prefix ~cmp_word ~period (e : 'a Error.e) =
    let got =
      match e.param_1 with
      | -1l -> None
      | x ->
          Int32.to_float x *. coef |> fun x ->
          Some (integrate_interval x period)
    in
    let must =
      match period with
      | `Unk -> None
      | x ->
          let s =
            Printf.sprintf "%s %g %s" cmp_word (period_to_float x)
              (period_to_unit_name x)
          in
          Some s
    in
    match (got, must) with
    ...
    | Some got, None ->
        Printf.sprintf "%s %g %s" prefix (period_to_float got)
          (period_to_unit_name got)
    | Some got, Some ext ->
        Printf.sprintf "%s %g %s (%s)" prefix (period_to_float got)
  ...
  let table_scrambled table =
    Printf.sprintf "Таблица %s скремблирована"
    @@ table_name table
  ...
  let table_id ?(short = false) ?(hex = false) (e : 'a Error.e) table =
    ...
    | _ ->
        if short then
          Printf.sprintf "Поле table_id не равно %s" possible
        else
          Printf.sprintf "Поле table_id = %s, должно быть %s" got
            possible
  ...
  let of_error ?(hex = true) (e : 'a Error.e) =
    ...
    | 0x14 -> (
        ...
        | 0x04 ->
            "Счётчик изменился при отсутствии \
             полезной нагрузки"
        | _ -> "" )
    | 0x15 ->
        let f =
          ...
          | 0x06 ->
              fun _ ->
                "Разный номер программы в PAT и PMT"
          | _ -> table_ext_unknown
        in
        f `PMT
    ...
    (* Second priority *)
    | 0x21 ->
        "В пакете установлен флаг \
         transport_error_indicator"
    | 0x22 -> crc_err ~hex e
    | 0x23 -> (
        match e.err_ext with
        | 0x01 ->
            let prefix = "Период повторения PCR" in
            let period = `Ms 40. in
            interval_err ~prefix ~cmp_word:"больше" ~period e
        | 0x02 ->
            interval_err ~prefix:"Разрыв PCR" ~cmp_word:"больше"
              ~period:(`Ms 100.) e
        | 0x03 ->
            Printf.sprintf
              "Отсутствует PCR для программы %04u"
            @@ Int32.to_int e.param_1
        | _ -> "" )
    | 0x24 ->
        interval_err ~coef:1.0 ~prefix:"Неравномерность PCR"
          ~cmp_word:"больше" ~period:(`Ns 500.) e
    | 0x25 ->
        interval_err ~prefix:"Период повторения PTS"
          ~cmp_word:"больше" ~period:(`Ms 700.) e
    | 0x26 ->
        let f =
          match e.err_ext with
          | 0x01 -> table_scrambled
          ...
          | 0x04 ->
              fun _ ->
                "Есть скремблирование, но нет CAT"
          | _ -> table_ext_unknown
        in
        f `CAT
    (* Third priority *)
    ...
    | 0x32 ->
        ...
        let f =
          ...
          | _ -> table_ext_unknown
        in
        f @@ MPEG_TS.SI_PSI.of_table_id id
