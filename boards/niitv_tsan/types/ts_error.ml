...
module Info = struct
  ...
  let interval_err ?(coef = 0.1) ~prefix ~cmp_word ~period (e : 'a Error.e) =
    let got =
      ...
      | -1l -> None
      | x -> Int32.to_float x *. coef |> fun x -> Some (integrate_interval x period)
    in
    let must =
      match period with
      | `Unk -> None
    ...
    | Some got, None ->
        Printf.sprintf "%s %g %s" prefix (period_to_float got) (period_to_unit_name got)
    | Some got, Some ext ->
        Printf.sprintf
          "%s %g %s (%s)"
          prefix
  ...
  let table_id ?(short = false) ?(hex = false) (e : 'a Error.e) table =
    ...
    | _ ->
        if short
        then Printf.sprintf "Поле table_id не равно %s" possible
        else
          Printf.sprintf "Поле table_id = %s, должно быть %s" got possible
    ...
    | 0x23 -> (
      ...
      | 0x02 ->
          interval_err
            ...
            e
      | 0x03 ->
          Printf.sprintf "Отсутствует PCR для программы %04u"
          @@ Int32.to_int e.param_1
      | _ -> "")
    | 0x24 ->
        interval_err
          ~coef:1.0
          ~prefix:"Неравномерность PCR"
          ~cmp_word:"больше"
          ~period:(`Ns 500.)
          e
    | 0x25 ->
        interval_err
          ~prefix:"Период повторения PTS"
          ~cmp_word:"больше"
          ~period:(`Ms 700.)
          e
    | 0x26 ->
