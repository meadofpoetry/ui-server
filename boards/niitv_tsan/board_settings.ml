...
let default =
  { Board_niitv_tsan_types.input = ASI
  ; input_source = 1
  ; t2mi_source = 2
  ; t2mi_mode =
      { pid = 0x1FFF
  ...
  ; jitter_mode =
      {pid = 0x1FFF; stream = Stream.Multi_TS_ID.of_int32_pure 0l; stream_id = None} }

let to_string x = Yojson.Safe.pretty_to_string @@ to_yojson x
