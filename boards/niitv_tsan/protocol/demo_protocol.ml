      ...
      source =
        {
          ...
          info =
            IPV4
              {
                scheme = "udp";
                addr = Ipaddr.V4.of_string_exn "224.1.2.2";
                port = 1234;
              };
        };
      typ = TS;
      ...
      source =
        {
          ...
          info =
            IPV4
              {
                scheme = "udp";
                addr = Ipaddr.V4.of_string_exn "224.1.2.3";
                port = 1234;
              };
        };
      typ = TS;
    ...
    Lwt_react.(
      E.from (fun () ->
          Lwt.(
            ...
            let pids_rate =
              List.map (fun (pid, _) -> (pid, Random.int 5_000_000)) pids
            in
            let total =
              List.fold_left (fun acc (_, x) -> acc + x) 0 pids_rate
            in
            let effective =
              List.fold_left
...
let create (src : Logs.src) (_sender : Cstruct.t -> unit Lwt.t)
    (_streams_conv :
  ...
  Lwt.return_ok
    ( {
        ...
        channel =
          (fun _ ->
            Lwt.return_error (Request.Custom "Sorry, this is demo board"));
        loop;
        push_data = (fun _ -> ());
        bitrate_queue;
      }
