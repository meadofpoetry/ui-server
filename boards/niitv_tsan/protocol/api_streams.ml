...
module Event = struct
  ...
  let get_streams (api : Protocol.api) incoming ids _user =
    ...
    let filter =
      match incoming, ids with
      | (None | Some false), [] -> None
      | Some true, [] -> Some (List.filter (is_incoming_stream api))
      | Some true, ids -> Some (List.filter (is_incoming_stream api) % filter_streams ids)
      | _, ids -> Some (filter_streams ids)
    in
    let signal =
      match filter with
...
let get_streams (api : Protocol.api) incoming ids _user _body _env _state =
  ...
  let streams =
    ...
    | Some f -> f @@ React.S.value api.notifs.streams
  in
