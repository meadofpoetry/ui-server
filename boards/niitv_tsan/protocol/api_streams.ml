...
module Event = struct
  ...
  let get_streams (api : Protocol.api) incoming ids inputs _user =
    let to_yojson = Util_json.List.to_yojson Stream.to_yojson in
    let filter streams =
      streams
      |> filter_by_id ids
      |> filter_by_incoming api incoming
      |> filter_by_input inputs
    in
    let eq x = Util_equal.List.equal Stream.equal x in
    let signal = S.map ~eq filter api.notifs.streams in
...
let get_streams (api : Protocol.api) incoming ids inputs _user _body _env _state
  ...
  let filter streams =
    ...
    |> filter_by_input inputs
  in
  let streams = filter (React.S.value api.notifs.streams) in
