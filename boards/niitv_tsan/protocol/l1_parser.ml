    ...
    if Bitstring.bitstring_length x = 0 then List.rev acc
      ...
      let rf =
        ...
        | {| rf_idx : 3
           ...
           |}
          ->
            { T2mi_info.rf_idx; frequency }
      in
      f (rf :: acc) (Bitstring.dropbits 35 x)
  in
  f [] bs

let parse_l1_post_conf_fef =
  function%bitstring
  | {| fef_type : 4
     ; fef_length : 22
     ; fef_interval : 8 |} ->
...
let parse_l1_post_conf_plp (bs : Bitstring.t) =
  let rec f acc x =
    if Bitstring.bitstring_length x = 0 then List.rev acc
      ...
      in
      f (plp :: acc) (Bitstring.dropbits 89 x)
  in
  f [] bs
    ...
    if Bitstring.bitstring_length x = 0 then List.rev acc
      ...
      let aux =
        ...
        | {| aux_stream_type : 4
           ...
           |}
          ->
            { T2mi_info.aux_stream_type; aux_private_conf }
      in
      f (aux :: acc) (Bitstring.dropbits 32 x)
  in
  f [] bs

let parse_l1_post_conf ~num_rf ~s2 =
  function%bitstring
  | {| sub_slices_per_frame : 15
    ...
    ->
      let fef =
        if Bitstring.bitstring_length fef = 0 then None
        else Some (parse_l1_post_conf_fef fef)
      in
      {
        T2mi_info.sub_slices_per_frame;
        aux_config_rfu;
        fef_length_msb;
        reserved_2;
        fef;
