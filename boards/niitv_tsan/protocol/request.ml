...
type response_tag = [ `Devinfo | `Mode | `Part ]

type simple_tag = [ event_tag | response_tag ]
...
let simple_tag_of_enum : int -> simple_tag option =
 fun i ->
...
let split_message (has_crc : bool) (buf : Cstruct.t) (tag : simple_tag) =
  let length =
    ...
    | `End_of_transmission -> sizeof_common_header
  in
...
type 'a simple_msg = { tag : 'a; body : Cstruct.t }
...
let simple_msg_to_string f m =
  Format.asprintf "tag: %s, data: %a" (f m.tag) Cstruct.hexdump_pp m.body

let make_complex_msg ?(request_id = Uint16.zero) ?(client_id = Uint16.zero)
    ?(body = Cstruct.empty) tag =
  { tag; client_id; request_id; body }

type 'a msg = [ `Complex of complex_msg | `Simple of 'a simple_msg ]
...
type _ t =
  ...
  | Set_mode : { input : input; t2mi_mode : t2mi_mode } -> unit t
  | Set_jitter_mode : jitter_mode -> unit t
  | Reset : unit t
  | Set_src_id : { input_source : int; t2mi_source : int } -> unit t
  | Get_t2mi_seq : {
      request_id : Uint16.t;
  ...
  | Get_section : {
      ...
      id_ext_1 : int option;
      (* ts_id for EIT, orig_nw_id for SDT *)
      id_ext_2 : int option;
      (* orig_nw_id for EIT *)
      section : int option;
    }
...
let to_string (type a) : a t -> string = function
  ...
  | Set_src_id { input_source; t2mi_source } ->
      Printf.sprintf "Set source ID (input=%d, t2mi=%d)" input_source
        t2mi_source
  | Get_deverr { request_id; timeout } ->
      Format.asprintf "Get deverr (rid=%a, timeout=%a)" Uint16.pp request_id
        pp_float_option timeout
  | Get_t2mi_seq { request_id; duration } ->
      Format.asprintf "Get T2-MI sequence (rid=%a, duration=%d)" Uint16.pp
        request_id duration
  | Get_section
      {
        request_id;
      ...
      Format.asprintf
        "Get section (rid=%a, stream=%a, table=%d, section=%a, table_ext=%a, \
         id_ext_1=%a, id_ext_2=%a)"
        Uint16.pp request_id Stream.Multi_TS_ID.pp stream_id table_id
        pp_int_option section pp_int_option table_id_ext pp_int_option id_ext_1
  ...
  | Get_structure { request_id; stream = `All } ->
      Format.asprintf "Get structure (rid=%a, all streams)" Uint16.pp request_id
  | Get_structure { request_id; stream = `Single s } ->
      Format.asprintf "Get structure (rid=%a, stream=%a)" Uint16.pp request_id
        Stream.Multi_TS_ID.pp s
  | Get_t2mi_info { request_id; t2mi_stream_id } ->
      Format.asprintf "Get T2-MI info (rid=%a, T2-MI stream ID=%d)" Uint16.pp
        request_id t2mi_stream_id
  | Set_mode { input; t2mi_mode = { enabled; pid; t2mi_stream_id; stream } } ->
      Format.asprintf
        "Set mode (input: %a, t2mi: enabled=%B, PID=%d, T2-MI stream ID=%d, \
         stream=%a)"
        pp_input input enabled pid t2mi_stream_id pp_stream stream
