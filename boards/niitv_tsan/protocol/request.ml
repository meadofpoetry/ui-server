...
let to_string (type a) : a t -> string = function
  ...
  | Set_src_id {input_source; t2mi_source} ->
      Printf.sprintf "Set source ID (input=%d, t2mi=%d)" input_source t2mi_source
  | Get_deverr {request_id; timeout} ->
      Format.asprintf
        "Get deverr (rid=%a, timeout=%a)"
        Uint16.pp
        request_id
        pp_float_option
        timeout
  | Get_t2mi_seq {request_id; duration} ->
      Format.asprintf
        "Get T2-MI sequence (rid=%a, duration=%d)"
        Uint16.pp
        request_id
        duration
  | Get_section
  ...
  | Get_t2mi_info {request_id; t2mi_stream_id} ->
      Format.asprintf
        "Get T2-MI info (rid=%a, T2-MI stream ID=%d)"
        Uint16.pp
        request_id
        t2mi_stream_id
  | Set_mode {input; t2mi_mode = {enabled; pid; t2mi_stream_id; stream}} ->
      Format.asprintf
        "Set mode (input: %a, t2mi: enabled=%B, PID=%d, T2-MI stream ID=%d, stream=%a)"
        pp_input
        input
        enabled
        pid
  ...
  | Set_jitter_mode {stream; pid; _} ->
      Format.asprintf
        "Set jitter mode (stream=%a, PID=%d)"
        Stream.Multi_TS_ID.pp
        stream
        pid
