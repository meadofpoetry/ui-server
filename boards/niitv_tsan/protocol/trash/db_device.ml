...
let init db =
  ...
  let insert_new =
    R.exec
      Types.(tup3 int ptime ptime)
      (sprintf
         "INSERT INTO %s (state,date_start,date_end)\n\
         \                VALUES (?,?,?)" table)
  in
...
let bump db state =
  ...
  let update_last =
    R.exec
      Types.(tup3 ptime ptime ptime)
      (sprintf
         "UPDATE %s SET date_end = ?\n\
  ...
  Conn.request db
    Request.(
      with_trans
        (find select_last () >>= function
         | None -> exec insert_new (new_state, now, now)
         | Some (state, st, en) ->
             if state = new_state then exec update_last (now, st, en)
...
let select_state db ?(limit = max_limit) ~from ~till =
  ...
  let select =
    R.collect
      ...
      Types.(tup3 int ptime ptime)
      (sprintf
         {|SELECT * FROM %s WHERE date_start <= $2 AND date_end >= $1
  ...
  Conn.request db
    Request.(
      ...
      let data =
        List.map
          (fun (st, from, till) -> { state = state_of_int st; from; till })
          l
      in
...
let select_state_compressed_internal db ~from ~till =
  ...
  let select_i =
    R.collect
      ...
      Types.(tup2 int ptime_span)
      (sprintf
         {|SELECT state, sum(date_end - date_start) FROM %s
  ...
  let select_l =
    R.find_opt
      Types.(tup2 ptime ptime)
      Types.(tup2 int ptime)
      (sprintf
         {|SELECT state, max(date_end) FROM %s
  ...
  let select_r =
    R.find_opt
      ...
      Types.(tup2 int ptime)
      (sprintf
         {|SELECT state, min(date_start) FROM %s
      ...
      | None ->
          ...
          let l =
            ...
            | Some (st, t) ->
                ...
                fun (s, v) ->
                  if Pervasives.( = ) s st then (s, Time.Span.add sp v)
                  else (s, v)
          in
          let r =
            match r with
            ...
            | Some (st, t) ->
                ...
                fun (s, v) -> if s = st then (s, Time.Span.add sp v) else (s, v)
          in
          let i =
            List.map
              (fun x ->
          ...
          let fine, init, no_response =
            List.fold_left
              (fun (f, i, n) -> function `Fine, x -> (f +. x, i, n)
                | `Init, x -> (f, i +. x, n)
                | `No_response, x -> (f, i, n +. x))
              (0., 0., 0.) i
          in
          return { fine; init; no_response })

let select_state_compressed db ~from ~till =
  select_state_compressed_internal db ~from ~till >|= fun data ->
  Compressed { data }
