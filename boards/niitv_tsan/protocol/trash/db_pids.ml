  ...
  Types.custom
    Types.(
      List.(
        ...
        & ptime))
    ~encode:
      (fun (id, ({ from; till; data = pid, data } : Pid.t Time.timespan)) ->
        ...
        ( ID.to_db id,
          ( pid,
            ( data.service_id,
              ( typ,
                ( data.has_pts,
                  (data.has_pcr, (data.scrambled, (data.present, (from, till))))
                ) ) ) ) ))
    ~decode:
      (fun ( id,
             ( pid,
               ( service_id,
                 ( typ,
                   (has_pts, (has_pcr, (scrambled, (present, (from, till))))) )
               ) ) ) ->
...
let insert db (pids : (ID.t * Pid.t Time.timespan list) list) =
  ...
  let insert =
    R.exec typ
      (sprintf
         {|INSERT INTO %s (stream,pid,service,type,
...
let bump db (pids : (ID.t * Pid.t Time.timespan list) list) =
  ...
  let data =
    List.map
      (fun (id, pids) ->
        List.map
          (fun ({ data = pid, _; till; _ } : Pid.t Time.timespan) ->
            (ID.to_db id, pid, till))
          pids)
      pids
    |> List.concat
  in
