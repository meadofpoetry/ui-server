...
module Event = struct
  ...
  let get_status (api : Protocol.api) _user =
    let event =
      E.map
        (fun (x : Parser.Status.t) -> status_to_yojson x.basic)
        api.notifs.status
    in
    Lwt.return event
  ...
  let get_errors (api : Protocol.api) _user =
    let event =
      E.map (Util_json.List.to_yojson Deverr.to_yojson) api.notifs.deverr
    in
    Lwt.return event

  let get_t2mi_mode (api : Protocol.api) _user =
    let event =
      E.map (fun (x : config) -> t2mi_mode_to_yojson x.t2mi_mode)
      @@ S.changes api.kv#s
    in
    Lwt.return event
end
...
let reset (api : Protocol.api) _user _body _env _state =
  api.channel Request.Reset >>=? fun () -> Lwt.return `Unit

let set_t2mi_mode (api : Protocol.api) _user body _env _state =
  match t2mi_mode_of_yojson body with
  | Error e -> Lwt.return (`Error e)
  | Ok t2mi_mode ->
      api.kv#get >>= fun ({ input; _ } as config) ->
      api.channel Request.(Set_mode { input; t2mi_mode }) >>=? fun () ->
      api.kv#set { config with t2mi_mode } >>= fun () -> Lwt.return `Unit

let set_input (api : Protocol.api) _user body _env _state =
  match input_of_yojson body with
  | Error e -> Lwt.return (`Error e)
  | Ok input ->
      api.kv#get >>= fun ({ t2mi_mode; _ } as config) ->
      api.channel Request.(Set_mode { input; t2mi_mode }) >>=? fun () ->
      api.kv#set { config with input } >>= fun () -> Lwt.return `Unit
...
let set_port (api : Protocol.api) port _user body _env _state =
  ...
  | Ok en -> (
      let input =
        match (input_of_enum port, en) with
        | Some i, true -> Some i
...
let set_jitter_mode (api : Protocol.api) _user body _env _state =
  ...
  | Error e -> Lwt.return (`Error e)
  | Ok jitter_mode ->
      api.channel Request.(Set_jitter_mode jitter_mode) >>=? fun () ->
      api.kv#get >>= fun config ->
      api.kv#set { config with jitter_mode } >>= fun () -> Lwt.return `Unit
...
let get_info (api : Protocol.api) force _user _body _env _state =
  ...
  | Some true ->
      api.channel Request.Get_devinfo >>=? return_value % devinfo_to_yojson

let get_errors (api : Protocol.api) timeout force _user _body _env _state =
  let timeout =
    match timeout with None -> None | Some x -> Some (int_ms_to_float_s x)
  in
  match force with
  | None | Some false ->
      let timeout =
        match timeout with None -> Fsm.status_timeout | Some x -> x
      in
      Lwt.pick
        [
...
let get_t2mi_mode (api : Protocol.api) force _user _body _env _state =
  ...
  | Some true ->
      Lwt.pick
        [
          Boards.Board.await_no_response api.notifs.state >>= not_responding;
          ( Util_react.E.next api.notifs.status >>= fun s ->
            Lwt.return_ok s.t2mi_mode );
          Fsm.sleep Fsm.status_timeout;
        ] )
