...
let error_to_string = function
  ...
  | Bad_crc (code, x, y) ->
      Printf.sprintf
        "invalid CRC in message with code = 0x%x,expected %d, got %d" code x y

module Part_id = struct
  type t = { code : int; request_id : Uint16.t; client_id : Uint16.t }
  [@@deriving eq, ord]
end

module Part = Map.Make (Part_id)

type part = { first : bool; param : int32; parity : bool; body : Cstruct.t }

type part_error = Unexpected_first | Unexpected_parity | Discontinuity
...
module Status = struct
  ...
  let parse_t2mi_versions (buf : Cstruct.t) =
    ...
    let rec aux acc = function
      ...
      | i ->
          let ver =
            Int32.shift_right v (4 * i) |> Int32.logand 0xfl |> Int32.to_int
          in
  ...
  let parse ?(timestamp = Ptime_clock.now ()) (msg : Cstruct.t) =
    ...
    in
    let t2mi_sync =
      non_zero_indexes_of_int ~n:8 (Message.get_status_t2mi_sync msg)
    in
    let input, t2mi_mode =
      make_mode
        ...
        @@ Message.get_status_t2mi_stream_id msg )
    in
    let jitter_mode =
...
module Section = struct
  let parse ?(timestamp = Ptime_clock.now ()) ~(table_id : int)
      ~(section : int option) (msg : Cstruct.t) =
    try
      ...
      let data =
        ...
        @@ Cstruct.fold
             (fun (acc : T2mi_sequence.item list) el ->
               ...
               let sframe_stream =
                 Message.get_t2mi_frame_seq_item_sframe_stream el
               in
               ...
               let count =
                 Int32.to_int @@ Message.get_t2mi_frame_seq_item_count el
               in
               ...
               let ts_packet =
                 Int32.to_int @@ Message.get_t2mi_frame_seq_item_time el
               in
               {
                 typ;
                 super_frame;
...
module Structure = struct
  ...
  type service_acc = { info : Cstruct.t; es : Cstruct.t; ecm : Cstruct.t }
  ...
  let service_acc_empty =
    { info = Cstruct.empty; es = Cstruct.empty; ecm = Cstruct.empty }
  ...
  let of_general_block (msg : Cstruct.t) =
    ...
    let nw_name =
      match Text_decoder.decode nw_name with Error _ -> "" | Ok s -> s
    in
    let bouquet_name =
      match Text_decoder.decode bq_name with Error _ -> "" | Ok s -> s
    in
    ( {
  ...
  let of_pids_block (msg : Cstruct.t) =
    let iter =
      Cstruct.iter (fun _ -> Some 2) (fun x -> Cstruct.LE.get_uint16 x 0) msg
    in
    Cstruct.fold
      (fun acc el ->
  ...
  let of_service_block (string_len : int) (msg : Cstruct.t) =
    ...
    in
    let provider_name =
      match Text_decoder.decode pn with Error _ -> "" | Ok s -> s
    in
    let info =
      {
        Service.name;
        ...
        service_type_list =
          Message.get_services_struct_block_service_type_list bdy;
  ...
  let of_es_block (msg : Cstruct.t) =
    ...
    @@ Cstruct.fold
         (fun acc x ->
           ...
           let info =
             {
               MPEG_TS.PID.Type.stream_type =
                 Message.get_es_struct_block_es_type x;
  ...
  let of_table_block (msg : Cstruct.t) =
    ...
    let eit_last_table_id =
      Message.get_table_struct_block_eit_last_table_id bdy
    in
  ...
  let split_into_blocks (msg : Cstruct.t) : acc =
    let rec aux service_flag msg acc =
      ...
      | _ ->
          let hdr, data =
            Cstruct.split msg Message.sizeof_struct_block_header
          in
          ...
          let acc, flag =
            ...
            | 0x2100 ->
                ({ acc with pids = Cstruct.append acc.pids block }, false)
            | 0x2200 ->
  ...
  let find_in_elements ((pid : int), _) elements =
    ...
    match pid with None -> None | Some (_, info) -> Some info
  ...
  let update_table services ((id : SI_PSI_table.id), (info : SI_PSI_table.t)) =
    ...
    | `EIT (`Actual, _) -> (
        let service =
          List.find_opt (fun (sid, _) -> sid = id.table_id_ext) services
        in
        match service with
        | None -> None
    ...
    | None -> (id, info)
    | Some (sid, name) ->
        (id, { info with service_id = Some sid; service_name = Some name })
  ...
  let parse_blocks timestamp (msg : Cstruct.t) =
    ...
    in
    let tables =
      List.rev_map (update_table services % of_table_block) acc.tables
    in
    let emm = of_emm_block acc.emm in
    let pids =
      List.map (update_pid elements tables emm) @@ of_pids_block acc.pids
    in
...
module Bitrate = struct
  let of_pids_bitrate total_pids br_per_pkt (buf : Cstruct.t) =
    let msg, rest =
      Cstruct.split buf (Message.sizeof_pid_bitrate * total_pids)
    in
    let iter =
  ...
  let of_tables_bitrate (total_tbls : int) (br_per_pkt : float)
      (buf : Cstruct.t) =
    let msg, _ =
      Cstruct.split buf (Message.sizeof_table_bitrate * total_tbls)
    in
    let iter =
...
module T2MI_info = struct
  ...
  let parse_l1 (body : Cstruct.t) =
    try
      let conf_len =
        Message.get_t2mi_info_ext_conf_len body |> fun x ->
  ...
  let parse ?(timestamp = Ptime_clock.now ()) (msg : Cstruct.t) =
    try
      ...
      | 0 ->
          let info =
            { T2mi_info.packets; t2mi_pid = None; l1 = None; timestamp }
          in
...
module TS_error = struct
  let parse ?(timestamp = Ptime_clock.now ()) (msg : Cstruct.t) =
    try
      ...
      let stream =
        Stream.Multi_TS_ID.of_int32_pure
        @@ Message.get_ts_errors_stream_id common
      in
      let iter =
        Cstruct.iter
...
module T2MI_error = struct
  type error = { code : int; t2mi_stream_id : int; param : int }
  ...
  let merge ~(count : error list) ~(param : error list) timestamp pid :
      Error.t list =
    List.map
      (fun (error : error) ->
        let param_1 =
  ...
  let parse_error (buf : Cstruct.t) =
    ...
    | 0 ->
        if param > 0 (* filter zero errors *) then
          `E { code; t2mi_stream_id; param }
        else `N
    | _ ->
        if code <> 0 (* t2mi parser error *) then
          `P { code; t2mi_stream_id; param }
        else if param <> 0 then
          `C { code = t2mi_parser_code; t2mi_stream_id; param }
        else `N
  ...
  let parse ?(timestamp = Ptime_clock.now ()) (msg : Cstruct.t) =
    try
      ...
      let stream =
        Stream.Multi_TS_ID.of_int32_pure
        @@ Message.get_t2mi_errors_stream_id common
      in
...
let parse_part src (buf : Cstruct.t) =
  try
    ...
    let param =
      Int32.mul 2l
        ( if long then Message.get_complex_rsp_header_ext_param buf
        else Int32.of_int @@ Message.get_complex_rsp_header_param buf )
    in
    let body =
      if long then Cstruct.shift buf Message.sizeof_complex_rsp_header_ext
    ...
    let id =
      {
        ...
        client_id =
          Uint16.of_int @@ Message.get_complex_rsp_header_client_id buf;
        request_id =
          Uint16.of_int @@ Message.get_complex_rsp_header_request_id buf;
      }
    in
    let part = { body; first = code_ext land 0x8000 <> 0; parity; param } in
    Logs.debug ~src (fun m ->
        m
          "parser - got complex message part (code: 0x%X, req_id: %a, \
           client_id: %a, first: %B, parity: %B, length: %d, param: %ld)"
          id.code Uint16.pp id.request_id Uint16.pp id.client_id part.first
          parity (Cstruct.len body) param);
    Ok (id, part)
  ...
  | ({ first = true; _ } as first) :: rest as sorted -> (
      try
        let length =
          List.fold_left
            (fun acc part ->
              if part.first = true then
                raise_notrace (Invalid_part Unexpected_first);
              if part.parity <> first.parity then
                raise_notrace (Invalid_part Unexpected_parity);
              if Int32.to_int part.param = acc then acc + Cstruct.len part.body
              else raise_notrace (Invalid_part Discontinuity))
            (Cstruct.len first.body) rest
        in
        if Int32.to_int first.param <> length then `P sorted
        else
          let acc = Cstruct.concat @@ List.map (fun x -> x.body) sorted in
          let message =
            if first.parity then fst @@ Cstruct.split acc (Cstruct.len acc - 1)
            else acc
          in
          `V message
      with
...
let deserialize ?(timestamp = Ptime_clock.now ()) src parts buf =
  let rec f ?error acc parts buf =
    if Cstruct.len buf < Message.sizeof_common_header then
      (List.rev acc, parts, buf)
    else
