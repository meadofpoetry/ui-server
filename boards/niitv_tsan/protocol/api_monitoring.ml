...
module Event = struct
  ...
  let filter_ids ids v = match filter_ids ids v with [] -> None | x -> Some x

  let get_bitrate (api : Protocol.api) ids _user =
    let to_yojson = stream_assoc_list_to_yojson Bitrate.cur_to_yojson in
    let event =
      E.fmap (Option.map to_yojson % filter_ids ids) api.notifs.bitrate
    in
    Lwt.return event
  ...
  let get_bitrate_with_stats (api : Protocol.api) ids _user =
    ...
    let event =
      E.fmap (Option.map to_yojson % filter_ids ids) api.notifs.bitrate_ext
    in
    Lwt.return event
  ...
  let get_si_psi_tables (api : Protocol.api) ids _user =
    ...
    let get_si_psi_tables =
      List.map (fun (id, x) -> (id, Structure.tables_ts x))
    in
    let event =
      api.notifs.structure
      |> S.changes
  ...
  let get_services (api : Protocol.api) ids _user =
    ...
    let get_services =
      List.map (fun (id, x) -> (id, Structure.services_ts x))
    in
    let event =
      api.notifs.structure
      |> S.changes
...
let filter_errors f x =
  match
    List.filter_map
      (fun (id, errors) ->
        match List.filter f errors with [] -> None | e -> Some (id, e))
      x
  with
  | [] -> None
  | x -> Some x
...
let get_bitrate_with_stats (api : Protocol.api) ids timeout _user _body _env
    _state =
  let timeout =
    match timeout with
    | None -> Fsm.status_timeout
...
let get_t2mi_info (api : Protocol.api) force ids t2mi_stream_ids _user _body
    _env _state =
  ( match force with
  | Some true -> (
        ...
        | t2mi_stream_id :: tl -> (
            ...
            let id =
              Boards.Util.List.find_map (fun (s : Stream.t) ->
                  match s.typ with T2MI -> Some s.id | _ -> None)
            ...
            | Some id ->
                let acc =
                  Boards.Util.List.Assoc.update ~eq:Stream.ID.equal
                    (function None -> Some [ x ] | Some l -> Some (x :: l))
                    id acc
                in
                loop acc tl )
      in
      match t2mi_stream_ids with [] -> loop [] (range 0 7) | l -> loop [] l )
  | None | Some false ->
      check_state api.notifs.state >>= fun () ->
      Lwt.return_ok @@ React.S.value api.notifs.t2mi_info )
  >>=? return_value
