...
type%cstruct common_header = { prefix : uint16_t; msg_code : uint16_t }
[@@little_endian]
...
type%cstruct req_t2mi_info = { rfu : uint8_t; t2mi_stream_id : uint8_t }
[@@little_endian]
...
type%cstruct t2mi_error = { index : uint16_t; data : uint16_t }
[@@little_endian]
...
type%cstruct board_errors = { count : uint32_t; errors : uint32_t [@len 17] }
[@@little_endian]

type%cstruct section = { length : uint16_t; result : uint16_t }
[@@little_endian]
...
type%cstruct struct_block_header = { code : uint16_t; length : uint16_t }
[@@little_endian]
...
type%cstruct ts_struct = { length : uint32_t; stream_id : uint32_t }
[@@little_endian]
...
type%cstruct pid_bitrate = { pid : uint16_t; packets : uint32_t }
[@@little_endian]
...
type%cstruct req_set_jitter_mode = { pid : uint16_t; stream_id : uint32_t }
[@@little_endian]

type%cstruct end_of_errors = { ts_count : uint16_t; rfu : uint16_t }
[@@little_endian]
