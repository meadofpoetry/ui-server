...
type send_client =
  Request.rsp ts React.event -> event Lwt_stream.t -> (unit, Request.error) result Lwt.t
  ...
  | Some source ->
      let (typ : Stream.stream_type) =
        if status.t2mi_mode.enabled
           && (match status.t2mi_sync with
              | [] -> false
              | _ -> true)
           && Stream.Multi_TS_ID.equal
                id
...
let log_unexpected_event src expected got =
  let expected = String.concat "/" @@ List.map Request.simple_tag_to_string expected in
  let got =
    match got with
    | `Status _ -> "Status"
    | `Streams _ -> "Streams"
...
let wait_status req events f =
  ...
  let rec loop () =
    ...
    >>= fun status -> if f status then Lwt.return_ok () else loop ()
  in
...
let request
  ...
  | Set_jitter_mode m ->
      wait_status req events (fun status -> equal_jitter_mode status.jitter_mode m)
  | Set_mode m ->
...
let start
    ...
    Lwt_stream.filter_map
      (fun {data; timestamp} ->
        ...
        let res =
          ...
          | {tag = `Ts_errors; body} ->
              Parser.TS_error.parse ~timestamp body >>= fun x -> Ok (`Ts_errors x)
          | {tag = `T2mi_errors; body} ->
              Parser.T2MI_error.parse ~timestamp body >>= fun x -> Ok (`T2mi_errors x)
  ...
  and detect () =
    ...
    let rec loop () =
      ...
      >>= function
      | `E (`Status _) ->
          Logs.debug (fun m -> m "The device was already initialized, got status event");
      ...
      | `R {data = `Simple {tag = `Devinfo; body}; _} -> (
        ...
        | Ok info as x ->
            Logs.debug (fun m ->
                m
                  "The device is waiting, for initialization, got device info event: %a"
                  pp_devinfo
                  info);
            Lwt.return x)
      | _ -> loop ()
  ...
  and initialize () =
    ...
    >>= fun () ->
    sender.send Request.(Set_jitter_mode jitter_mode)
    >>= fun () ->
    ...
    | Ok () -> idle ()
    | Error e ->
        Logs.err (fun m -> m "Initialization failed: %s" @@ Request.error_to_string e);
        restart ()
  and idle () =
    set_state `Fine;
  ...
  and wait_streams acc =
    ...
    | Error e ->
        Logs.err (fun m ->
            m "Error while waiting for streams event: %s" @@ Request.error_to_string e);
        Lwt.return_error e
  and wait_errors acc =
    let wait_event =
      Lwt_stream.next evt_queue
      ...
      | `End_of_transmission as rsp -> (
        ...
        | _ ->
            ...
            Lwt.return_error (Request.Custom "Unexpected event"))
      | rsp ->
          let exp = [`T2mi_errors; `Ts_errors; `End_of_errors; `End_of_transmission] in
    ...
    | Error e ->
        Logs.err (fun m ->
            m "Error while waiting for errors event: %s" @@ Request.error_to_string e);
        Lwt.return_error e
  and wait_eot acc =
    Lwt.pick
      [ sleep event_timeout
  ...
  and finalize_events ({status; streams; errors; _} as acc) =
    ...
    | Ok probes ->
        ...
        let raw_streams =
          List.filter_map (to_raw_stream ~input_source ~t2mi_source status) streams
        in
        let step = React.Step.create () in
