...
module Event = struct
  ...
  let get_bitrate_with_stats ?(ids = []) sock control =
    ...
    Api_js.Websocket.JSON.subscribe
      ~path:
        Path.Format.("board" @/ Int ^/ "monitoring/bitrate/with-stats" @/ empty)
      ~query:Query.[ ("id", (module List (Stream.ID))) ]
      control ids of_yojson sock
...
let get_errors ?(ids = []) ?timeout ?(pids = []) ?(priority = []) control =
  ...
  Api_http.perform ~meth:`GET
    ...
    control ids timeout pids priority
...
let get_bitrate_with_stats ?(ids = []) ?timeout control =
  ...
  Api_http.perform ~meth:`GET
    ~path:
      Path.Format.(
        "api/board" @/ Int ^/ "monitoring/bitrate/with-stats" @/ empty)
    ~query:
      Query.
        [
          ("id", (module List (Stream.ID))); ("timeout", (module Option (Int)));
...
let reset_bitrate_stats ?(ids = []) control =
  Api_http.perform_unit ~meth:`POST
    ~path:
      Path.Format.(
        "api/board" @/ Int ^/ "monitoring/bitrate/reset-stats" @/ empty)
    ~query:Query.[ ("id", (module List (Stream.ID))) ]
    control ids
...
let get_ts_info ?force ?(ids = []) control =
  ...
  Api_http.perform ~meth:`GET
    ...
    ~query:
      Query.
        [ ("id", (module List (Stream.ID))); ("force", (module Option (Bool))) ]
    control ids force
    (ignore_env_bind (Lwt.return % map_err % of_yojson))
...
let get_t2mi_info ?force ?(ids = []) ?(t2mi_stream_ids = []) control =
  ...
  Api_http.perform ~meth:`GET
    ...
    control ids t2mi_stream_ids force
    (ignore_env_bind (Lwt.return % map_err % of_yojson))
