...
let set_port ~port state control =
  Api_http.perform_unit ~meth:`POST
    ...
    ~query:Query.empty control port ignore_env
...
let get_errors ?force ?timeout control =
  ...
  Api_http.perform ~meth:`GET
    ...
    ~query:
      Query.
        [
          ("force", (module Option (Bool))); ("timeout", (module Option (Int)));
        ]
