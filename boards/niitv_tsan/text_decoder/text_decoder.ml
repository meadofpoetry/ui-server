...
let equal_encoding x1 x2 =
  encoding_to_int x1 = encoding_to_int x2
...
let get_encoding (text : Cstruct.t) =
  ...
  try
    ...
    | x when x > 0x00 && x <= 0x0B ->
      let encoding = match int_to_encoding (x + 4) with
    ...
    | 0x10 ->
      (match Cstruct.BE.get_uint16 text 1 with
       | x when x < 17 ->
         let encoding = match int_to_encoding x with
           | None -> raise Not_found
           | Some x -> x in
...
let trim (buf : Cstruct.t) =
  try
    ...
    if Cstruct.get_uint8 buf 0 = 0
    then Cstruct.empty
    else trim_cstruct buf
...
let get_encoding_and_convert (text : Cstruct.t) =
  ...
  | enc ->
    ...
    | ISO8859_11 | ISO8859_12 | ISO8859_13
