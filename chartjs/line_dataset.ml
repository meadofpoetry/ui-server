...
class virtual t_base ~(label:string) (o:#t_js Js.t) () =
object(self)
...
class ['a,'b] t ~(label:string)
  ...
  object(self)
    ...
    method private tl = Js.array_get _obj##.data (self#length - 1)
    ...
    method private set_data_no_sort (data:('a,'b) point list) =
      _obj##.data := Js.array @@ Array.of_list @@ List.map (fun x -> self#point_to_js x) data;
    ...
    method length = _obj##.data##.length

    (** Data points *)
    method data : ('a,'b) point list =
      List.map (fun x -> self#point_of_js x) (Array.to_list @@ Js.to_array _obj##.data)
    ...
    method push (x:('a,'b) point) =
      ...
      | Some tl when cmp tl.x x.x = 0 -> _obj##.data##pop |> ignore;
                                         self#ps_remove 1 `Tail;
                                         self#push x
      | _                             -> _obj##.data##push (self#point_to_js x) |> ignore;
                                         self#ps_push [x] `Tail;
                                         self#update_max ()
