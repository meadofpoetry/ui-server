...
class t () =
  ...
  object(self)
    ...
    method color : clr = match Cast.to_list ~f:CSS.Color.ml _obj##.color with
      | Some l -> `Several l
      | None   -> `Single (CSS.Color.ml @@ CSS.Color.js_t_of_js_string @@ Js.Unsafe.coerce _obj##.color)
    method set_color : clr -> unit = function
      | `Single c  -> let v = Js.string @@ CSS.Color.string_of_t c in
                      _obj##.color := Js.Unsafe.coerce v
    ...
    method line_width : lw = match Cast.to_list ~f:(fun x -> x) _obj##.lineWidth with
      | Some l -> `Several l
      | None   -> `Single (int_of_float @@ Js.float_of_number (Js.Unsafe.coerce _obj##.lineWidth))
    method set_line_width : lw -> unit = function
      | `Single w  -> _obj##.lineWidth := Js.Unsafe.coerce @@ Js.number_of_float @@ float_of_int w
      | `Several l -> _obj##.lineWidth := Js.Unsafe.coerce @@ Js.array @@ Array.of_list l

    (** If true, draw border at the edge between the axis and the chart area. *)
    method draw_border : bool = Js.to_bool _obj##.drawBorder
    method set_draw_border x = _obj##.drawBorder := Js.bool x

    (** If true, draw lines on the chart area inside the axis lines.
      This is useful when there are multiple axes and you need
      to control which grid lines are drawn. *)
    method draw_on_chart_area : bool = Js.to_bool _obj##.drawOnChartArea
    method set_draw_on_chart_area x = _obj##.drawOnChartArea := Js.bool x

    (** If true, draw lines beside the ticks in the axis area beside the chart. *)
    method draw_ticks : bool = Js.to_bool _obj##.drawTicks
    method set_draw_ticks x = _obj##.drawTicks := Js.bool x

    (** Length in pixels that the grid lines will draw into the axis area. *)
    method tick_mark_length : int = _obj##.tickMarkLength
    method set_tick_mark_length x = _obj##.tickMarkLength := x

    (** Stroke width of the grid line for the first index (index 0). *)
    method zero_line_width : int = _obj##.zeroLineWidth
    method set_zero_line_width x = _obj##.zeroLineWidth := x

    (** Stroke color of the grid line for the first index (index 0). *)
    method zero_line_color : CSS.Color.t = CSS.Color.ml _obj##.zeroLineColor
    method set_zero_line_color x = _obj##.zeroLineColor := CSS.Color.js x

    (** Length and spacing of dashes of the grid line for the first index (index 0). *)
    method zero_line_border_dash : int list = Array.to_list @@ Js.to_array _obj##.zeroLineBorderDash
    method set_zero_line_border_dash x = _obj##.zeroLineBorderDash := Js.array @@ Array.of_list x

    (** Offset for line dashes of the grid line for the first index (index 0). *)
    method zero_line_border_dash_offset : int = _obj##.zeroLineBorderDashOffset
    method set_zero_line_border_dash_offset x = _obj##.zeroLineBorderDashOffset := x

    (** If true, grid lines will be shifted to be between labels.
      This is set to true in the bar chart by default. *)
    method offset_grid_lines : bool = Js.to_bool _obj##.offsetGridLines
    method set_offset_grid_lines x = _obj##.offsetGridLines := Js.bool x
