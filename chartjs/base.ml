...
module Cast = struct
  ...
  let typeof x = Js.typeof x |> Js.to_string

  let to_bool x : bool option =
    if String.equal (typeof x) "boolean"
    then Some (Js.to_bool @@ Js.Unsafe.coerce x)
    else None

  let to_int x : int option =
    if String.equal (typeof x) "number"
  ...
  let to_js_string x : Js.js_string Js.t option =
    if String.equal (typeof x) "string"
  ...
  let to_object x =
    if String.equal (typeof x) "object"
    then Some (Js.Unsafe.coerce x)
    else None
...
let easing_of_string_exn x =
  ...
  let f pre s =
    let get_typ rest = (match pre with
                        | x when String.equal x in_s     -> `Ease_in (animation_type_of_string_exn rest)
                        | x when String.equal x out_s    -> `Ease_out (animation_type_of_string_exn rest)
                        | x when String.equal x in_out_s -> `Ease_in_out (animation_type_of_string_exn rest)
