...
let file_size_to_string (x : int) =
  ...
  let rec aux acc = function
    | [] -> acc
    | unit :: tl ->
        if fst acc > threshold then aux (fst acc /. threshold, unit) tl else acc
  in
...
module Selector = struct
  open Page_server_settings_tyxml.Certificate

  let filename = Printf.sprintf ".%s" CSS.filename

  let action_info = Printf.sprintf ".%s" CSS.action_info

  let action_remove = Printf.sprintf ".%s" CSS.action_remove
  ...
  object (self)
    ...
    val info_dialog : Dialog.t option =
      let f (type a) : a typ -> Dialog.t option = function
        | Key -> None
        | Crt -> Some (make_certificate_dialog ())
      in
      f typ
    ...
    method set_value (v : 'a) : unit =
      ...
      let name =
        match filename_of_value v typ with
        | None -> Page_server_settings_tyxml.Certificate.empty
        | Some name -> name
      in
