...
struct
  ...
  let make_icon_button icon = Icon_button.create ~icon:Icon.SVG.(create_of_d icon)
  ...
  module HTTPS = struct
    ...
    let make ?classes ?(attrs = []) (v : Server_types.settings) : 'a elt =
      let submit =
        Button.create ~classes:[Card.CSS.action] ~label:"Применить" ()
      in
      let enable_input_id = enable_id ^ "-input" in
      let switch = Switch.create ~input_id:enable_input_id ~checked:v.https_enabled () in
      let enable =
        Form_field.create
          ~attrs:[a_id enable_id]
          ~label:
            (Form_field.create_label
  ...
  module Certificate = struct
    ...
    let make_row ?(classes = []) ?(attrs = []) ?name typ =
      ...
      let class_, icon, title =
        match typ with
        | `Crt ->
            CSS.Certificate.certificate, Svg_icons.certificate, "Сертификат"
        | `Key ->
            CSS.Certificate.private_key, Svg_icons.key, "Приватный ключ"
      in
      let classes = CSS.Certificate.row :: state :: class_ :: classes in
      let icon = Icon.SVG.create_of_d icon in
      let import =
        make_import_button
      ...
      div
        ...
        ; div ~a:[a_class [CSS.Certificate.actions]] (import :: (info ^:: [remove])) ]
