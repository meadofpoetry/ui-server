...
let make_icon path =
  Tyxml.Html.toelt @@ Components_tyxml.Icon.F.SVG.icon ~d:path ()

let get_certificate_info (path : string) =
  Futil_lwt.File.read path >>= function
...
let settings_of_config (config : Server.Config.t) =
  ...
  | Some name -> (
      get_certificate_info (config.tls_path / name) >>= function
  ...
  >>=
  fun res ->
  let tls_cert = match res with Ok x -> x | Error _ -> None in
  let tls_key =
    match config.tls_key with None -> None | Some x -> Some (Uri.pct_decode x)
  in
  Lwt.return
    { Server_types.https_enabled = config.https_enabled; tls_cert; tls_key }
...
module Event = struct
  ...
  let get_config (conf : Server.config) _user =
    let event =
      E.map_s (fun x ->
          settings_of_config x >>= fun x ->
          Lwt.return @@ Server_types.settings_to_yojson x)
      @@ S.changes conf#s
    in
    Lwt.return event
end
...
let get_config (conf : Server.config) _user _body _env _state =
  conf#get >>= fun config ->
  settings_of_config config >>= fun x ->
  Lwt.return (`Value (Server_types.settings_to_yojson x))
...
let add_file validate setter (conf : Server.config) name _user body _env _state
  ...
  | Ok (name, body) -> (
      ...
      Futil_lwt.File.write ~create:true path body >>= function
      | Error _ -> Lwt.return (`Error "could not save the file")
      | Ok () ->
          conf#set (setter settings (Some name)) >>= fun () -> Lwt.return `Unit
      )
...
let remove_file getter setter (conf : Server.config) _user _body _env _state =
  ...
  | Some name ->
      ...
      >>= fun _ ->
      conf#set (setter settings) >>= fun () -> Lwt.return `Unit
...
let validate_name ((name, _) as x) =
  if String.contains name '/' then Error "filename must not contain separators"
  else Ok x
...
let remove_cert =
  remove_file (fun x -> x.tls_cert) (fun x -> { x with tls_cert = None })

let remove_key =
  remove_file (fun x -> x.tls_key) (fun x -> { x with tls_key = None })

let handlers (config : Server.config) =
  let open Api_http in
  Api_http.make ~prefix:"server"
    [
      node ~doc:"Server configuration" ~meth:`GET
        ~path:Path.Format.("config" @/ empty)
        ~query:Query.empty (get_config config);
      node ~doc:"Restart" ~restrict:[ `Operator; `Guest ] ~meth:`POST
        ~path:Path.Format.("restart" @/ empty)
        ~query:Query.empty
        (fun _user _body _env _state ->
          Server.kill_server ();
          Lwt.return `Unit);
      node ~doc:"Set https flag" ~restrict:[ `Operator; `Guest ] ~meth:`POST
        ~path:Path.Format.("config/https-enabled" @/ empty)
        ~query:Query.empty (set_https config);
      node_raw ~doc:"Set tls certificate" ~restrict:[ `Operator; `Guest ]
        ~meth:`POST
        ~path:Path.Format.("config/crt" @/ String ^/ empty)
        ~query:Query.empty (add_crt config);
      node_raw ~doc:"Set tls private key" ~restrict:[ `Operator; `Guest ]
        ~meth:`POST
        ~path:Path.Format.("config/key" @/ String ^/ empty)
        ~query:Query.empty (add_key config);
      node_raw ~doc:"Delete tls certificate" ~restrict:[ `Operator; `Guest ]
        ~meth:`DELETE
        ~path:Path.Format.("config/crt" @/ empty)
        ~query:Query.empty (remove_cert config);
      node_raw ~doc:"Delete tls private key" ~restrict:[ `Operator; `Guest ]
        ~meth:`DELETE
        ~path:Path.Format.("config/key" @/ empty)
        ~query:Query.empty (remove_key config);
    ]
...
let ws (config : Server.config) =
  ...
  make ~prefix:"server"
    [
      event_node ~doc:"Server configuration" ~restrict:[ `Operator; `Guest ]
        ~path:Path.Format.("config" @/ empty)
        ~query:Query.empty (Event.get_config config);
    ]
...
let pages : 'a. unit -> 'a Api_template.item list =
 fun () ->
  ...
  let props =
    make_template_props ~title:"Настройки безопасности"
