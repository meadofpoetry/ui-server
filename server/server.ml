open Lwt.Infix
module Api_http =
  Api_cohttp.Make (Application_types.User) (Application_types.Body)
...
let create_config kv = Serv_conf.create ~default:Config.default kv [ "server" ]

let server_stop, server_kill = Lwt.wait ()

let kill_server () = Lwt.wakeup_later server_kill ()
...
let handler ~resources_path ~auth_filter ~forbidden ~not_found ~routes =
  ...
  let resource base uri user =
    Cohttp_lwt_unix.Server.(
      ...
      respond_file ~fname () >>= fun ((rsp, _body) as resp) ->
  ...
  fun (conn : Conduit_lwt_unix.flow * Cohttp.Connection.t)
    ...
    let default = resource resources_path path in
    (*let root, path = Common.Uri.(Path.next uri.path) in*)
    Cohttp_lwt.Body.to_string body >>= fun body ->
    Api_http.handle routes ~state:sock_data ~forbidden ~default ~meth ~env
      ~redir:auth_filter uri body

let create ~forbidden ~not_found (config : config) auth_filter
    (routes : Api_http.t) =
  config#get >>= fun settings ->
  let resources_path =
    try Futil.Path.to_explicit_exn settings.resources
    with _ -> Config.default.resources
  in
  let callback =
    handler ~resources_path ~auth_filter ~forbidden ~not_found ~routes
  in
  let server = Cohttp_lwt_unix.Server.make_response_action ~callback () in
  let http_mode = `TCP (`Port settings.http_port) in
  let http_server =
    Cohttp_lwt_unix.Server.create ~stop:server_stop ~mode:http_mode
      ~on_exn:(fun e ->
  ...
  then (
    Logs.warn (fun m -> m "Https server was not created due to: not configured");
    http_server )
  else
    try
      ...
      if (not (Sys.file_exists key_path)) || not (Sys.file_exists crt_path) then (
        Logs.warn (fun m ->
        ...
        in
        let tls_server =
          Cohttp_lwt_unix.Server.create ~mode ~stop:server_stop
            ~on_exn:(fun e ->
              Logs.err (fun m ->
