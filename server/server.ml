...
let create ~forbidden ~not_found (config : config) auth_filter (routes : Api_http.t) =
  ...
  in
  let callback = handler ~resources_path ~auth_filter ~forbidden ~not_found ~routes in
  ...
  then (
    ...
    try
      ...
      if (not (Sys.file_exists key_path)) || not (Sys.file_exists crt_path)
      then (
        Logs.warn (fun m ->
        ...
        let tls_server =
          Cohttp_lwt_unix.Server.create
            ...
            ~stop:server_stop
            ~on_exn:(fun e ->
              Logs.err (fun m -> m "(TLS Server) Exception: %s" (Printexc.to_string e)))
            server
        in
        Lwt.pick [http_server; tls_server]
    with _ ->
      Logs.warn (fun m -> m "Https server was not created due to: unexpected error");
      http_server
